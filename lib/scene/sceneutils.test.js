...
sinon.stub(fut, 'getModifyTimeMs').returns(Date.now());
sinon.stub(fut, 'writeFileP').resolves();
sinon.stub(fut, 'readFileP').resolves(script1);
sinon.stub(fut, 'readdirP').resolves(['script1.js']);

const appconfig = require('../appconfig');
sinon.stub(appconfig, 'getScriptFilename').returns('script/script1');
sinon.stub(appconfig, 'getReqScriptFilename').returns('req/script1');
sinon.stub(appconfig, 'getScriptPath').returns('script');
