...
async function getSceneDocs() {
  return (await dm.dbstore.get('scenes')).filter(doc => !doc.folder);
}

async function getTypeScenes() {
  const arr = await dm.dbstore.get('types');
  const res = [];
  // Для одной записи м б до 3 сценариев
  arr.forEach(item => {
    if (item.scriptOnChange) add(item, '_OnChange');
    if (item.scriptOnInterval) add(item, '_OnInterval');
    if (item.scriptOnSchedule) add(item, '_OnSchedule');
  });
  return res;

  function add(item, event) {
    const id = item._id;
    const par = 'par' + event; // par_OnChange
    const parVal = item[par] != undefined ? item[par] : '';
    // Для cron - нужно вытащить par2_OnSchedule
    res.push({ _id: id + '_' + event, multi: 1, type: id, par: parVal, startEvent: event});
  }
}
