...
function removeScriptFile(sceneId) {
  scriptUnlink(appconfig.getScriptFilename(sceneId));
}
...
function scriptUnlink(filename) {
  fs.unlink(filename, err => {
    if (err) {
      if (err.code != 'ENOENT') console.log('Error unlink script file:' + filename + util.inspect(err));
