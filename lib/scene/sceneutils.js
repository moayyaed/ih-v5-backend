 ...
 */
function processScript(scriptStr, sceneId) {
  const result = {};
  const parseResult = selectCommentDescriptAndScript(scriptStr);
  // const { comment, descript, scriptstr } = selectCommentDescriptAndScript(scriptStr);
  if (parseResult.comment) {
    Object.assign(result, parseComment(parseResult.comment)); // name, version
  }
  if (!parseResult.scriptstr) throw { message: 'Not found "script" clause' };

  if (parseResult.descript) {
    Object.assign(result, parseDevice(parseResult.descript)); // {multi, devs, triggers, realdevs, def};
  }
...
function parseDevice(astr) {
  ...
  return { multi, devs: devs.join(','), triggers: triggerStr || triggers.join(','), realdevs: realdevs.join(','), checkStr, def };
}
