...
async function getTypeScenes() {
  const arr = await dm.dbstore.get('types', {usemain:1});
  return arr.map(item => {
    const res = {};
    const when = item.main_when;
    if (when == 'interval') {
      res.startInterval = item.main_when_interval; 
    } else if (when == 'schedule') {
      res.startSchedule = item.main_when_schedule;
    } else {
      res.startOnChange = '*';
    }
    return {_id:item._id+'_MAIN', multi:1, type:item._id, ...res}
  });
}
