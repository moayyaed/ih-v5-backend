 ...
 *  - ведения списка используемых устройствами сценариев
 *  - организации процесса debug
 *
...
class Scenengine {
   ...
   *
   * @param {Object} doc - документ из таблицы scene
   * @param {Array of Objects} calls - наборы из таблицы scenecall
   */
  addScene(doc, calls) {
    const data = this.addSceneItem(doc, calls);
    if (data.length) this.holder.deviceWorker.postMessage({ name: 'add:scenes', data });
  }
  ...
  addSceneItem(doc, calls) {
    ...
    if (oneScene.multi) {
      const arr = [];
      this.multis[oneScene.sceneId] = new Set();
   ...
   *  - отправляет на worker сообщение unrequire:scene
   *
   * @param {String} sceneId - id сценария (scen002)
   */
  async removeScene(sceneId) {
    const data = [];
    if (!this.multis[sceneId]) {
      // Простой сценарий
      const item = this.removeOneInstance(sceneId);
      if (item) data.push(item);
    } else {
      // Все экземпляры мультисценария
      for (const callId of this.multis[sceneId]) {
        const item = this.removeOneInstance(sceneutils.getOneInstanceId(sceneId, callId));
        if (item) data.push(item);
      }
    }
    if (data.length) this.holder.deviceWorker.postMessage({ name: 'remove:scenes', data });
  ...
  /** updateScene
   * Изменение сценария в целом (простой сценарий или все экземпляры мульти)
  ...
  addSceneCall(oneScene, call) {
    ...
    if (oneInstance) this.holder.deviceWorker.postMessage({ name: 'add:scenes', data: [oneInstance] });
  }

  /** removeSceneCall
   * Удаление одного экземпляра, отправка на worker
   * @param {String} callId - id из документа таблицы scenecall без привязки к сценарию:
   *               'call_004'. Запись в scenecall уже удалена
   */
  removeSceneCall(callId) {
    try {
      const sceneId = this.findSceneIdForCallId(callId);
      if (!sceneId) throw { message: 'Not found sceneId for scenecall=' + callId };

      const id = sceneutils.getOneInstanceId(sceneId, callId);
      const item = this.removeOneInstance(id);
      if (item) this.holder.deviceWorker.postMessage({ name: 'remove:scenes', data: [item] });
    } catch (e) {
      console.log('ERROR: removeSceneCall. ' + e.message + '. Skipped');
    }
  }
