...
class Scenengine {
  ...
  addScene(doc) {
    ...
    if (this.worksceneSet[id]) {
      debug = this.worksceneSet[id].debug;
    }
    this.worksceneSet[id] = new Worksceno(id, this.agent);
    this.worksceneSet[id].debug = debug;
    
    if (doc.blk || doc.err) {
      // Сценарий с ошибкой - блокировать, не запускать
      this.worksceneSet[id].blk = 1;
      this.worksceneSet[id].error = doc.errstr;
      return;
    }
