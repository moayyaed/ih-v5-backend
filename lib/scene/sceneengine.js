...
class Scenengine {
  ...
  */
  ...
  addScene(doc) {
    const id = doc._id;
    const filename = appconfig.getReqScriptFilename(id);
    this.sceneSet[id] = new Sceno(id, filename, this.getActualParams(doc, id), this.agent);

    if (doc.devs && doc.extprops) {
      // Добавить свойства устройствам
      this.addExtPropsFromScene(id, doc.devs, doc.extprops);
    }
  }
