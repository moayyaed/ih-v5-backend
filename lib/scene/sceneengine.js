...
class Scenengine {
  ...
  async updateScene(doc, calls) {
    ...
    this.addSceneItem(doc, calls);
    this.holder.deviceWorker.postMessage({ name: 'update:scenes', data: [this.sceneSet[sceneId]] });
  }
  ...
  async removeScene(sid) {
    ...
    console.log('SCENEENGINE removeScene sid='+sid+' sceneId='+sceneId)
    let single;
    let unreq;
    if (sid == sceneId) {
  ...
  removeSceneCall(callId) {
    ...
    console.log('SCENEENGINE removeSceneCall callId='+callId+' sceneId='+sceneId)
    if (sceneId) {
