...
class Sceno {
  constructor(name, path, devs, agent) {
    this.name = name;
    this.blk = 0;
    ...
    const script = require(path)(devs);
    Object.assign(this, script);
    if (!this.hasScript('start', 'function')) throw { message: 'Missing function start' };
  ...
  assign(dobj, prop, value) {
    this.agent.emit('assign', this.name, dobj, prop, value);
  }

  do(dobj, command, value) {
    this.agent.emit('doCommand', this.name, dobj, command, value);
  }

  doAll(filter, command, value) {
    this.agent.emit('doAll', this.name, filter, command, value);
  }

  pluginCommand(command, callback) {
    this.agent.emit('pluginCommand', this.name, command, callback);
  }
  ...
  exit() {
    this.agent.emit('exit', this.name);
  }

  log(txt) {
    this.agent.emit('log', this.name,  txt);
  }

  syslog(txt) {
    this.agent.emit('syslog', this.name,  txt);
  }

  getSysTime(name, date) {
    // ??
    // return this.agent.getSysTime(this.name, name, date);
  }

  dbwrite(payload, tablename, columns) {
    // return this.agent.dbwrite(this.name, payload, tablename, columns);
    this.agent.emit('dbwrite', this.name, payload, tablename, columns);
  }

  dbread(fobj, func) {
    const id = String(Date.now());
    this.responsefn[id] = func;
    // this.agent.dbread(this.name, fobj, id);
    this.agent.emit('dbread', this.name,  fobj, id);
  }

  snap(fobj, func) {
    const id = String(Date.now());
    this.responsefn[id] = func;
    this.agent.emit('snap', this.name, fobj, id);
  }
  ...
  startTimer(timername, interval, call) {
    ...
    let tsobj = this.agent.emit('startTimer', this.name, timername, interval);
  ...
  stopTimer(timername) {
    if (this.timerSet[timername]) {
      this.agent.emit('stopTimer', this.name, timername, this.timerSet[timername].interval);
