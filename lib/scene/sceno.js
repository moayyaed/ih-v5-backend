...
class Sceno {
  ...
  exit() {
    this.agent.emit('sceneExit', this.name);
  }

  sendSMS(mess) {
    console.log(this.name + ' sendSMS: ' + mess);
  }
  ...
  snap(fobj, func) {
    const id = String(Date.now());
    this.responsefn[id] = func;
    this.agent.snap(this.name, fobj, id);
  }
  ...
  execOS(txt, func) {
    if (func) {
      const id = String(Date.now());
      this.responsefn[id] = func;
      this.agent.execOS(this.name, txt, id);
    } else this.agent.execOS(this.name, txt);
  }
  ...
  startTimer(timername, interval, call) {
    ...
    let tsobj = this.agent.startTimer(this.name, timername, interval);
  ...
  stopTimer(timername) {
    if (this.timerSet[timername]) {
      this.agent.stopTimer(this.name, timername, this.timerSet[timername].interval);
