...
const sceneSet = {}; // объекты сценариев
const devRefs = {}; // Ключ - dn реального устройства - параметры сценариев
const debugScene = {};
const debugWorkscene = {};

//    devRefs['STEMP1'] = Set {<id scnene1>, <id scnene2>}

exports.start = function(holder) {
  const scriptpath = appconfig.get('scenepath') + '/script';

  // Запустить механизм таймеров для интервальных таймеров сценариев c мин интервалом 100 мс
  const tm = new timerman.Timerman(0.1);
  tm.on('ready', onTimerReady);

  // Обработка внешних событий - houser
  /*
  holder.on('data', onDeviceData);
  holder.on('scene', onSceneChange);
  holder.on('schedule', onScheduleChange);
  holder.on('startscene', onStartScene);
  holder.on('stopscene', onStopScene);
  holder.on('blockscene', onBlockScene);
  holder.on('blocksch', onBlockSch);
  holder.on('debugctrl', onDebugCtrl);
  */
  ...
  function onTimerReady(timeobj) {
    ...
    if (timeobj && timeobj.owner && timeobj.tname) {
      ...
      if (
        sceneSet[name] &&
        sceneSet[name].isActive() &&
        sceneSet[name].getTimerState(timername) == 1 &&
        timeobj.sts >= sceneSet[name].__laststart
      ) {
        sceneSet[name].setTimerState(timername, 2, timeobj);
        debugMsg(name, 'Done timer ' + timername);
