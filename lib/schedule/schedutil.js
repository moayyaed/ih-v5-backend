...
function getTime(name, date) {
  return name == 'sunrise' || name == 'sunset' || predefined.includes(name)
    ? getPredefinedTime(name, date)
    : getCronTime(name);
}

function getPredefinedTime(name, date) {
  const dt = new Date();
  let resDate = dt;
  if (name == 'sunrise' || name == 'sunset') {
    resDate = getSunTime(name, date || dt); //
  } else {
    resDate = getCalendarTime(name, dt);
  }
...
function getSunTime(name, date, location) {
  location = location || appconfig.getLocation();
