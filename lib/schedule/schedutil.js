...
function getTimeForRule(rule) {
  ...
  if (typeof rule == 'object' && rule.when) {
    if (predefined.includes(rule.when)) {
      ...
      let cdate = findDate(today, rule);

      if (!hut.isTheSameDate(new Date(), cdate)) cdate = setZeroTime(cdate);

      const ts = getCalendarTime(rule.when, cdate).getTime();
    ...
    if (rule.when == 'cycle') {
      return calcTimerQts({ ...rule, whenId: 3 });
    }
  }
}

function setZeroTime(dt) {
  dt.setHours(0);
  dt.setMinutes(0);
  dt.setSeconds(0);
...
function checkRule(rule) {
  ...
  if (!rule.everyday && !rule.weekdays && !rule.dates && rule.when != 'cycle') throw { message: 'Ежедневно или выберите дни' };
  if (rule.when == 'intime' && !rule.timehhmm) throw { message: 'Время не определено' };

  if (rule.when == 'cycle') {
    if (!rule.period) throw { message: 'Период не определен! Формат (00:00:00)' };
    if (!checkPeriodStr(rule.period)) throw { message: 'Неверный период ' + rule.period + '. Формат (00:00:00)' };
  }
}
...
function getSunTime(name, date, location) {
  ...
  if (!location || !location.lat || !location.lng) {
    console.log('ERROR: Location not set! It is impossible to find out the time of ' + name);
    return 0;
  }
...
function findDate(startdt, item) {
  if (item.weekdays) return getDateForDaysOfWeek(startdt, weekDaysArray(item));
