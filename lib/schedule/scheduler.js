 ...
 *
 *  schedengine.js
 *  Составляет план действий по расписанию, созданному в системе - таблица schedrule
 *  Добавляет таймеры в sctm и сохраняет в структуру ?? (было выполнено/будет выполнено)
 *  Отрабатывает действия по своему расписанию, которые вызываются через onScheduleReady (owner:'sched006') - конкретный пункт
 *   перевзводит таймер и структуру было/будет (schedcurrent - хранит данные для каждого пункта расписания)
 *
 * schedmate.js
 *  Слушает события изменения расписания (добавление, удаление, блокировка, изменение условий)
 *
 *
