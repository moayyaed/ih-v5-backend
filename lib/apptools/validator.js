...
async function validateForm(metaUpForm, recsToWrite, dm) {
  const errdata = {};
  // Проверка records
      ...
      if (data[mainprop] && typeof data[mainprop] == 'object') {
        for (const prop of Object.keys(data[mainprop])) {
          ...
          if (data[mainprop] && typeof data[mainprop][prop] == 'object') {
            if (valObj && (valObj[prop] || valObj['*'])) {  // Не разрешаются пустые поля в dropliste
  ...
  async function checkProp({ rule, prop, doc, _id, table }) {
    ...
    if (rule.custom) {
      const result = await dm.datamaker.customValidate({ rule, prop, doc, _id, table }, dm);
