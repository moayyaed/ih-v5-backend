...
async function insert(body, dm) {
  ...
  if (body.type == 'subtree') {
    ...
    console.log('treeUpdate insert body='+util.inspect(body))
    const treeObj = await dm.getCachedSubTree({id: body.id, nodeid:navnodeid});

    await ordering.exec({ payloadItem: payload, treeObj: { children: treeObj }, treeDesc: desc, parentid, previd }, dm);

    const newDocs = payload.map(item => Object.assign(item, { folder: item.popupid && item.popupid.endsWith('folder') ? 1 : 0, navnodeid }));
    ...
    for (const rootid in payload) {
      ...
      const treeObj = await dm.getCachedTree({id:rootid});
...
async function copy(body, dm) {
  ...
  if (body.type == 'subtree') {
    ...
    const desc = descriptor.getDescItem('tree', body.id);
    const treeObj = await dm.getCachedSubTree({id: body.id, nodeid:body.navnodeid });
...
async function remove(body, dm) {
  ...
  if (body.type == 'subtree') {
    ...
    if (payload.folders) {
      const tree = await dm.getCachedSubTree({id: body.id, nodeid:body.navnodeid });
    ...
    for (const rootid in payload) {
      ...
      if (payload[rootid].folders) {
        const tree = await dm.getCachedTree({id:rootid});
