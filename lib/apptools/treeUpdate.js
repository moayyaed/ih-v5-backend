...
async function insert(body, dm) {
  ...
  if (body.type == 'subtree') {
    ...
    const treeObj = await dm.getCachedSubTree(body.id, navnodeid);
...
async function copy(body, dm) {
  ...
  if (body.type == 'subtree') {
    ...
    const desc = descriptor.getDescItem('tree', body.id);
    const treeObj = await dm.getCachedSubTree(body.id, nodeid);
...
async function remove(body, dm) {
  ...
  if (body.type == 'subtree') {
    ...
    if (payload.folders) {
      const tree = await dm.getCachedSubTree(body.id, body.navnodeid);
