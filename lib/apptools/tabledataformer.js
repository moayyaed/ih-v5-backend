 ...
 * НЕ ИСПОЛЬЗУЕТСЯ
 * Функционал перенесен в datamanager - dm.tabledatFit
 *                    и в formdataGet - formTableData
 */
...
async function formTableData(dataFromTable, table, nodeid, pitem, holder) {
  ...
  return formColumnVals(pitem.columns, arr, holder, nodeid);
}
...
async function formColumnVals(columns, dataArr, holder, nodeid ) {
  const dm = holder.dm;
  // Обработка полей типа droplist, подготовить списки
  const dropLists = await formDroplists(columns, holder, nodeid);
...
async function formDroplists(columns, holder, nodeid) {
  ...
  for (let item of larr) {
    ...
    } else if (nodeid && item.data && item.data.startsWith('__')){
      const lst = await specreq.getSpecDroplist({id:item.data, nodeid}, holder);
      droplists[item.prop] = lst && lst.data ? lst.data : [];
    } else {
      // item.data - это имя списка
      const listdata = await holder.dm.datagetter.getDroplist({id:item.data}, holder); // => {data:arr}
      console.log('TF formDroplists listdata='+util.inspect(listdata))
