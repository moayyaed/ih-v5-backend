...
const metaUpFormGet = require('./metaUpFormGet');
...
async function update(body, holder) {
  ...
  let { id, nodeid, navnodeid, rowid, payload } = body;
  const res = {};
  const insert = {};
  const remove = {};
  const formId = id;

  const metaUpFormData = await dm.getCachedData({ type: 'upform', id, nodeid, method: 'getmeta' }, metaUpFormGet);
  const metaUpForm = metaUpFormData.data;
  // console.log('UPDATE nodeid='+nodeid+' id='+id+'  metaUpForm ='+util.inspect(metaUpForm))
    ...
    if (payload[item.cell]) {
      ...
      Object.keys(payload[item.cell]).forEach(field => {
        if (spec.includes(field)) {
          ...
          dm.datamaker.processInputval({table:item.table, nodeid, value:payload[item.cell][field]}, holder);
      ...
      for (const xrowid in payloadItem) {
        ...
        for (const columnField in rowObj) {
          if (spec.includes(columnField) && typeof rowObj[columnField] == 'object') {
            ...
            dm.datamaker.processInputval({table:item.table, nodeid, rowId:xrowid, value:rowObj[columnField]}, holder);
            delete rowObj[columnField];
          }
        }
      }
  ...
  // console.log('FORMMETHODS UPDATE result =' + util.inspect(result, null, 4));
    ...
    for (const field of Object.keys(exfieldtype)) {
      ...
      if (payload[item.cell] && payload[item.cell][field]) {
        ...
        if (chItem && chItem.table && chItem.doc) {
          if (!recsToWrite[chItem.table]) recsToWrite[chItem.table] = { _id: chItem._id || nodeid, doc: {} };
  ...
  */
    ...
    if (recsToWrite[table].table) {
      ...
      for (const rid in recsToWrite[table].table) {
        if (recsToWrite[table].table[rid] == null) {
          // Удаление строки
          // Если строки нет - то и удалять не надо (только что добавленная строка)
  ...
  async function formForOne(table, desc) {
    ...
    const olddoc = await dm.datamaker.findOrAddDoc({ collection: desc.collection, id: _id, navnodeid, formId }, dm);
    const resDoc = olddoc;
    ...
    if (recsToWrite[table].doc) {
      ...
      // if (newDoc.pwdx) newDoc.pwdx = Buffer.from(newDoc.pwdx).toString('base64');
