...
class Informengine {
  ...
  start(docs) {
    ...
    this.holder.on('send:info', data => {
      // data = {infotype, sendObj:{txt, dest:'admin'}}
      try {
        this.sendInfo(data.infotype, data.sendObj);
      } catch (e) {
        console.log('ERROR: send:info data='+util.inspect(data))
      }
      // console.log('INFORMER send:info data='+util.inspect(data))
    });
