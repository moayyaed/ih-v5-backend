...
class Informengine {
  ...
  addInfoaddr(doc) {
    ...
    // console.log('AFTER addInfoaddr '+util.inspect( this.infoaddrSet, null, 4))
  }
  ...
  removeInfoaddr(doc) {
    ...
    // console.log('AFTER removeInfoaddr '+util.inspect( this.infoaddrSet, null, 4))
  }
  ...
  sendInfo(unitId, sendObj) {
    ...
    const data = sendObj;
    if (!sendObj.sendTo) {
      // Нужно формировать адреса!!
      sendObj.sendTo = this.formToSend(unitId, sendObj.dest);
    }
