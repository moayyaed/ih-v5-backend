...
class Informengine {
  ...
  async start() {
    ...
    /*
    this.holder.on('show:dialog', data => {
      // data = { dialogId:'d002', dest:'admin', sender:'scene_scen042', context:{} }

      try {
        if (!data.dialogId) throw { message: 'Missing dialogId!' };
        // Проверить, что получатель (группа) существует, преобразовать в массив  
        data.dest = this.getDestArray(data.dest)
        // Проверить, что диалог такой существует
      
        this.holder.emit('showdialog', data);

      } catch (e) {
        console.log('ERROR: send:info data=' + util.inspect(data) + '. Reason:' + util.inspect(e));
        this.emitDebugMsg(data.sender, 'ERROR Send info! ' + e.message);
      }
    });
  */
  }
  ...
  getDestArray(dest) {
    if (dest && this.groupSet[dest]) {
      // Это группа
      if (!this.groupSet[dest].size) throw { message: appconfig.getMessage('EmptyGroup') + ': ' + dest };
      return Array.from(this.groupSet[dest]);
    }

    // Проверить, что пользователь существует
    if (!liststore.hasItem('userList', dest)) {
      throw { message: appconfig.getMessage('UserNotFound') + ': ' + dest };
    }
    return [dest];
  }
  ...
  formToSend(infotype, dest) {
    ...
    userRecs.forEach(rec => {
      if (rec.allowed) toSend.push({ addr: rec.addr, sign: rec.sign, userId: rec.userId });
    });
