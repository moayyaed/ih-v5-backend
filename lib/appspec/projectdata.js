...
async function joinContainersForLayout(layoutId, rt, holder) {
  ...
  if (rt) {
    for (const id of ids) {
      ...
      const oneCont = getRtForContainerElements(data, holder);
      ...
      if (typeof data.elements[el] == 'object') {
        ...
        Object.keys(data.elements[el]).forEach(visprop => {
          if (typeof data.elements[el][visprop] == 'object' && data.elements[el][visprop].enabled) {
            const did = data.elements[el][visprop].did;
            const prop = data.elements[el][visprop].prop;
            if (!did.startsWith('local') && isProp(holder, did, prop)) {
              if (!res[el]) res[el] = {};
...
function getPropVal(holder, did, prop) {
  ...
  if (isGlobalVar(did)) return holder.glSet.getValue(did);
  if (isLocalVar(did)) return 0;
  return holder.devSet[did][prop] || 0;
  // const val = isGlobalVar(did) ? holder.glSet.getValue(did) : holder.devSet[did][prop];
...
async function getUpPobj(query) {
      ...
      Object.keys(pobj.elements[el]).forEach(visprop => {
        if (
          ...
          if (pobj.elements[el][visprop].enabled) {
            ...
            if (pobj.elements[el][visprop].left && Array.isArray(pobj.elements[el][visprop].left)) {
              // processActionsArray(pobj.elements[el][visprop].left);
            }
          }
        }
      });
  ...
  return res;
/*
  function processActionsArray(arr) {
    arr.forEach(item => {
