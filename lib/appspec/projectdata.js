...
async function getWidgetdata(cmd, query, holder) {
  ...
  for (const el of Object.keys(pobj.elements)) {
    if (
      ...
      res[pobj.elements[el].type] = await getOneWidgetData(pobj.elements[el].type);
    }
  }
  return res;

  async function getOneWidgetData(widget) {
    // Для каждого виджета сформировать данные
    // Устройство м б в widgetlinks напрямую или через contextId
    let arr;
    const did = 'd0007';
...
async function getDataByContext(query, holder) {
  /*
  
    Object.keys(dataObj.elements[contextObj.element].links).forEach(lprop => {
      const didFromLink = dataObj.elements[contextObj.element].links[lprop].did;

      res.state = getStaticObj(holder, didFromLink, 'state');
    });
  }
  */
}
