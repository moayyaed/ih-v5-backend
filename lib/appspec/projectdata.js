  ...
  arr
    ...
    .forEach(item => {
      ...
      if (elems.length > 1 && elems[0] && elems[1]) {
        // frame_1,vc002,d042
        const [frame, vc, device, multichart_id, timelinechart_id] = elems;
        if (!res) res = {};
        res[frame] = { vc, device, multichart_id, timelinechart_id };
      }
    });
  return res;
}
...
function gatherFramesWithWidgetlinksFromLayoutData(dataObj) {
  ...
  function getFrameLink(item) {
    ...
    if (item.value) {
      ...
      res.journal_id = item.value.journal_id_id ? item.value.journal_id_id.id : '';
