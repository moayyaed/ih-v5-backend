...
async function getWidgetdata(cmd, query, holder) {
  ...
  for (const el of Object.keys(pobj.elements)) {
    ...
    if (pobj.elements[el] != null && typeof pobj.elements[el] == 'object' && pobj.elements[el].type && wtypes.includes(pobj.elements[el].type)) {
      widgets.push()
    }
  }
...
async function getRtForContainerElements(id, query, holder) {
  ...
  for (const did of Object.keys(uppobj)) {
    if (did == '__template') {
      ...
      // contextId=layout:l007__container:vc046__element:button_1
      const contextObj = querystring.parse(query.contextId, '__', ':');
      if (!contextObj.element) throw { message: 'Missing element in ' + query.contextId };

      if (contextObj.container) {
        // Нужно найти в контейнере элемент и его links
        const dataObj = await getCachedProjectObj('container', contextObj.container);
        if (!dataObj) throw { message: 'Missing container ' + contextObj.container };
        if (!dataObj.elements) throw { message: 'Missing elements in container ' + contextObj.container };
        if (!dataObj.elements[contextObj.element])
          throw { message: 'Missing element ' + contextObj.element + ' in container ' + contextObj.container };
        if (!dataObj.elements[contextObj.element].links)
          throw { message: 'Missing links for ' + contextObj.element + ' in container ' + contextObj.container };
        ...
        Object.keys(dataObj.elements[contextObj.element].links).forEach(lprop => {
          ...
          // res.template = getStaticObj(holder, tempdid, 'state');
