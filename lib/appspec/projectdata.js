...
async function getDidsFromLinksByContext(query) {
  ...
  if (dataObj.elements[contextObj.element] && dataObj.elements[contextObj.element].links) {
...
async function getDidsAndPropsFromLinksByContext(query) {
  ...
  if (dataObj.elements[contextObj.element] && dataObj.elements[contextObj.element].links) {
    Object.keys(dataObj.elements[contextObj.element].links).forEach(visprop => {
      const one = dataObj.elements[contextObj.element].links[visprop];
      resObj[visprop] = {did:one.did, prop:one.prop};
    });
  }
  */
  return resObj;
...
async function getElementLinks(id, nodeid) {
...
async function getUpPobj(query) {
  ...
  const res = {};
console.log('getUpPobj '+util.inspect(query))
  // Получить исходный объект
  const pobj = await getCachedProjectObj(id, nodeid);
  if (!pobj || !pobj.elements) return {};
  ...
  for (const el of Object.keys(pobj.elements)) {
    ...
    if (pobj.elements[el] != null && typeof pobj.elements[el] == 'object') {
      ...
      if (isChart(pobj.elements[el], 'chart_multi')) {
      ...
      } else if (isChart(pobj.elements[el], 'chart')) {  // график с одним значением - подписка на did_prop
        ...
        /*
        if (pobj.elements[el].widgetlinks.link && pobj.elements[el].widgetlinks.link.prop) {
          // Могли передать реальное значение: widgetlinks.link.id = did
          // или через контекст: widgetlinks.link.id = '__device' (или пусто?)

          const dn_prop = pobj.elements[el].widgetlinks.link.dn+'.'+pobj.elements[el].widgetlinks.link.prop;
          if (!res.charts) res.charts = {};
          if (!res.charts[dn_prop]) res.charts[dn_prop] = dn_prop;
        }
        */

      } else if (isWidgetOfType(pobj.elements[el], 'devicelog')) {
        if (!res.devicelog) res.devicelog = {};
        res.devicelog[pobj.elements[el].widgetlinks.link.id] = pobj.elements[el].widgetlinks.link.id;
      }
    }
  }
  // });
