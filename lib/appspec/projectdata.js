...
async function getWidgetdata(cmd, query, holder) {
  ...
  const pobj = await getCachedProjectObj(cmd, query.id);
  ...
  for (const el of Object.keys(pobj.elements)) {
    if (
      ...
      res[pobj.elements[el].type] = await getOneWidgetData(pobj.elements[el].type, did, holder);
    }
  }
  return res;

  async function getDidForWidget(elObj) {
    if (elObj.widgetlink && elObj.widgetlink.link) {
      let wdid;
      if (elObj.widgetlink.link.title && elObj.widgetlink.link.title.startsWith('template.')) {
        // Искать по контексту
        const prop = elObj.widgetlink.link.prop;
        const dids = await getDidsFromLinksByContext(query);
        // нужен link для prop?
        wdid = dids[prop];
      } else {
        wdid = elObj.widgetlink.link.did;
      }
      return wdid;
    }
  }
  ...
  switch (widget) {
    case 'devicelog':
      ...
      return arr.map(item => ({
        ...
        message: 'Значение: ' + item.val
      }));
    // return [{title:'17.09 13:00:01', message:'Test1'}, {title:'17.09 13:00:02', message:'Test2'}, {title:'17.09 13:00:03', message:'Test3'}];
...
async function getRtForContainerElements(id, query, holder) {
  ...
  for (const did of Object.keys(uppobj)) {
    if (did == '__template') {
      ...
      // contextId=layout:l007__container:vc046__element:template_3
      /*
      if (!query.contextId) throw { message: 'Missing contextId!' };

      const contextObj = querystring.parse(query.contextId, '__', ':');
      const dataObj = await getContextPobj(query.contextId, contextObj);

      Object.keys(dataObj.elements[contextObj.element].links).forEach(visprop => {
        const didFromLink = dataObj.elements[contextObj.element].links[visprop].did;
        res[visprop] = getStaticObj(holder, didFromLink);
      });
      */
      const dids = await getDidsFromLinksByContext(query);
      Object.keys(dids).forEach(visprop => {
        res[visprop] = getStaticObj(holder, dids[visprop]);
      });
    } else {
...
async function getDidsFromLinksByContext(query) {
  if (!query.contextId) throw { message: 'Missing contextId!' };
  const resObj = {};
  const contextObj = querystring.parse(query.contextId, '__', ':');
  const dataObj = await getContextPobj(query.contextId, contextObj);
  ...
  Object.keys(dataObj.elements[contextObj.element].links).forEach(visprop => {
    resObj[visprop] = dataObj.elements[contextObj.element].links[visprop].did;
  });
  return resObj;
}

async function getContextPobj(contextId, contextObj) {
  if (!contextObj.element) throw { message: 'Missing element in ' + contextId };
  if (!contextObj.container && !contextObj.layout) throw { message: 'Expected container or layout in ' + contextId };

  const contType = contextObj.container ? 'container' : 'layout';
  const contId = contextObj[contType];
  const contLabel = contType + ' ' + contId;

  // Нужно найти в контейнере элемент и его links
  const dataObj = await getCachedProjectObj(contType, contId);
  if (!dataObj) throw { message: 'Missing ' + contLabel };
  if (!dataObj.elements) throw { message: 'Missing elements in  ' + contLabel };
  if (!dataObj.elements[contextObj.element])
    throw { message: 'Missing element ' + contextObj.element + ' in ' + contLabel };
  if (!dataObj.elements[contextObj.element].links)
    throw { message: 'Missing links for ' + contextObj.element + ' in ' + contLabel };
  return dataObj;
}
