...
async function getDataVis(cmd, query, holder) {
  if (query.rt) return getRtData(cmd, query, holder);
...
async function getRtData(cmd, query, holder) {
  switch (cmd) {
    case 'layout':
      return getRtForContainerElements('layout', query.id, holder);
...
async function getRtForContainerElements(id, nodeid, holder) {
  ...
  console.log('getRtForContainerElements uppobj='+util.inspect(uppobj))
            ...
            if (pobj.elements[el][visprop].type == 'multi') {
              Object.keys(pobj.elements[el][visprop]).forEach(key => {
                if (key.startsWith('action')) {
                  ...
                  if (item.left) processActionsArray(item.left);
                  if (item.right) processActionsArray(item.right);
                }
                /* "actions": {
                  "type": 'multi',
                  "action_1": {
  ...
  function processActionsArray(arr) {
    ...
    arr.forEach(item => {
      if (item && item.command == 'setval') {
        addToRes(item.did, item.prop, { el: item.action });
      }
    });
  }
