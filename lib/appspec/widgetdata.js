...
async function getWidgetdata(cmd, query, holder) {
  ...
  if (cmd == 'layout') {
    ...
    return getWidgetdataForLayoutFrames(query, holder);
  }
...
async function getWidgetdataFor(cmd, query, holder) {
  console.log('WARN: getWidgetdataFor query.frames=' + util.inspect(query.frames));
  let framesObj;
  let firstFrame;
  if (query.frames && query.frames != 'null') {
    framesObj = projectdata.parseFrames(query.frames);
  }
  console.log('WARN: getWidgetdataFor framesObj=' + util.inspect(framesObj));

  if (framesObj && Object.keys(framesObj).length > 0) {
    firstFrame = framesObj[Object.keys(framesObj)[0]];
  }
  console.log('WARN: getWidgetdataFor firstFrame=' + util.inspect(firstFrame));
  return getWidgetdataForOne(cmd, { ...firstFrame, id: query.id }, holder);
}
...
async function getWidgetdataForLayoutFrames(query, holder) {
  ...
  for (const frame of defFrames) {
    ...
    if (id) {
      const data = await getWidgetdataForOne('container', { ...frameObj, id }, holder);
      resObj[frameId] = data;
    }
  }
  return resObj;
