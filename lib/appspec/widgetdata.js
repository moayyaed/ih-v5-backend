...
async function getWidgetdataForOne(cmd, query, holder) {
    ...
    if (elObj.widgetlinks && elObj.widgetlinks.link && elObj.widgetlinks.link.id) {
      ...
      if (id == '__chart') {
        if (query.frame && query.frame != 'null') {
          const framesObj = projectdata.parseFrames(query.frame);
          id = framesObj.multichart_id;
        }
      } 
  ...
  async function getChartTimelineMetaData(elObj) {
    ...
    if (elObj.widgetlinks && elObj.widgetlinks.link && elObj.widgetlinks.link.id) {
      // Вернуть описание графика
      const doc = await holder.dm.findRecordById('timelinechart', elObj.widgetlinks.link.id);
