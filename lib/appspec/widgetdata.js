...
async function getWidgetdata(cmd, query, holder) {
  ...
  for (const el of Object.keys(pobj.elements)) {
    if (
      ...
      const did = await getDidForWidget(pobj.elements[el], query.contextId);
      res[el] = await getOneWidgetData(pobj.elements[el].type, did, holder);
    }
  }
  return res;

  async function getDidForWidget(elObj, contextId) {
    if (contextId && contextId != 'undefined') return contextId;

    if (elObj.widgetlinks && elObj.widgetlinks.link) return elObj.widgetlinks.link.did;
  }

  async function getDidForWidget2(elObj) {
