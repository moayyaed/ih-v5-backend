...
async function getWidgetdataForLayoutFrames(query, holder) {
  ...
  for (const frame of defFrames) {
    ...
    if (id) {
      const data = await getWidgetdataForOne('container', { ...frameObj, id, contextId: query.contextId }, holder);
      resObj[frameId] = data;
    }
  }
  return resObj;
...
function getExtPropArrayByScenes(dobj, holder) {
  ...
  function getExtPropsByScenes() {
    ...
    console.log('WARN: '+dobj.dn+' devExtProps '+util.inspect(devExtProps));
