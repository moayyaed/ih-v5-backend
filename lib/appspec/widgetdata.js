...
async function getWidgetdataForFrames(query, holder) {
  ...
  const ids = projectdata.gatherFramesFromLayoutData(dataObj).map(item => item.id); // { id: name, title }

  console.log('WARN: query.frames='+util.inspect(query.frames)+'  ids='+util.inspect(ids))
  let framesObj;
  ...
  for (const frameId of ids) {
    if (framesObj && framesObj[frameId]) {
      ...
      const id = framesObj[frameId].vc;
      if (id) {
        const data = await getWidgetdataForOne('container', { id, ...framesObj[frameId] }, holder);
        resObj[frameId] = data;
      }
    }
  }
...
async function getWidgetdataForOne(cmd, query, holder) {
  ...
  async function getChartTimelineMetaData(elObj) {
    ...
    if (elObj.widgetlinks && elObj.widgetlinks.link && elObj.widgetlinks.link.id) {
      ...
      let recId = elObj.widgetlinks.link.id;
      if (recId == '__timelinechart') {
        if (query.timelinechart_id) {
          recId = query.timelinechart_id;
        }
      }
      const doc = await holder.dm.findRecordById('timelinechart', recId);
      ...
      if (doc) {
        ...
        resObj.colors = {};
        if (doc.colors) {
          Object.keys(doc.colors).forEach(id => {
            const item = doc.colors[id];
