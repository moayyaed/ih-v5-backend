...
function getExtPropArrayByScenes(dobj, holder) {
  ...
  const extPropsByScenes = getExtPropsByScenes(); // {scen001:[{name, note,..}]}
...
function getMobileWidgetdata(dobj, holder) {
  const result = {
    style: {
      margin: 8,
      padding: 16
  ...
  const els = [
    { name: 'auto', note: 'Автоматический режим', type: 'cb', val: 0 },
    { name: 'timeOff', note: 'Время работы без движения', type: 'number', val: 30 },
    { name: 'reautoTimeAfterOn', note: 'Переход в Авто после включения', type: 'input', val: 15 },
    { name: 'reautoTimeAfterOff', note: 'Переход в Авто после выключения', type: 'input', val: 25 }
  ];
  els.forEach(el => {
    addOneLineItem(el.name, el, {});
    addDivider();
  });
  return result;
  ...
  function addOneLineItem(prop, el, titleProps) {
    const dn = dobj.dn;
    const value = dobj[prop];
    const inputObj = { dn, ...getInputObject(prop, el), ...titleProps };
    inputObj.title = el.note;
    inputObj.proportion = '55%'; // ???
