...
async function getWidgetdata(cmd, query, holder) {
  ...
  for (const el of Object.keys(pobj.elements)) {
    if (pobj.elements[el] != null && typeof pobj.elements[el] == 'object' && pobj.elements[el].widget) {
      if (pobj.elements[el].type == 'chart') {
  ...
  async function getChartMetaData(elObj) {
    ...
    if (elObj.widgetlinks && elObj.widgetlinks.link && elObj.widgetlinks.link.id) {
      ...
      if (doc) {
        ...
        res.data_type = doc.data_type;
        res.chart_type = doc.chart_type;
        res.leftaxis_title = doc.leftaxis_title;
        res.leftaxis_min = doc.leftaxis_min;
        res.leftaxis_max = doc.leftaxis_max;
        res.rightaxis = doc.rightaxis;
  ...
  function getDidForWidget(elObj, contextId) {
    ...
    if (elObj.widgetlinks && elObj.widgetlinks.link && elObj.widgetlinks.link.did) return elObj.widgetlinks.link.did;
