...
async function getLogData(query, holder) {
  ...
  const sql = 'SELECT * FROM ' + doc.variant + sqlutil.formLogWhere(doc, startTsid) + ' ORDER BY tsid DESC LIMIT ' + limit;
console.log('WARN: SQL='+sql)

  const recs = await holder.logconnector.read(sql);
