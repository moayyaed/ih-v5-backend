...
async function formSysdeviceArray() {
  ...
  for (const doc of unitDocs) {
    if (!doc.folder && doc.plugin && doc._id) {
      const sysObj = getUnitIndicatorObj(doc._id);
      res.push(sysObj);
...
function getUnitIndicatorObj(unitId) {
  const _id = '__UNIT_' + unitId;
  return { _id, dn: _id, sys: 1, name: 'Индикатор плагина ' + unitId, type: typestore.getSysIndicatorTypeId() };
}
