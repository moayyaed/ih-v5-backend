...
const { prepareMobileData, rebuidMobileLists } = require('../mobile/mobiledata');
...
async function getTreeData(id, filter, dm) {
  const data = await dm.getCachedData({type:'tree', id}, dataformer.getTree);
...
async function getContent(name, query, dm) {
  console.log('dgetter getContent '+name)
  if (name == 'docs') {
    // вернуть страницу документации
    const {id, alias, lang} = query;
    return getDocPage(id, lang);
  } 
}

async function getDocPage(id, lang) {
  const filename = appconfig.getDocPageFilename(id, lang);
  console.log('getDocPage '+filename)
  if (fs.existsSync(filename))  return fut.readFileP(filename);
