...
function getDeviceFormUrl(did) {
  switch (getDeviceKind(did)) {
    ...
    default:
      return 'dev/devices/deviceview/' + did + '/tabDeviceCommon';
  }
}
...
function formTitle(listname, item) {
  ...
  switch (listname) {
    ...
    case 'dbagentList':
      emo = item.id == appconfig.get('dbname') ? emObj[1] + ' ' : '';
      return emo + item.title;
    default:
...
function formSmartbutton2Obj(table, colItem, item, val) {
  console.log('WARN: formSmartbutton2Obj val='+val)
  switch (colItem.prop) {
    case 'dn_prop':
  ...
  function getObjByDn_prop(dn_prop) {
    console.log('WARN: formSmartbutton2Obj getObjByDn_prop ='+dn_prop)
    const [dn, prop] = dn_prop.spllit('.');
    // По dn найти did 
    const id = getDeviceDidByDn(dn);
    console.log('WARN: getDeviceDidByDn '+dn+' ='+id)
...
function getScenescriptPath(id) {
  return 'scenes/scenes/scenescript/' + id + '/tabSceneCodeEditor';
}
