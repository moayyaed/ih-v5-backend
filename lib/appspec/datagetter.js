...
async function getAlertLogData(query, holder) {
  ...
  const filter = {};
  const columns = [];
  return {data: getAlertLogRows(filter, columns, holder)};
}

// Записи алертов - для UI и PM
...
async function getAlertLogRows(filter, columns, holder) {
  ...
  const arr = docs.sort(hut.byorder('tsStart')).map(item => ({
    ...item,
    id: item._id,
  ...
  switch (table) {
    case 'alertjournal':
      return [
        ...
        { title: 'Оператор', prop: 'user', width: 200 }
      ];
    default:
      return [
