...
function getGlobalTitle(did) {
  const item = liststore.getItemFromList('globalList', did);
  return !item || !item.dn ? did + ': Global var not found' : formTitle('globalList', item);
}
...
function getTitleLinkObj(table, id, item) {
  ...
  if (table.indexOf('global') >= 0) return getGlobalLinkObj(table, id, item);
  return { title: item.title, path: '' };
}

function getGlobalLinkObj(table, id, item) {
  return { title: item.title, path: 'dev/globals/globalview/' + id + '/tabGlobalVarCommon' };
