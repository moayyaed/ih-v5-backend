...
function getRealtimeValues(table, id, nodeid, holder) {
  switch (table) {
    case 'devicecommonTable':
      console.log('getRealtimeValues ' + table);
...
// Получить данные журнала, пока без фильтра
async function getLogData(query, holder) {
  console.log('GET LOG DATA '+util.inspect(query))

  const {id='mainlog', ts, count} = query;
  const startTs = hut.isTs(ts) ? Number(ts) : 0;
  const limit = Number(count) || 100;
  const recs = await holder.dm.getLog(id, startTs ? {ts:{$lt:startTs}} : {}, {limit, sort:{ts:-1} })
  console.log('LOG getLogData recs='+util.inspect(recs))
  const data = recs.map(rec => ({id:rec.ts, dts:hut.getDateTimeFor(new Date(rec.ts)), txt:rec.txt}));
  console.log('LOG getLogData data='+util.inspect(data))
  return {data};
}
