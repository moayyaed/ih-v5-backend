...
async function getLogData(query, holder) {
  ...
  return doc.src == 'alertlog' ? getAlertLogData({}, holder) : getMainLogData(query, doc, holder);
}


async function getMainLogData({ rowid, count }, doc, holder) {
  const columns = [];
  if (doc.props) {
  ...
  console.log('INFO: getMainLogData SQL '+sql)
  const recs = await logconnector.read(sql);
  console.log('INFO: getMainLogData result: '+util.inspect(recs))
...
async function getAlertLogData(filter={}, holder) {
