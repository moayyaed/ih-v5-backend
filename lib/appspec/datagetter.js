...
async function getLogData(query, holder) {
  ...
  const { id, variant = 'mainlog', count } = query;
  const startTsid = id == 0 ? '' : id ;

  const limit = Number(count) || 1000;
  const recs = await holder.getLog(variant, startTsid ? { tsid: { $lt: startTsid } } : {}, { limit, sort: { tsid: -1 } });
 
  const data = recs.map(rec => ({ id: rec.tsid, dts: hut.getDateTimeFor(new Date(rec.ts)), txt: rec.txt }));
