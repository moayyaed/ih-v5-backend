...
async function getLogData(query, holder) {
  console.log(' getLogData query '+util.inspect(query))
  // id - номер журнала
  ...
  // Получен результат
  const data = recs.map(rec => ({ id: rec.tsid, dts: hut.getDateTimeFor(new Date(rec.ts)), txt: rec.txt, level: rec.level, tags: rec.tags }));
  return { data };
}
