...
function chooseTreeItemComponent(item, desc) {
  if (desc.table == 'units') {
    ...
    const manifest = dm.getFromCache({method:'getmeta', type:'manifest', id: listItem.plugin}).data;
    const ext = manifest && manifest.extrapattern;
...
function formTitle(listname, item) {
  ...
  switch (listname) {
    ...
    case 'dbagentList':
      emo = item.id == appconfig.get('project_dbname') ? emObj[1] + ' ' : '';
      return emo + item.title;
    default:
