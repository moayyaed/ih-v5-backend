  ...
  switch (table) {
    ...
    case 'snippet':
      ...
      case 'apihandler':
        ...
        doc.name += ' (copy)';
        // Копировать файл скрипта
        await fileutil.copyFileP(appconfig.getApihandlerFilename(oldId), appconfig.getApihandlerFilename(doc._id));
        return doc;
...
async function saveHandler(nodeid, value) {
  ...
  if (nodeid.startsWith('api')) {
    const filename = appconfig.getApihandlerFilename(nodeid);
...
async function saveScript(nodeid, value) {
  ...
  if (nodeid.startsWith('snippet')) {
    ...
    // await fileutil.writeFileP(filename, value);
      ...
      table = 'apihandler';
      filename = appconfig.getApihandlerFilename(nodeid);
      event = 'saved:apihandler';
  } else {
    table = 'scene';
