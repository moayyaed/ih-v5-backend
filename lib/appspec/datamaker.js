...
async function createOneRecord({ table, filter, item, parentid, body }, dm) {
  ...
  switch (table) {
    ...
    case 'pages': 
      ...
      return { ...doc, ...item, props: getDocLangs()};
...
function getDocLangs() {
  return {ru:{pagename:'Новая страница'}, en:{pagename:'New page'}};
}
...
async function beforeUpdate(table, doc, dm) {
  ...
  switch (table) {
    case 'device':
      ...
      } else if (doc.$set && doc.$set.mob && doc.mob != doc.$set.mob) {
        // Нужно создать запись в таблице mobiledevice
        await mobiledeviceutil.upsertMobileDeviceDoc(doc._id, dm);
      }
      break;
...
async function findOrAddDoc({ collection, id }, dm) {
  console.log('FIND OR ADD ' + collection + ' id=' + id);
  ...
  const spec = ['devicedb', 'glcurrent', 'infogroupmembers', 'mobiletypes', 'mobiledevices'];
  if (spec.includes(collection)) {
    switch (collection) {
      ...
      case 'mobiletypes':
        ...
        console.log('AFTER INSERT ' + collection + ' newdoc =' + util.inspect(newdoc));
        return dm.dbstore.findOne(collection, { _id: id });
...
async function saveDevicesTreeTable(recToWrite, nodeid, holder) {
  ...
  const mobiledeviceToUpsertDocs = [];
  const dids = Object.keys(recToWrite);
  try {
    for (const did of dids) {
      if (recToWrite[did].mob != undefined) {
        if (recToWrite[did].mob) {
          const newDoc = await mobiledeviceutil.tryAddMobileDeviceDoc(did, holder.dm);
          if (newDoc) mobiledeviceToUpsertDocs.push(newDoc);
        }
  ...
  try {
    if (mobiledeviceToUpsertDocs.length) {
      await holder.dm.upsertDocs('mobiledevice', mobiledeviceToUpsertDocs);
    }
    ...
    console.log('ERROR: saveDevicesTreeTable mobiledeviceToUpsertDocs=' + util.inspect(mobiledeviceToUpsertDocs) + ' ' + e.message);
    throw e;
  }
}
...
async function saveExField({ item, field, nodeid, value }, dm) {
  ...
  switch (item.type) {
    ...
    case 'script':
      ...
      } else  if (field == 'docpage') {
        retObj = await saveDocPage(nodeid, value, dm);
      } else {
...
async function saveDocPage(nodeid, value, dm) {
  ...
  console.log('getDocPageFilename ='+filename)
  await fileutil.writeFileP(filename, value);
}
...
async function getDocPage(id, lang) {
  const filename = appconfig.getDocPageFilename(id, lang);
  if (fs.existsSync(filename))  return fileutil.readFileP(filename);
