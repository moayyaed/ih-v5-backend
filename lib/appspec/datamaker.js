...
async function beforeUpdate(table, doc, dm) {
  ...
  switch (table) {
    case 'device':
      ...
      if (doc.$set && doc.$set.type && doc.type != doc.$set.type) {
      ...
      } else if (doc.$set && doc.$set.mob != undefined && doc.mob != doc.$set.mob) {
        ...
        dm.datagetter.rebuidDeviceMobileLists(dm);
      }
      break;
  ...
  switch (item.type) {
    ...
    case 'script':
      if (field == 'handler') {
        ...
        retObj = await documentation.saveDocPage(nodeid, value, dm);
      } else {
