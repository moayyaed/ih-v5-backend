...
async function createDevhardDoc(doc, item, body) {
  ...
  if (item.folder) {
    ...
    if (item.navnodeid) {
      // TODO Формируется из дерева каналов - генерировать уникальный ID канала
      const rule = await dm.getManifestItem(doc.unit, 'ruleID');
      // doc.chan = 'NewChannel';
      doc.chan = await getNewChan(item.navnodeid, rule);
    } else if (item.chan) {
      // Проверить на уникальность, если не уникальный - подобрать уникальный id
      doc.chan = item.chan;
    } else if (body && body.method == 'update') {
      // / Проверить на уникальность, если не уникальный - подобрать уникальный id
      doc.chan = item.chan || 'NewChannel';
    } else {
      // Если при добавлении формы - просто NewChannel, т к при сохранении ничего не присылается, если не было редактирования
      doc.chan = 'NewChannel';
    }
  }
  return doc;
...
async function getNewChan(unit, rule) {
  if (!rule || !rule.len) rule = { pref: '', len: 3 };
