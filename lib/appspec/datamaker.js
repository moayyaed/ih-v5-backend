...
// При редактировании таблицы - менять другие таблицы (smartbutton для chanlink)
// Возвращает
async function processTableSpecField(nodeid, rowid, field, fieldItem) {
  ...
  if (field == 'chanlink') {
    ...
    if (_id) {
      ...
      doc = await dm.dbstore.findOne('devhard', { _id });
      if (!doc) throw {message:'Not found channel record: id ='+_id};
      doc.$set = { did, prop };
    } else if (fieldItem.value && hut.isObjIdle(fieldItem.value)) {
     // Отвязка  - приходит {value:{}, title:"", path:""}
     // Нужно найти запись с каналом для этого устройства и сбросить его
     // Считаем, что привязка одна
     doc = await this.dm.dbstore.findOne('devhard', { did, prop });
     doc.$set = { did: '', prop: '' };
    }
    if (doc && doc.$set) {
      await dm.updateDocs('devhard', [doc]);
    }
  }
}
