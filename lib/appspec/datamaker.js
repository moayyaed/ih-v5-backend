  ...
  switch (table) {
    ...
    case 'snippet':
      ...
      case 'restapihandler':
        ...
        doc.name += ' (copy)';
        // Копировать файл скрипта
        await fileutil.copyFileP(appconfig.getRestapihandlerFilename(oldId), appconfig.getRestapihandlerFilename(doc._id));
        return doc;
...
async function customValidate({ prop, doc, _id, table }) {
  ...
  switch (table) {
    ...
    case 'scene':
    case 'multiscene':
      res = await sceneutils.customValidate({ prop, doc, _id, table });
      return res;
...
async function saveScript(nodeid, value) {
  ...
  if (nodeid.startsWith('snippet')) {
    ...
    table = (nodeid.startsWith('ms')) ? 'multiscene' :  'scene';
