...
async function saveScript(nodeid, value) {
  ...
  const filename = nodeid.startsWith('snippet')
    ? appconfig.getSnippetFilename(nodeid)
    : appconfig.getScriptFilename(nodeid);

  hut.unrequire(filename);
  await fileutil.writeFileP(filename, value);
  dm.emit('saved:script', nodeid);
}
