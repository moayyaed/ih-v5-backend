...
async function createDevhardDoc(doc, item, body) {
  console.log('START createDevhardDoc item='+util.inspect(item));
  if (item.navnodeid) {
...
async function getDefaultChannelForPlugin(unit) {
  console.log('START getDefaultChannelForPlugin unit='+unit);
  const manifest = await dm.getCachedData({ method:'getmeta', type: 'manifest', id: unit });
  console.log('RET '+util.inspect(manifest.data.chdefault));
