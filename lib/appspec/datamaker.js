...
const integrationutil = require('./integrationutil');
...
async function createOneRecord({ table, filter, item, parentid, body }, dm) {
  ...
  switch (table) {
    ...
    case 'infoaddr_common':
      ...
      console.log('infoaddr_common '+util.inspect({ ...item, userId: parentid }))
        return { ...item, userId: parentid };
  ...
  if (spec.includes(collection)) {
    switch (collection) {
      ...
      case 'integrations':
        ...
        newdoc = await integrationutil.createIntegrationDoc(nodeid, navnodeid);
        // console.log('insert integrations newdoc='+util.inspect(newdoc))
        await dm.dbstore.insert(collection, newdoc);
        return dm.dbstore.findOne(collection, { _id: id });
