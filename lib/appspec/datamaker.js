...
async function cloneOneFromTree(node, table, target, body, seqObj) {
  const tdesc = descriptor.getDescItem('table', table);
  const doc = await dm.dbstore.findOne(tdesc.collection, { _id: node.id });
  if (doc) await cloneDoc(table, doc, target, body);
  return doc;
}

async function cloneDoc(table, doc, target, body, seqObj) {
  const oldId = doc._id;
  ...
  switch (table) {
    case 'devhard': // При копировании канала привязку к устройству не копировать!!
      return devhardutil.cloneDevhardDoc(doc, body);
...
async function customValidate({ prop, doc, _id, table }) {
  ...
  switch (table) {
    ...
    case 'devicecommonTable':
      res = await deviceutil.customValidate({ prop, doc, _id });
      return res;

    case 'scene':
      res = await sceneutils.customValidate({ prop, doc, _id });
      return res;
...
async function saveHandler(nodeid, value) {
  ...
  if (nodeid.startsWith('api') || nodeid.startsWith('restapi')) {
    const filename = appconfig.getRestapihandlerFilename(nodeid);
  ...
  if (prop != 'MAIN') {
    ...
    if (fuse < 2)
      throw { message: 'Для редактирования на вкладке "Свойства" переключите тип обработчика на "Пользовательский".' };
  }
