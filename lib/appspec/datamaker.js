...
async function createOneRecord({ table, filter, item, parentid, body }, dm) {
  ...
  switch (table) {
    ...
    case 'pages':
      ...
      return { ...doc, ...item, props: getDocLangs('NewPage') };

    case 'pagegroup':
      doc.name = appconfig.getMessage('NewFolder');
      return { ...doc, ...item, props: getDocLangs('NewFolder') };
    default:
      return doc;
  }
}

function getDocLangs(name) {
...
async function copypasteDoc({ table, doc, target, seqObj }, dm) {
  ...
  switch (table) {
    ...
    case 'pages':
        ...
        await domaindata.copyPage(oldId, doc);
        doc.name += ' (copy)';
        return doc;
...
async function saveDevicesTreeTable(recToWrite, nodeid, holder) {
  ...
  try {
    ...
    console.log(
      'ERROR: saveDevicesTreeTable mobiledeviceToUpsertDocs=' + util.inspect(mobiledeviceToUpsertDocs) + ' ' + e.message
    );
    throw e;
  }
}
