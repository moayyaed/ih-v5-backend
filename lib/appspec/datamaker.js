...
async function saveHandler(nodeid, value, dm) {
  ...
  if (nodeid.indexOf('.') > 0) {
    ...
    return typestore.updateHandler(typeId, prop);
  }
...
async function processTableSpecField({nodeid, rowid, field, fieldItem}, dm) {
  if (field == 'devtrig') {
    return fieldItem && fieldItem.result && fieldItem.result.did && fieldItem.result.prop ? fieldItem.result.did+'.'+fieldItem.result.prop : '';
  }
