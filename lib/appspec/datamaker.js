...
async function createOneRecord(table, filter, item, parentid, body) {
  ...
  switch (table) {
    ...
    case 'devhard':
      ...
      if (item.navnodeid){ // Формируется из дерева каналов
        doc.unit = item.navnodeid;
      } else { 
        // в item пришли данные - канал создается из формы вместе с привязкой { did: 'd0518', prop: 'value' }
        if (item.did) doc.did = item.did;
        if (item.prop) doc.prop = item.prop;
        if (item.unit) {
          doc.unit = item.unit;
        } else {
          ...
          if (!unit) {
            throw  {err: 'SOFTERR', message: 'datamaker: Failed to define unit from id!'};
          }
          doc.unit = unit;
        }
      ...
      if (item.folder) {
        ...
        // Default для канала взять от плагина
