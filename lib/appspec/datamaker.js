...
async function createOneRecord({ table, filter, item, parentid, body }, dm) {
  ...
  async function addInfogroupmember() {
    if (!item || !item.userId || typeof item.userId == 'object') throw { message: 'Выберите пользователя!' };

    const rr = { _id, groupId: parentid, userId: item.userId };
...
async function saveTypepropalert(recToWrite, nodeid, holder) {
  ...
  function makeAlertSetObj(mainprop, data) {
    let setObj;
    // mainprop = 'state', data = { B_0:{ level:1 },B_1:{msg:'!!'}}
    // нужно вернуть "alerts.state.B_0.level":1
