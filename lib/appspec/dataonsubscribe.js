...
async function formDeviceLogOnSub(event, key, logobj, holder) {
  ...
  switch (event) {
    ...
    case 'dialog':
      uppobj = await projectdata.getCachedUpProjectObj({ id: event, nodeid: key });
      console.log('formDeviceLogOnSub key='+key+' uppobj='+util.inspect(uppobj))
      return formChangedDeviceLog(logobj, uppobj.data);
    default:
  }

}

function formChangedDeviceLog(logobj, uppobj) {
  
  if (!uppobj || !uppobj.devicelog || !logobj) return;
  const did = logobj.did;
  // if (uppobj.devicelog[did]) return {devicelog:{[uppobj.devicelog[did]]:logobj.data}};

  if (uppobj.devicelog) {
    const id = Object.keys(uppobj.devicelog)[0];
    return {[uppobj.devicelog[id]]:logobj.data};
  }
  ...
  */
  ...
  if (uppobj.charts) {
    ...
    changed.forEach(chItem => {
      ...
      if (uppobj.charts[did_prop] == did_prop) { // график с одним значением - подписка на did_prop
          if (!chartdata) chartdata = {};
