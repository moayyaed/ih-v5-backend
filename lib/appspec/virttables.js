...
async function sysdevice(data, table, nodeid) {
  ...
  const arr = await dm.dbstore.get('units', {}, { order: 'order' });
  return arr
...
async function sysdevicegroup(data, table, nodeid) {
  ...
  res.forEach(item => {
    if (item.folder && item.parent == 0) item.name = 'Системные индикаторы';
  });
  return res;
}
...
async function dblastrecs(data, table, nodeid, pitem, holder) {
  ...
  // Считать данные по устройству
  const dn = holder.devSet[nodeid].dn;
  if (!dn) return [];

  const ts = Date.now();
  const from = hut.dateToISOString(new Date(ts - 180 * 1000));  // за последние 180 сек
  const to = hut.dateToISOString(new Date(ts));
  const strArr = await dbconnector.read({ from, to, select: dn });
  if (!strArr) return [];

  // const strArr = await dbconnector.read({ from, to, select: 'value' });

  // Данные пришли в виде массива строк ["AI002,value,1604589971493,0""AI002,value,1604589971493,0", ..]
  const arr = [];
