...
async function dblastrecs(data, table, nodeid, pitem, holder) {
  ...
  // Считать данные по устройству, которые записаны в БД за последний час по всем свойствам
  ...
  // НЕТ просто запросить dn_prop =  dn
  ...
  const arr = fromDb.map((item, idx) =>
    ...
    ({ id: idx, dn: item.dn, prop: item.prop, val: item.val, realtime_ts: item.time })
  );
  /*
