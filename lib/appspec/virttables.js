...
async function devicevis(dataFromTable, table, nodeid, pitem, holder) {
  ...
  if (!contArr || !contArr.length) return [];

  const result = [];

  for (const item in contArr) {
    const id = item.id;
    const uppobj = await dm.getCachedData(
      { type: 'uppobj', id: 'container', nodeid: id, method: 'get' }, getUpPobj);
    if (uppobj && uppobj[did]) {
      result.push({ id, container_id: id, content_str: JSON.stringify(uppobj[did]) });
    }
  }
  return result;
}
