...
async function dblastrecs(data, table, nodeid, pitem, holder) {
  if (!nodeid || !holder.devSet[nodeid]) return [];
  // Считать данные по устройству за последний час

  const dn = holder.devSet[nodeid].dn;
  if (!dn) return [];

  const ts = Date.now();
  const start = hut.dateToISOString(new Date(ts - 3600 * 1000)); // за последний час = 3600 сек
  const end = hut.dateToISOString(new Date(ts));
  const dn_prop = dn + '.value'; // Собрать свойства, которые пишутся в базу сейчас?

  const fromDb = await dbconnector.read({ start, end, dn_prop, target: 'trend' });
  if (!fromDb) return [];

  // Данные пришли в виде массива [[ts, val]] - переделать!!
  const arr = fromDb.map((item, idx) => {
    const [xts, val] = item;
    return { id: idx, dn, prop: 'value', val, realtime_ts: hut.getDateTimeFor(new Date(xts), 'dtms') };
  });
  /*
  for (let i = 0; i < fromDb.length; i++) {
    const lineArr = strArr[i].split(',');

    if (lineArr && lineArr.length > 2) {
      const realtime_ts = lineArr[2] > 0 ? hut.getDateTimeFor(new Date(Number(lineArr[2])), 'dtms') : '';
      arr.push({ id: i + 1, dn: lineArr[0], prop: lineArr[1], realtime_ts, val: lineArr[3] });
    }
  }
  */
  return arr;
...
async function dbmetrics(data, table, nodeid, pitem, holder) {
  ...
  docs.forEach(doc => {
    if (doc.did && doc.prop && doc.dbmet > 0) {
      i++;
      // arr.push({ id: i, did: datagetter.getDeviceLinkObj(doc.did), prop: doc.prop, dbmet: doc.dbmet });
      arr.push({
        id: i,
