...
async function typeprops(dataFromTable, table, nodeid, pitem, holder) {
  ...
  if (datautil.isLink(nodeid)) {
    ...
    let propItem;
    if (prop) {
      propItem = prop == 'MAIN' ? getMainHandlerItem() : arr.find(item => item.id == prop);
      if (propItem) {
  ...
  function getWhenStr(op, prop) {
    switch (op) {
      ...
      default:
        return 'При поступлении данных для приема и обработки значения "' + prop + '"';
    }
  }

  function getMainHandlerItem() {
    const main_when = {};
    Object.keys(rec).forEach(attr => {
      if (attr.startsWith('main_when')) main_when[attr] = rec[attr];
    });
    return { id: 'MAIN', fuse: 2, ...main_when };
  }
}
