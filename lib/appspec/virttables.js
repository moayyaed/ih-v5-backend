...
async function dblastrecs(data, table, nodeid, pitem) {
  // Считать данные за последние 60 сек
  const ts = Date.now();
  const from = hut.dateToISOString(new Date(ts - 60 * 1000));
