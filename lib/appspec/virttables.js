...
async function dbagentstate(data, table, nodeid, pitem, holder) {
  ...
  const res = { _id: item.id, name: item.dbname, status: '-', uptime: '-' };
  ...
  res.state = liststore.getTitleFromList('unitStateList', item.state || 0);
  res.error = item.error;
  return [res];
  // return tabledata.formRows(table, [res], pitem.columns);
}
