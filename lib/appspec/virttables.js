...
async function globalstree(data, table, nodeid, pitem, holder) {
  ...
  const arr = data[table].map(item =>{
    ...
    const value = dn && holder.globals[dn] != undefined ? holder.globals[dn] : 999;
...
async function devicelog(data, table, nodeid, pitem) {
  ...
  arr.forEach(item => {
    item.did_prop = {
      title: datagetter.getDeviceTitle(item.did) + (item.prop ? ' ▪︎ ' + item.prop : ''),
      path: isSysDevice(item.did)
        ? 'dev/sysdevices/sysdeviceview/' + item.did + '/tabSysDeviceCommon'
        : 'dev/devices/deviceview/' + item.did + '/tabDeviceCommon'
    };
