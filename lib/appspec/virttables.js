...
async function typeprops(dataFromTable, table, nodeid, pitem, holder) {
  ...
  if (datautil.isLink(nodeid)) {
    ...
    let propItem;
    if (prop) {
      propItem = prop.startsWith('_On') ? { id: prop, fuse: 2 } : arr.find(item => item.id == prop);
      if (propItem) {
  ...
  function getWhenStr(op, prop) {
    if (prop.startsWith('_On')) {
      if (prop == '_OnChange')
        return 'При изменении ' + (rec.par_OnChange ? 'свойства ' + rec.par_OnChange : ' любого свойства устройства');
      if (prop == '_OnInterval')
...
async function devicecommon(dataFromTable, table, nodeid, pitem, holder) {
  ...
  // Данные каналов теперь есть в устройстве!
  ...
  typeItem.proparr.forEach(propItem => {
    ...
    /*
    // Параметры обработчика редактируемые на уровне устройства - пока скрываю
    item.fn_opt_str = dobj.getFn_opt_str(prop);
    */
  ...
  if (extProps) {
    Object.keys(extProps).forEach(prop => {
      if (!typeItem.props[prop]) {
        ...
        const item = getItem(prop, title, 'Scene Parameter Number', {
  ...
  function getType_fuse(prop) {
    return {
      title: liststore.getTitleFromList('fuseList', typeItem.props[prop].fuse),
      path: datagetter.getTypeHandler(dobj.type, prop)
    };
  }
...
async function devicescene(dataFromTable, table, nodeid, pitem, holder) {
  ...
  const dobj = holder.devSet[nodeid];
  const dn = dobj.dn;
  // devsceneSet ={main:this.dnMainSet, child:this.dnChildSet};

  const result = [];
  if (holder.devsceneSet.main[dn]) form(holder.devsceneSet.main[dn], 0);
  if (holder.devsceneSet.child[dn]) form(holder.devsceneSet.child[dn], 1);

  return result;

  function form(devsceneSet, child) {
    devsceneSet.forEach(scene => {
      const sceneListItem = liststore.getItemFromList('sceneList', scene);
      const scenePath = datagetter.getScenescriptPath(scene);
      result.push({ id: scene, scene_id: scene, child, scene_name: { title: sceneListItem.name, path: scenePath } });
    });
  }
}
...
function layoutNameAsLink(id, item) {
  ...
  const title = `${appconfig.getMessage('Layou')}: ${item.title || id}`;
  return { title, path };
}
...
function addHardProps(dobj, prop, item) {
  ...
  if (chanObj) {
    item.chanlink = datagetter.getChannelLinkObj(chanObj._id, chanObj.unit, chanObj.chan);
  }
}
...
async function scenestree(data, table, nodeid, pitem, holder) {
  ...
  data[table].forEach(rec => {
    const item = holder.worksceneSet[rec.id];
    if (item.multi) {
      arr.push({ _id: item.id, name: item.id, state: 'Multi', uptime: '', error: item.error });
      // И экземпляры
    } else {
...
async function restapihandlerstree(data, table, nodeid, pitem) {
  const arr = data[table].map(item => Object.assign({ _id: item.id, name: item.title }, item));
  return tabledata.formRows(table, arr, pitem.columns);
}
...
async function unitchannels(dataFromTable, table, nodeid, pitem) {
  const arr = await dm.dbstore.get('devhard', { unit: nodeid }, { order: 'chan' });
...
async function pluginlog(data, table, nodeid, pitem) {
  ...
  // const arr = await dm.logstore.get('pluginlog', { unit: { $in: unitArr } });
...
async function devicelog(data, table, nodeid, pitem, holder) {
  ...
  let arr = [];
  if (didArr.length > 1 || datagetter.isGlobal(didArr[0])) {
    arr = await dm.dbstore.get('devicelog', { did: { $in: didArr } }, { order: 'ts,_id' });
  } else {
    const dobj = holder.devSet[didArr[0]];
    if (dobj) arr = dobj.innerLog;
    // if (dobj) arr = dobj.getInnerLog();
  }
...
async function targetFrame(data, table, nodeid, pitem, holder) {
  return [{ target_frame: { id: '-', title: '-' }, target_container_id: { id: '-', title: '-' } }];
}
...
async function plugininfo(data, table, nodeid, pitem, holder) {
  ...
  const res = await dm.getPluginInfo(plugin);
  return res ? [res] : [];
}
...
async function dbagentinfo(data, table, nodeid, pitem, holder) {
  ...
  if (!nodeid) return [];
  const res = await dm.getDbagentInfo(nodeid);
  return res ? [res] : [];
}
...
async function projectdeps(data, table, nodeid, pitem, holder) {
  ...
  if (!rec || !rec.deps) return [];

  return projectutil.getNpmList(nodeid);
}
...
async function customdata(data, table, nodeid, pitem, holder) {
  // nodeid - id в customtable => tablename - в папке custom
  // Если таблицы нет - создать??
  const doc = await holder.dm.findOne('customtable', { _id: nodeid });

  console.log('customdata nodeid='+nodeid+' doc='+util.inspect(doc));
  if (!doc) throw { message: 'Not found doc with _id' + nodeid + ' in customtable' };


  // const proparr = doc.props ? tabledata.transfromGenfieldObjToArray(doc.props) : [];
  const proparr = doc.props ? Object.keys(doc.props) : [];
  const tablename = doc.tablename;
  console.log('customdata tablename='+tablename+' proparr='+util.inspect(proparr));
  if (!tablename) return[]; // Здесь возможно нужен скрипт

  const arr = [];
  holder.dm.createCustomTable(tablename);  // Если есть - ничего не будет
  const customDocs = await holder.dm.get(tablename);

  customDocs.forEach(row => {
    const line = {_id:row._id};
...
const xx = {"_id":"cttab004","parent":"customtablegroup","order":1500,"name":"Пользователи","props":{"LCXN_Y3vL":{"name":"_id","title":"ID","type":"text","width":"100"},"H-mx7Nb2F":{"name":"wpid","title":"WP id","type":"text","width":"100"},"6gGWU7T_g":{"name":"activate","title":"Activate","type":"cb","width":"64"}},"tablename":"ih_users"}
