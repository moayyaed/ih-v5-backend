...
async function devicescene(dataFromTable, table, nodeid, pitem, holder) {
  // if (!holder.devSet[nodeid]) return [];
  if (!holder.devSet[nodeid]) throw {message:'Not found '+nodeid+' in devSet!'}
  if (!holder.devsceneSet[nodeid]) {
    const typex = typeof holder.devsceneSet;
    const size =  (typex == 'object') ? Object.keys(holder.devsceneSet).length : '-';
    throw {message:'Not found '+nodeid+' in devsceneSet! type: '+typex +' size='+size}
  }
