...
async function multiscenestree(data, table, nodeid, pitem) {
...
async function projectdeps(data, table, nodeid, pitem, holder) {
  ...
  const folder = appconfig.getTheProjectPath(nodeid);
  return projectutil.getNpmList(folder);
}
...
module.exports = {
  ...
  multiscenesTreeTable: multiscenestree,
