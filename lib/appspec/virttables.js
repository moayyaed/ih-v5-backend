...
async function sysdevice(data, table, nodeid) {
  ...
  if (nodeid) {
    ...
    return datagetter.getUnitIndicatorObj(_id) || '';
  }
  ...
  return arr
    ...
    .map(item => {
      const res = datagetter.getUnitIndicatorObj(item._id);
      res.dn = res._id;
...
async function devicelog(data, table, nodeid, pitem) {
  ...
  arr.forEach(item => {
    item.did_prop = {
      ...
      path: 'dev/devices/deviceview/' + item.did + '/tabDeviceCommon'
    };
