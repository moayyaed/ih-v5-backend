...
async function getImagegrid({ nodeid }, dm, holder) {
  const docs = await dm.dbstore.get('images', { parent: nodeid });
  const data = docs && Array.isArray(docs) ? docs.filter(item => !item.folder).map(item => item._id) : [];
  return { data };
...
async function getElementlink({ nodeid, root, anchor }, dm, holder) {
  ...
  if (nodeid.startsWith('gl')) {
    ...
    properties = dobj
      .getPropsForVislink()
      .map(prop => getItem(did, prop, datagetter.getDeviceDn(did) + '.' + prop));
