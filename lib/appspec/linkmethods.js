...
async function get(query, dm, holder) {
  ...
  switch (query.id) {
    case 'devicelink':
      ...
      if (query.dialogid == 'devcmd') return getCmdlink(query, dm, holder);
  ...
  if (templateObj && templateObj.state && typeof templateObj.state == 'object') {
    Object.keys(templateObj.state).forEach(stateId => {
      if (stateId.startsWith('state') && templateObj.state[stateId].title) {
        ...
        const result = { prop, title, value: { did:nodeid, prop, template:true } };
        properties.push({ did:nodeid, prop, template:true, title: 'template.' + prop, link: '', enable: true, result });
      }
    });
  }
  return { data: { properties } };
