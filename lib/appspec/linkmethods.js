...
async function getDevicelink({ nodeid, anchor }, dm, holder) {
 ...
 if (nodeid.startsWith('gl')) { // Глобальные переменные
  ...
  properties = anchor ? await forHardlink() : dobj.getPropsForVislink().map(prop => getItem(prop));
 }
