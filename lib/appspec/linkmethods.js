...
async function get(query, dm, holder) {
  ...
  switch (query.id) {
    case 'devicelink':
      ...
      if (query.dialogid == 'devcmdAndAny') return query.root == 'anydevcmd' ? getAllCmd() : getCmdlink(query, dm, holder);
      throw { error: 'SOFTERR', message: 'Unexpected dialogid: ' + query.dialogid };

    case 'elementlink':
      return query.root == 'anydev' ? getAllProps() : getElementlink(query, dm, holder);
    ...
    case 'imagegrid':
      return getImagegrid(query, dm, holder);
...
async function getImagegrid({ nodeid }, dm, holder) {
  const data = await dm.dbstore.get('images', { parent: nodeid });
  return { data: data && Array.isArray(data) ? data.map(item => item._id) : [] };
}

async function getAllProps(dm, holder) {
  return {
    data: {
      properties: [
        {
          ...
          enable: true,
          result: { did: '__device', prop: 'state', title: 'Свойство state', value: { did: '__device', prop: 'state' } }
        }
      ]
    }
  };
      ...
      properties: [
        {
          ...
          title: 'Команда on',
          enable: true,
          result: { did: '__device', prop: 'on', title: 'Команда on', value: { did: '__device', prop: 'on' } }
        },
        {
          prop: 'off',
          title: 'Команда off',
          enable: true,
          result: { did: '__device', prop: 'off', title: 'Команда off', value: { did: '__device', prop: 'off' } }
        },
        {
          prop: 'toggle',
          title: 'Команда toggle',
          enable: true,
          result: {
            did: '__device',
            prop: 'toggle',
            title: 'Команда toggle',
            value: { did: '__device', prop: 'toggle' }
          }
        },
        {
          prop: 'up',
          title: 'Команда up',
          enable: true,
          result: { did: '__device', prop: 'up', title: 'Команда up', value: { did: '__device', prop: 'up' } }
        },
        {
          prop: 'down',
          title: 'Команда down',
          enable: true,
          result: { did: '__device', prop: 'down', title: 'Команда down', value: { did: '__device', prop: 'down' } }
        }
      ]
    }
  };
...
async function getElementlink({ nodeid, root, anchor }, dm, holder) {
  ...
  if (nodeid.startsWith('gl')) {
    ...
    const prop = item.dn;
    properties = [getItem(did, prop, 'globals.' + prop)];
    ...
    const prop = item.dn;
    properties = [getItem(did, prop, 'locals.' + prop)];
...
function getItem(did, prop, title, anchor) {
  const result = { did, prop, title, value: { did, prop } };
