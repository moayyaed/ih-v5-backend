...
async function getSmartbuttonForDevicelink(dataItem) {
  ...
  return {
    ...
    path:datagetter.getDeviceLinkObj(did)
  };
}
...
async function getSmartbuttonForChannellink(dataItem, preNodeid, rowid) {
  ...
  if (!preNodeid || !datautil.isLink(preNodeid)) throw {err:"SOFTERR", message:'Expected nodeid as link: '+preNodeid};
  ...
  if (filter) {
    ...
    if (hrec) {
      ...
      path = datagetter.getChannelLinkObj(hrec._id, hrec.unit, hrec.chan);
