 ...
 * @param {*} nodeid 
 */
async function get(dialog, dataItem, nodeid) {
  switch (dialog) {
    case 'devicelink':
...
// preNodeid='d0123.setpoint'
async function getSmartbuttonForChannellink(dataItem, preNodeid) { 

  // Найти привязку к свойству. Если нет - будет пустой элемент??
  if (!preNodeid || !datautil.isLink(preNodeid)) return "";

  const [did, prop] = preNodeid.split('.');
  const hrec = await dm.dbstore.findOne('devhard', { did, prop });

  if (hrec) {
    const title = hrec.unit+'.'+hrec.chan;
    return {
      title,
