...
class Devicemate {
  ...
  async start() {
    ...
    this.dm.on('inserted:type', docs => {
      docs.forEach(doc => this.engine.addType(doc));
      this.engine.formTypeHandlerSets(docs); // ??
    });

    this.dm.on('removed:type', docs => {
      docs.forEach(doc => this.engine.removeType(doc));
      this.engine.formTypeHandlerSets(docs);
   });

    this.dm.on('updated:type', docs => {
      docs.forEach(doc => this.onUpdateTypeDoc(doc)); // TODO - нужно перенести из typestore?
      const arr = [];
      docs.forEach(doc => {
      ...
      this.engine.formTypeHandlerSets(arr);
   });
