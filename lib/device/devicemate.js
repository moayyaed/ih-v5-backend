...
class Devicemate {
  ...
  async start() {
    ...
    this.engine.holder.on('create:unitIndicator', unitId => {
      ...
      if (!this.engine.hasDevice(id)) this.addDevice(deviceutil.createUnitIndicatorDoc(unitId));
    });

    // Удалить системный индикатор
    this.engine.holder.on('remove:unitIndicator', unitId => {
      const id = deviceutil.getUnitIndicatorId(unitId);
      if (this.engine.hasDevice(id)) this.removeDevice({_id:id});
    });
  ...
  async formSysdeviceArray() {
    ...
    const dbSysObj = deviceutil.createUnitIndicatorDoc('DBHIST');
    res.push(dbSysObj);
