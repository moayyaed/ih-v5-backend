  ...
  async start() {
    ...
    this.dm.on('changed:typeprops', async (type, setUnsetProps, updateProps) => {
      ...
      if (setUnsetProps) {
        ...
        console.log('addedProps=' + util.inspect(addedProps));
        console.log('deletedProps=' + util.inspect(deletedProps));
        ...
        if (updatedDocs && updatedDocs.length) {
          const deletedPropLinks = [];
          for (const devDoc of updatedDocs) {
            const deletedLinks = await this.updateLinksForProps(devDoc, deletedProps, renamedProps);
            deletedPropLinks.push(...deletedLinks);
            // Добавить/удалить свойства в devSet
            this.engine.changeDeviceProps(devDoc._id, addedProps, deletedProps);
          }

          // if (deletedPropLinks.length) this.engine.deletedLinks(deletedPropLinks);
          // Это только для devSet, для каналов срабатывает pluginmate.onChannelsChange
          // Добавить/удалить свойства в devSet
        }
      }
