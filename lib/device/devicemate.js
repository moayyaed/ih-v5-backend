...
class Devicemate {
    ...
    this.dm.on('updated:device', async docs => {
      ...
      for (const doc of docs) {
        if (doc.$set) {
          ...
          if (doc.$set.type) {
            // старый и новый тип - сравнить свойства
            const newTypeProps = this.engine.typestore.getPropNameArray(doc.$set.type);
            const oldTypeProps = this.engine.typestore.getPropNameArray(doc.type);
          ...
          this.engine.changeDeviceFlatfields(doc._id, doc.$set);

          // Если изменены табличные свойства - min, max, dig, onread - заменить в _aux
          const auxArr = [];
          ...
          if (auxArr.length) this.engine.changeDeviceAux(auxArr);
  ...
  // старый тип и новый тип - сравнить свойства, удалить привязки для несущ свойств
  getMissingProps(oldtype, newtype) {
    const missingProps = [];
    const oldprops = this.engine.typestore.getPropNameArray(oldtype);
    const newprops = this.engine.typestore.getPropNameArray(newtype);
  }
}
