...
class Devicemate {
  ...
  async formSysdeviceArray() {
    ...
    const dbSysObj = deviceutil.getUnitIndicatorObj('DBHIST', appconfig.getMessage('DBHISTIndicator'));
    res.push(dbSysObj);
    liststore.onInsertDocs('device', [dbSysObj]);
    ...
    for (const doc of unitDocs) {
      if (!doc.folder && doc.plugin && doc._id) {
        const sysObj = deviceutil.getUnitIndicatorObj(doc._id, appconfig.getMessage('PluginIndicator')+' '+doc._id);
        res.push(sysObj);
