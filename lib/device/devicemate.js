...
class Devicemate {
  ...
  async updateType(doc) {
    ...
    let alertsNeedUpdate = false;
    const propsToRemoveAlerts = [];
    ...
    if (doc.$set) {
      Object.keys(doc.$set).forEach(field => {
        if (field.startsWith('props.') && !field.endsWith('.errstr')) {
          ...
          if (field.endsWith('.ale') && !doc.$set[field]) {
            propsToRemoveAlerts.push(propname);
          }
        } else if (field.startsWith('alerts.')) {
          alertsNeedUpdate = true;
        } else {
          if (!flat) flat = {};
    ...
    if (propNeedUpdate || alertsNeedUpdate) {
      const docType = await this.dm.findRecordById('type', typeId);
      if (!docType) {
        throw { message: 'ERROR: updateType: Missing type ' + typeId };
      }

      if (propNeedUpdate) this.engine.changeTypeProps(typeId, docType.props);
      if (alertsNeedUpdate) this.engine.changeTypeAlerts(typeId, docType.alerts);
    }
    ...
    if ($set || $unset) {
      ...
      console.log('propsToRemoveAlerts ='+propsToRemoveAlerts.join(','))
      this.removeAlertsForProps(typeId, propsToRemoveAlerts);
    }
  }
