...
class Devicemate {
  ...
  async start() {
    ...
    this.dm.on('inserted:mobiledevice', async docs => {
      // Списки mobile_places, mobile_rooms, mobile_subsystems, mobile_deviceimagelist - всегда перестраиваем
      // Список mobile_devicelist - только добавляем
    });

    this.dm.on('updated:mobiledevice', async docs => {
      // Списки mobile_places, mobile_rooms, mobile_subsystems, mobile_deviceimagelist - всегда перестраиваем
      this.dm.datagetter.rebuidMobileLists(this.dm);
      // Список mobile_devicelist - только конкретное устройство заменить
      for (const doc of docs) {
      }
    });
    ...
    this.dm.on('removed:mobiledevice', async docs => {
      // Списки mobile_places, mobile_rooms, mobile_subsystems, mobile_deviceimagelist - всегда перестраиваем
      this.dm.datagetter.rebuidMobileLists(this.dm);
      // Список mobile_devicelist - только конкретное устройство удалить
      for (const doc of docs) {
      }
    });
  }
