...
class Devicemate {
  ...
  async start() {
    ...
    this.dm.on('updated:globals', docs => {
      for (const doc of docs) {
        let deleteFromLog;
        if (doc.$set && doc.save && doc.$set.save == 0) {
          // сброшен флаг сохранения в журнале - Удалить все записи по этой переменной
          deleteFromLog = 1;
        }
        const upDoc = Object.assign({}, doc, doc.$set);
        delete upDoc.$set;
        this.engine.updateGlobal(upDoc, deleteFromLog);
      }
    });
    ...
    // Изменены триггеры обработчика
    this.dm.on('updated:globalScripttriggersTable', docs => {
      for (const doc of docs) {
        // Загрузить заново из таблицы? И передать на воркер, чтобы перезагрузил триггеры и обработчик
        console.log('updated:globalScripttriggersTable doc=' + util.inspect(doc));
      }
    });
    ...
    this.dm.on('updated:type', async docs => {
      for (const doc of docs) {
        if (doc._id) await this.updateType(doc);
      }
    });

    this.dm.on('updated:typepropsTable', async docs => {
      for (const doc of docs) {
        if (doc._id) await this.updateType(doc);
      }
    });
    ...
    this.dm.on('updated:device', async docs => {
      ...
      const mobDevsToAdd = []; // устройства, для которох потенциально требуется добавление записи mobiledevice
      let rebuidDeviceMobile = 0; // Требуется перегенерация список для мобильного
      for (const doc of docs) {
        if (doc.$set) {
          ...
          //
          if (doc.$set.mob != undefined) {
            if (doc.$set.mob) mobDevsToAdd.push(doc._id);
            rebuidDeviceMobile = 1;
          }

          // Перемещение в дереве ???
          if (doc.$set.order || doc.$set.parent) {
            rebuidDeviceMobile = 1;
          }
        }
      }

      if (mobDevsToAdd.length) {
        await this.tryAddMobileDevices(mobDevsToAdd);
      }

      if (rebuidDeviceMobile) {
        // Нужно перестроить списки
        this.dm.datagetter.rebuidDeviceMobileLists(this.dm);
      }
    });
    ...
    // ---------------- mobiledevice ----------------------
    this.dm.on('inserted:mobiledevice', async docs => {
      this.dm.datagetter.rebuidDeviceMobileLists(this.dm);
    });

    this.dm.on('updated:mobiledevice', async docs => {
      this.dm.datagetter.rebuidDeviceMobileLists(this.dm);
    });

    this.dm.on('removed:mobiledevice', async docs => {
      this.dm.datagetter.rebuidDeviceMobileLists(this.dm);
    });

    // ---------------- mobiletype ----------------------
    this.dm.on('inserted:mobiletype', async docs => {
      this.dm.datagetter.rebuidDeviceMobileLists(this.dm);
    });

    this.dm.on('updated:mobiletype', async docs => {
      this.dm.datagetter.rebuidDeviceMobileLists(this.dm);
    });

    // ---------------- place ----------------------
    this.dm.on('inserted:place', async docs => {
      this.dm.datagetter.rebuidMobilePlaceList(this.dm);
    });
    this.dm.on('updated:place', async docs => {
      // Если изменилось название или подчиненность в дереве 
      // - в устройствах тоже надо поменять locationStr

      this.dm.datagetter.rebuidMobilePlaceList(this.dm);
    });

    this.dm.on('updated:list:placeList', async arr => {
      // Передать список в worker
      this.engine.sendToWorker('updated:list:placeList', arr)
    });
  }
  ...
  async updateType(doc) {
    // console.log('updateType START doc.$set=' + util.inspect(doc.$set) + ' doc.$unset=' + util.inspect(doc.$unset));
    ...
    if (doc.$set) {
      Object.keys(doc.$set).forEach(field => {
        if (field.startsWith('props.') && !field.endsWith('.errstr')) {
          // меняется атрибут свойства в табличке
          propNeedUpdate = true;
          ...
          // меняется плоское свойство типа
        } else {
          if (!flat) flat = {};
  ...
  async tryAddMobileDevices(devArrIdx) {
    for (const _id of devArrIdx) {
      try {
        const newdoc = await mobiledeviceutil.tryAddMobileDeviceDoc(_id, this.dm);
        if (newdoc) {
          // Используется запись на уровне коллекции чтобы не вызывалась функция updated:
          console.log('TryAddMobileDevice ' + _id + ': ' + util.inspect(newdoc));
          await this.dm.dbstore.update('mobiledevices', { _id }, newdoc, { multi: false, upsert: true });
          // return this.dm.upsertDocs('mobiledevice', [newdoc]);
        }
      } catch (e) {
        console.log('ERROR: tryAddMobileDevice ' + _id + ': ' + util.inspect(e));
      }
    }
  }
}
