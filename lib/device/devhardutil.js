...
async function createDevhardDoc(doc, item, body, dm) {
  ...
  async function getDefaultChannel(unit) {
    const popupItems = ['channel_folder', 'channel', 'channel_read', 'channel_write'];
...
async function addDevhardDocsWithFixChan(indocs, unit, nodeid, dm) {
  ...
  if (docs.length) {
    await dm.insertDocs('devhard', docs);
    // return { alert: 'info', message:'Добавлены каналы ', refresh: true, data:[] };

    // Вернуть добавленные узлы дерева
    /*
    return {
      data: docs.map(doc => ({
        id: doc._id,
