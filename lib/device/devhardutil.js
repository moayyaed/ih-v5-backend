...
// Канал chan д б уникален для плагина, если не исп innerId
// Если innerId=1 - проверять не надо
async function isChanNotUnique(doc, _id, dm) {
  ...
  const isInner = await isInnerId(unit, dm);
  if (isInner) return;
...
async function isInnerId(unit, dm) {
  const manifest = await dm.getManifest(unit, true);
  if (manifest && manifest.innerId) return true;
}

async function customValidate({ prop, doc, _id }, dm) {
  // console.log('customValidate ' + prop + ' doc=' + util.inspect(doc));
  if (prop == 'chan') {
...
async function addDevhardDocs(docs, unit, nodeid, dm) {
  ...
  const onlySingleChan = unit.startsWith('opc'); // TODO !!!
  if (onlySingleChan) {
    // Проверить, что канал есть и не дублируется
    // Если дублируется - исключить
    for (let i = docs.length - 1; i >= 0; i--) {
      const doc = docs[i];
      if (!doc.chan || (await isChanNotUnique({...doc, unit}, '', dm))) {
        docs.splice(i, 1);
      }
    }
  }
  ...
  function processOne(parts, titles) {
    ...
    parts.forEach((part, idx) => {
      ...
      if (chObj[chan]) {
        ...
        parent = newDoc._id;
        docs.push(newDoc);
        chObj[newDoc.chan] = newDoc;
      }
    });
  }
}

async function copyDevhardDoc(oldId, doc, dm) {
  // Если используется внутренний id, то можно ничего не менять
  const isInner = await isInnerId(doc.unit, dm);
  if (!isInner) doc.chan += '_COPY';

  // При копировании канала привязку к устройству не копировать!!
  doc.did = '';
  doc.prop = '';
  return doc;
}
