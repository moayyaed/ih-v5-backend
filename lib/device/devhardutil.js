...
async function createDevhardDoc(doc, item, body, dm) {
  ...
  const defchan = await getDefaultChannel(doc.unit, item.folder);
  ...
  async function getDefaultChannel(unit, folder) {
    const manifest = await dm.getManifest(unit, true);
    if (folder) {
      return manifest.chdefault_parent || '';
    } 
   return manifest.chdefault_child || manifest.chdefault || '';
