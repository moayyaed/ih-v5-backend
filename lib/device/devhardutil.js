...
async function channelsLinkToDevice(unit, payload, holder) {
  ...
  const linkObj = getDeviceHardLinks(did, dm); // существующие привязки
  const freePropArr = propArr.map(prop => !linkObj[prop]);
