...
async function createDevhardDoc(doc, item, body, dm) {
  ...
  async function getDefaultChannel(unit) {
    const popupItems = ['channel', 'channel_read', 'channel_write'];
    const folder = item.folder;
    const manifest = await dm.getManifest(unit, true);
    if (folder) {
      return manifest.default_folder || '';
    }
    // Могут быть каналы отдельно для чтения или записи
    let manProp = 'default_channel';
    if (item.popupid) {
      if (popupItems.includes(item.popupid)) {
        manProp = 'default_' + item.popupid;
      }
    }
