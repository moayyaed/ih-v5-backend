...
const DeviceFlatFields = ['name', 'parent', 'sys', 'tags', 'location', 'mob'];
...
function changeFlatFields(dobj, upObj) {
  ...
  DeviceFlatFields.forEach(field => {
    if (field == 'location') {
      dobj[field] = '/'+getDevobjPlacePath(dobj, 'id')+'/';
      dobj.locationStr = getDevobjPlacePath(dobj, 'title');
      res.location = dobj.location;
      res.locationStr = dobj.locationStr;
    }
    ...
    if (upObj[field] != undefined) {
      ...
      res[field] = dobj[field]
    }
  });
  return res; // Вернуть для передачи на worker
}
...
function getLogTitleAndMessage(dobj, item) {
  ...
  return { title, message, ts:item.ts };
}
...
async function typePropCustomValidate({ prop, doc, _id }, dm) {
  if (prop == 'newid') {
    if (doc[prop] == 'type' || doc[prop] == 'error') return 'reserved';
    if (DeviceFlatFields.includes(doc[prop])) return 'reserved';
    if (!hut.isIdValid(doc[prop])) return 'mustBeValidId';
  }
}

function getPlaceList() {
  return liststore.getListAsArray('placeList');
}
