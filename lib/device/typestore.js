...
//   => key=_id {item:{_id, name, tags,...}, proparr:[{prop:"value", vtype, op, min, max,...}, ]}
...
module.exports = {
  ...
  add(typeDoc) {
    ...
    const res = { item: {} };
    Object.keys(typeDoc).forEach(prop => {
      if (prop != 'props') {
        res.item[prop] = typeDoc[prop];
      } else {
        res.proparr = hut.objectToArray(typeDoc.props, 'prop');

        // Включить функции для каждого свойства
        res.proparr.forEach(p => {
          if (p.onread) {
          ...
          // p.change = new Function('prop','value', 'device', fstr);
        });
      }
    });
...
function getDefaultReadFun(vtype, op) {
  ...
  if (vtype == 'N') {
    return function(device, prop, value) {
      ...
      value =  Number(value.toFixed(device.getDig(prop)));
      return device.inRange(prop, value) ? value : {error:'Out of range!', value};
    }
  }
