...
module.exports = {
  ...
  updateProps(typeId, newprops) {
    ...
    const typeItem = typeMap.get(typeId);
    if (!typeHandler[typeId]) typeHandler[typeId] = {};
    if (!typeHandler[typeId].props) typeHandler[typeId].props = {};
    ...
    Object.keys(typeItem.props).forEach(prop => {
      ...
      if (!typeHandler[typeId].props[prop]) typeHandler[typeId].props[prop] = {};
      ...
      p.fn = '';
      if (p.fuse == 1) {
        p.fn = handlerutils.getDefaultHandlerName(typeId, prop, typeItem);
        typeHandler[typeId].props[prop].fn = p.fn ? defaulthandlers[p.fn] : '';
      } else if (p.fuse == 2) {
  ...
  getPropHandler(typeId, prop) {
    if (typeHandler[typeId] && typeHandler[typeId].props && typeHandler[typeId].props[prop]) {
      const fn = typeHandler[typeId].props[prop].fn || '';
      return fn;
    }
  },

  getPropFormatHandler(typeId, prop) {
    if (typeHandler[typeId] && typeHandler[typeId].props && typeHandler[typeId].props[prop]) {
      return typeHandler[typeId].props[prop].fn_format || '';
    }
  },
  requireHandler(typeId, inprop) {
    ...
    if (inprop.startsWith('_On')) {
      ...
      try {
        if (filename) {
          typeItem.onHandlers['fn_' + inprop] = filename;
          typeHandler[typeId]['fn_' + inprop] = require(filename);
        }
      } catch (e) {
    ...
    function requirePropFun(prop_fn, prop_err) {
      ...
      try {
        if (filename) {
          p[prop_fn] = filename;
          typeHandler[typeId].props[prop][prop_fn] = require(filename);
        ...
        p[prop_fn] = '';
        typeHandler[typeId].props[prop][prop_fn] = '';
      }
  ...
  getHandlerFun(typeId, prop) {
    if (typeHandler[typeId] && typeHandler[typeId]) {
      return typeHandler[typeId]['fn_' + prop] || '';
    }
  },
