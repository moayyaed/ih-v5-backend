...
module.exports = {
  ...
  showHandlers(typeId) {
    ...
    str += '*    Для свойств-команд on,off,toggle существует Default функция, реализующая команду через свойство "value" \n';
    ...
    Object.keys(props).forEach(prop => {
      ...
      if (props[prop].commandfun) {
        ...
        str += '/** onCommand: ' + prop + ',  module: ' + module + ' **/\n' + props[prop].commandfun.toString() + '\n\n';
      }
    });
    return str;
  },
  ...
  updateProps(typeId, newprops) {
    ...
    Object.keys(typeItem.props).forEach(prop => {
      ...
      if (!p.commandfun) p.commandfun = getDefaultCommandFun(p.vtype, p.op, prop);
