...
module.exports = {
  ...
  updateProps(typeId, newprops) {
    ...
    Object.keys(typeItem.props).forEach(prop => {
      ...
      // p.fuse: 0-default, 1-обработчик не используется (??), 2-внешний (из файла)
    
      if (!p.fuse) {
        handlerutils.useDefaultHandler(typeId, prop, typeItem);
      } else if (p.fuse == 2) {
        handlerutils.requireHandler(typeId, prop, typeItem);
      }
