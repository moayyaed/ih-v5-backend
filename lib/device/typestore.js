...
module.exports = {
  ...
  updateProps(typeId, newprops) {
    ...
    Object.keys(typeItem.props).forEach(prop => {
      ...
      const filename = p.fname ? appconfig.getHandlerFilenameIfExists(p.fname) : '';

      if (p.op == 'cmd') {
        typeItem.commands.push(prop);
        p.fn = filename ? require(filename) : typeutil.getDefaultCommandFun(prop);
      }  else {
        typeItem.proparr.push({prop, ...typeutil.getProparrAux(prop, p)});
        p.fn = filename ? require(filename) :typeutil.getDefaultFun(p.vtype, p.op);
      }

      // Подцепить функцию fn для каждого свойства 
      if (p.fname) {
        // const filename = appconfig.getHandlerFilenameIfExists(p.fname);
        if (filename) p.fn = require(filename);
      }
      if (!p.fn) {
        p.fn = typeutil.getDefaultCommandFun(p.vtype, p.op, prop);
      }
      /*
      if (p.command) {
        typeItem.commands.push(prop);
    ...
    // console.log('UpdateProps ' + util.inspect(typeItem));
  ...
  createPropsFromType(typeId) {
    ...
    if (typeMap.has(typeId)) {
      ...
      arr.forEach(propItem => {
        props[propItem.prop] = typeutil.getProparrAux(propItem.prop, propItem.prop);
      });
