...
module.exports = {
  ...
  requireHandler(typeId, prop) {
    ...
    // Для общих обработчиков err_OnSchedule, 
    if (prop.startsWith('_On')) {
      const errPropName = 'err' + prop;
      let errstr = '';
      try {
        if (filename) {
          typeItem.onHandlers['fn_' + prop] = require(filename);
        }
      } catch (e) {
        console.log('ERROR: typestore requireHandler ' + util.inspect(e));
        errstr = 'Handler ERROR: ' + hut.getShortErrStr(e);
        typeItem.onHandlers['fn_' + prop] = '';
      }
