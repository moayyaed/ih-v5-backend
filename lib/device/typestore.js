...
// {_id:тип, name, tags, props:{value:{vtype, op, min, max,...}}}  => Map (key=_id,
//    {item:{_id, name, tags,...}, proparr:[{prop:"value", min, max,...}, ],  props:{value:{vtype, op, min, max,...}}}
...
module.exports = {
  ...
  showHandlers(typeId) {
    ...
    Object.keys(props).forEach(prop => {
      if (props[prop].readfun) {
        ...
        str += '/** on Read: ' + prop + ',  module: ' + module + ' **/\n' + props[prop].readfun.toString() + '\n\n';
      }
    });
    return str;
  },
  ...
  async onUpdateTypeDoc(doc) {
    ...
    if (propNeedUpdate) {
      ...
      if (!docType) throw{err:'SOFTERR', message:'Not found type '+typeId +' in types collection after update!'}
      
      const newprops = docType[0].props;
      console.log('GET from types _id='+typeId +' newprops='+util.inspect(newprops));
    ...
    if ($set || $unset) {
      ...
      console.log('EMIT: changed:typeprops');
    }
  },
  ...
  updateProps(typeId, newprops) {
    ...
    Object.keys(typeItem.props).forEach(prop => {
      ...
      typeItem.proparr.push(getProparrAux(prop, p));
      ...
      if (!p.readfun) p.readfun = getDefaultReadFun(p.vtype, p.op);
