...
module.exports = async function(holder) {
  ...
  function restartWorker() {
    ...
    holder.deviceWorker = startWorker(getDevStructSet(), getTypeStructMap());
  ...
  function tracing({ did, hanid, state }) {
    const ts = Date.now();
    if (!holder.traceSet[hanid]) holder.traceSet[hanid] = { count: 0 };
  ...
  setInterval(() => {
    ...
    if (threadId < 0) {
      ...
      restartWorker();
      holder.deviceWorker.postMessage({ name: 'RESTART', data: { } });
      return;
    }

    if (currentScript) {
      if (Date.now() - holder.traceSet[currentScript].startTs > 1000) {
        console.log('WARN: Script ' + currentScript + 'running > 1s, TREMINATE WORKER!');
