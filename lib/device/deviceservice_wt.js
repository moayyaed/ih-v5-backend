 ...
 * 
 *  Для отслеживания изменений запускает: 
...
module.exports = async function(holder) {
  ...
  // Запуск worker-а. 
  holder.deviceWorker = startWorker(devStructSet, typeStructMap, holder.global.glByDid, holder.sceneSet);
  ...
  const scenemate = new Scenemate(engine);
  scenemate.start();
  sceneengine.start();
  ...
  function startWorker(devSet, typeMap, glSet, sceneSet) {
    const fullPath = require.resolve('../deviceworker/worker_engines');
    let w = new Worker(fullPath, { workerData: { devSet, typeMap, glSet, sceneSet } });
