...
module.exports = async function(holder) {
  ...
  holder.deviceWorker = startWorker(
    devStructSet,
    typeStructMap,
    holder.global.glByDid,
    holder.sceneSet,
    deviceutil.getPlaceList(),
    settings,
    true
  );
   ...
   *
   **/
  function startWorker(devSet, typeMap, glSet, sceneSet, placeList, settings, boot) {
    const fullPath = require.resolve('../deviceworker/worker_engines');
    let w = new Worker(fullPath, { workerData: { devSet, typeMap, glSet, sceneSet, placeList, settings, boot } });
  ...
  function getSettings() {
    // Настройки - lang, lat, lng
    const locationObject = appconfig.getLocation();
    return { ...locationObject, lang: appconfig.get('lang') };
  }
  ...
  async function loadscenes() {
    ...
    // {"_id":"scen002","name":"Свет по датчику движения","multi":1,
    //  "devs":"motion,lamp","triggers":"motion,lamp.auto","realdevs":"DD101_1,H102_1",
    //  "def":{"motion":"DD101_1","lamp":"H102_1"},"

    for (const doc of sceneDocs) {
      let calls;
      // {"_id":"call_002","sid":"scen002","id":"__FsOqiqKXS","motion":"d0051","lamp":"d0137"}
      if (doc.multi) calls = await holder.dm.dbstore.get('scenecalls', { sid: doc._id });
      sceneengine.addSceneItem(doc, calls);
    }
  }
