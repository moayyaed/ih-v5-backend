...
module.exports = async function(holder) {
  ...
  deviceDocs.forEach(item => {
    if (item.dn) {
      ...
      holder.devInnerLog[item._id] = firstLogRecords(item._id, devData[item._id]);
    } else {
  ...
  function firstLogRecords(did, dataObj) {
    const recs = [];
    if (typeof dataObj == 'object') {
      Object.keys(dataObj).forEach(prop => {
        recs.push({ did, prop, val: dataObj[prop].val, ts: dataObj[prop].ts, boot: 1 });
      });
    }
    // Сформировать пустую запись в журнале, которая указывает позицию записей после загрузки
    if (!recs.length) recs.push({ boot: 1 });
    return recs;
  }
