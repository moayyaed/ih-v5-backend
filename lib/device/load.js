...
module.exports = async function(holder) {
  ...
  async function fillGlobal() {
    ...
    const toLog = [];
    glDocs.forEach(doc => {
      const logObj = holder.global.addItem(doc._id, doc, glData[doc._id]);
   
      if (logObj) {
        // На старте присвоено значение - записать в журнал
        toLog.push({  ...logObj });
      }
    });
    if (toLog.length)  holder.logconnector.addLog('devicelog', toLog);
  }
};
