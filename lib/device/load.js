 ...
 * 
 *  - Загружает типы и устройства в holder
 */
...
module.exports = async function (holder, agent) {
  const dm = holder.dm;
  ...
  deviceDocs.forEach(item => {
    if (item.dn) {
      const devStruct = device_struct.create(item, holder.typeMap, devData[item._id], devChannels[item._id]);
      holder.devSet[item._id] = new Devo(devStruct, holder.typeMap, agent);
