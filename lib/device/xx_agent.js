...
module.exports = {
  ...
  getLocation(did) {
    return '/place/'+deviceutil.getDevicePlacePath(did, this.holder, 'id');
  },
    ...
    if (dobj && dobj.innerLog) {
      ...
      if (dobj.innerLog.length > this.deviceloglines+10) {
        ...
        if (this.removedLines[did] >  this.deviceloglines) {
          ...
          this.holder.logconnector.deleteLog('devicelog',{ts:dobj.innerLog[0].ts, where:[{name:'did', val:did}]});
        }
      }
