...
async function loadOrCreate(typeId, prop, typeObj) {
  ...
  if (!strFromFile)
    strFromFile = prop.startsWith('_format')
      ? createNewFormatHandler(typeId, prop)
...
function createNewFormatHandler(typeId, inprop) {
  if (!prop || !prop.startsWith('_format')) return '';
  ...
  const desc_val = 'val - выводимое значение';
  const fn = defaulthandlers.format_def;
