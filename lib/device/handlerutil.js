...
function defaultGlobalVarHandler() {
  ...
  module.exports = function (currentValue, devs)  { 
    ...
    Object.keys(devs).forEach(dn => {
      res = res || devs[dn].state; // Если хотя бы один state=1,  то значение = 1
      // res = res && devs[dn].state; // Если все state=1,  то значение = 1
    });
    return res;
  }
  `;
...
*
*   Полный список команд документацию в разделе Скрипты визуализации   
*/

module.exports = function ({local}, responder)  { 
  // if (local.level == 1) {
