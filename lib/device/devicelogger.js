...
module.exports = {
  ...
  addLog(did, logObj) {
    ...
    // Передать подписчикам (devicelog на интерфейсах - только для устройств!!
  ...
  /**
   * Удаление всех записей по устройству
   * @param {String} did 
   */
  deleteLog(did) {
    this.devInnerLog[did] = [];
    logconnector.deleteLog('devicelog', { where: [{ name: 'did', val: did }] });
  },
  ...
  async fillInnerLogFromTable(did) {
    ...
    // При первом заполнении сортировать, т к данные, загруженные на старте м б не по порядку
    innerLog.sort(hut.byorder('ts'));
