...
class Deviceengine {
  ...
  start() {
    ...
    this.holder.on('create:unitIndicator', (unitId, { parent, version }) => {
  ...
  complyDevice(accepted) {
    ...
    accepted.forEach(item => {
      if (item.did && this.devSet[item.did]) {
        ...
        const { did, prop, ts, val, prev, raw, param, txt, fval } = this.devSet[item.did].comply(item);

        // Запись в таблицы текущих значений - только если есть изменение. Иначе придет {}
        ...
        if (did && prop) {
          ...
          // Запись в devicelog - пишем отформатированное значение сразу
          const value = fval != undefined ? fval : val;
          devicelogger.addLog(did, { did, prop, ts, val: value, txt });
        }
      }
    });
  ...
  changeTypeProps(typeId, newprops) {
    const typeObj = this.getTypeObj(typeId);
    // Нужно сформировать новую структуру (type_struct?) и передать готовую структуру + список изменений ()
    // typeObj.changeProps(chobj);
    type_struct.updateProps(typeObj, newprops);
    this.holder.deviceWorker.postMessage({ name: 'update:type:props', data: { typeId, newprops } });
  }

  changeTypeAlerts(typeId, chobj) {
    const typeObj = this.getTypeObj(typeId);
    typeObj.changeAlerts(chobj);
    this.holder.emit('update:type:alerts', typeId, propArr);
  }

  emitUpdateTypeAlerts(name, typeId, propArr) {
    this.holder.emit(name + ':type:alerts', typeId, propArr);
  }
