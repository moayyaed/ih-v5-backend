...
class Deviceengine {
  constructor(holder) {
    ...
    // Журнал устройства
    this.devInnerLog = holder.devInnerLog;
    this.removedLines = {}; // журнал devInnerLog
  ...
  addType(doc) {
    ...
    const typeObj = new Typo(t_struct);
    this.typeMap.set(typeId, typeObj);
  ...
  changeTypeHandler(filename) {
    hut.unrequire(filename);
    this.holder.deviceWorker.postMessage({ name: 'update:type:handler', data: { filename } });
  }
  ...
  logDevice(did, logObj) {
    const dobj = this.devSet[did];
    if (!dobj) return [];

    const innerLog = this.holder.devInnerLog[did];
    if (!innerLog) this.holder.devInnerLog[did] = [];
