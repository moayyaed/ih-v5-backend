...
class Deviceengine {
  ...
  start() {
    ...
    this.holder.on('create:unitIndicator', (unitId, version) => {
      ...
      if (this.devSet[id] && version) {
        this.devSet[id].comply({did:id, prop:'version', value:version})
      }
    });
  ...
  changeTypeAlerts(typeId, chobj) {
    // console.log('MAIN update:type:alerts  chobj='+util.inspect(chobj, null, 4))
    const typeObj = this.getTypeObj(typeId);
    typeObj.changeAlerts(chobj);
    this.holder.deviceWorker.postMessage({ name: 'update:type:alerts', data: { typeId, chobj } });
  }
