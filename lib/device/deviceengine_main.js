...
class Deviceengine {
  ...
  start() {
    ...
    // Получены архивные данные - пока просто записать??  
    this.holder.on('received:device:archive', arr => {
      arr.forEach(item => {
        const {did, prop, ts, val} = item;
        devicelogger.addLog(did, { did, prop, ts, val});
      });
    });
  ...
  sendToWorker(name, data) {
    this.holder.deviceWorker.postMessage({ name, data });
  }
