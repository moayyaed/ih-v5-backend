...
class Deviceengine {
  ...
  complyGlobal(changed) {
    console.log('complyGlobal '+util.inspect(changed))
    changed.forEach(item => {
      const logObj = this.holder.global.comply(item);

      // Запись в devicelog
      if (logObj && logObj.did) {
  ...
  addGlobal(doc) {
    if (!doc || !doc._id) return;
    const did = doc._id;
   //  const struct = gl_struct.create(doc);
    this.holder.global.addItem(did, doc);
    this.holder.deviceWorker.postMessage({ name: 'add:global', data: doc });
  }
