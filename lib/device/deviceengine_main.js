...
class Deviceengine {
  ...
  async changeTypeAlerts(typeId, prop) {
    ...
    this.holder.emit('update:type:alerts', typeId, prop);
  }

  emitAddTypeAlerts(typeId, propArr) {
    this.holder.emit('add:type:alerts', typeId, propArr);
  }

  emitRemoveTypeAlerts(typeId, propArr) {
    this.holder.emit('remove:type:alerts', typeId, propArr);
  }
  ...
  async processRenamedPropHandler(typeId, oldProp, newProp, use) {
    console.log('processRenamedPropHandler')
    ...
    if (use == 2) { // Пользовательский обработчик
