...
class Deviceengine {
  ...
  complyDevice(accepted) {
    ...
    //  console.log('complyDevice accepted=' + util.inspect(accepted));
    // }

    accepted.forEach(item => {
      if (item.did && this.devSet[item.did]) {
        // Сохранить значение в basedevo
        const { did, prop, ts, val, prev, raw, param, txt, fval, err } = this.devSet[item.did].comply(item);
  ...
  changeTypeProps(typeId, newprops) {
    const typeObj = this.getTypeObj(typeId);
    // Нужно сформировать новую структуру (type_struct?) и передать готовую структуру на worker
