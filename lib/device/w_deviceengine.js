...
class Deviceengine {
  ...
  start() {
    ...
    this.wCore.on('getcb:device:raw', data => {
      const did = data.did;
      if (did && this.devSet[did]) {
        const sobj = { ...data, result: this.devSet[did]._raw };
    ...
    this.wCore.on('remove:device', data => {
      console.log('WORKER remove:device '+util.inspect(data))
      this.removeDevice(data);
    });

    // Изменения в таблицах - приходит сообщение, что конкретно изменилось
  ...
  getHandlerForType(type, event) {
    if (this.typeMap.has(type)) {
      ...
      return typeObj.getOnHandler(event);
    }
  ...
  //    Операции добавления/ удаления/ редактирования устройств по сообщениям от main
  /**
  ...
  removeDevice(doc) {
    ...
    console.log('WORKER removeDeviceDevice '+util.inspect(doc))
