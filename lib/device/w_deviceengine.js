...
class Deviceengine {
  ...
  start() {
    ...
    this.wCore.on('update:type:handler', data => {
      if (!this.checkData(data, 'update:type:props', ['typeId','filename'])) return;
      const typeObj = this.typeMap.get(data.typeId);
      if (!typeObj) return;
   
      const prop = handlerutil.getPropFromHandlerName(data.typeId,data.filename);
      // Сбросить fn, чтобы затем ее заново загрузить
      typeObj.clearFn(prop);
  ...
  getHandlerForType(type, event) {
    if (this.typeMap.has(type)) {
      ...
      return typeObj.getOnHandlerFunction(event);
    }
  ...
  runDeviceHandler(did, hanid, fn, triggers) {
    try {
      if (fn) {
        this.wCore.postMessage('trace:handler', { did, hanid, state: 1 });
        this.wCore.currentScriptTs = Date.now();
        fn(this.devSet[did], triggers, this.globals);
        this.wCore.postMessage('trace:handler', { did, hanid, state: 0 });
      }
    } catch (e) {
