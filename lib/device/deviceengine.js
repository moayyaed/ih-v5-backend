...
class Deviceengine {
  ...
  addDevice(item, dataObj) {
    if (item.dn) {
      this.devSet[item._id] = new Devo(item, this.typestore, this.agent, dataObj);
