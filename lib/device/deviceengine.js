  ...
  start(devDocs) {
    ...
    devDocs.forEach(item => {
      if (item.dn) {
        ...
        this.dnSet[item.dn] = this.devSet[item._id].dnobj; // Объект устройства для обращения по dn, свойства плоские
    ...
    console.log('HOLDER this.nSet = ' + util.inspect(this.dnSet));
