...
class Deviceengine {
    ...
    this.holder.on('received:device:data', getObj => {
      ...
      Object.keys(getObj).forEach(did => {
        if (did && this.devSet[did]) {
          ...
          console.log(
            'ERROR: Handle received:device:data. Device not found ' +
              did +
              ' Exists: ' +
              Object.keys(this.devSet).join(',')
          );
      });
    ...
    this.holder.on('accepted:device:data', accepted => {
      ...
      if (changed.length) {
        ...
        changed.forEach(item => {
          if (item.did && this.devSet[item.did] && this.devSet[item.did].type && this.onChangeByType[this.devSet[item.did].type]) {
        ...
        Object.keys(devChanged).forEach(did => {
          ...
          if (props) {
            ...
            if (triggers.length) {
              this.runDeviceHandler(did, '_OnChange', triggers);
            }
          }
        });
      }
  ...
  runDeviceHandler(did, event, triggers) {
    try {
      const fn = this.devSet[did].typeobj.onHandlers['fn_' + event];
      if (fn) fn(this.devSet[did], triggers);
    } catch (e) {
      console.log('ERROR: Handler ' + event + ': ' + util.inspect(e));
      this.devSet[did].agent.logDevice(did, { prop: 'error', txt: 'Handler ' + event + ' error' });
    }
  }
  ...
  formTypeHandlerSets(arr) {
    arr.forEach(item => {
      ...
      if (item.scriptOnSchedule) {
        addToSet(this.onScheduleByType, type, item.par_OnSchedule);
      } else {
        // сбросить
        removeFromAllSet(this.onScheduleByType, type);
      }
    });
