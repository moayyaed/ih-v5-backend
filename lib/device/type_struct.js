...
function fillHandlerObj(typeObj, event, usage) {
  ...
  // Если обработчика нет - создать файл с пустым скриптом
  let filename = '';
  if (usage) {
    filename = handlerutil.getHandlerFilename(typeId, event);
    // Если нет файла - создать из стандартного
    if (!filename) {
      filename = appconfig.getHandlerFilename(typeId + '_' + event);
      // handlerutil.createHandler(typeId, event, typeObj, filename);
      handlerutil.createOnHandler(typeId, event, filename);
    }
  }
...
function fillPropHandlerObj(typeObj, prop) {
  ...
  if (propItem.fuse == 1) {
    ...
    filename = appconfig.getHandlerFilenameIfExists(typeId + '_' + prop);
    // Если нет файла - создать из стандартного
    if (!filename) {
      filename = appconfig.getHandlerFilename(typeId + '_' + prop);
      handlerutil.createHandler(typeId, prop, typeObj, filename);
    }
    sys = 0;
  }
...
function getProparrAux(prop, propItem) {
  ...
  switch (propItem.vtype) {
    case 'N':
      ...
      res.def = !isNaN(propItem.def) ? Number(propItem.def) :0;
      break;
    case 'S':
      res.def = propItem.def != undefined ? String(propItem.def) : '';
      break;
    case 'B':
      res.def = propItem.def == 1 ? 1 : 0;
      break;
    default:
  }
