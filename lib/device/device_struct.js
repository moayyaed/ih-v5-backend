...
function create(devDoc, typeMap, dataObj, chanObj) {
  ...
  const typeobj = typeMap.get(devObj.type);
  ...
  // Расширенные свойства - общий набор
  devObj.extProps = {};
  // Добавить расширенные свойства системного уровня - для системных индикаторов
  if (devDoc.extProps && Array.isArray(devDoc.extProps)) {
    devDoc.extProps.forEach(item => {
      if (item.prop) {
        const prop = item.prop;
        if (devObj._raw[prop] == undefined) {
          // Нет типового
          devObj[prop] = item.val;
          devObj._raw[prop] = { raw: item.val, val: item.val, src: 'def' };
          devObj.extProps[prop] = hut.clone(item);
        }
      }
    });
  }
...
/**
 * Извлечь структуру из объекта для повторной передачи на worker
 * @param {Object} devObj
 */
function extract(devObj) {
  ...
  flat.push('dn', 'type', 'error'); // Эти есть всегда
  flat.forEach(prop => {
