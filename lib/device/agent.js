...
module.exports = {
  ...
  async fillInnerLogFromTable(did) {
    const dobj = this.holder.devSet[did];
   if (dobj.innerLog.length && dobj.innerLog[0].boot) { // Первые записи - boot -> из таблицы данные еще не брались 
        const bootTs = dobj.innerLog[0].boot;
        // const beforeArr = await this.holder.dm.getLog('devicelog', { did,  ts: { $lt: bootTs }  }); 
