...
module.exports = {
  ...
  doCommand(dobj, prop, sender) {
    const did = dobj._id;
    this.logDevice(did, {cmd:prop, ts:Date.now()-1, ...sender} )
  ...
  getWriteChan(did, prop) {
    ...
    const key = did+'.'+prop;
    const unitId = this.holder.writeChanMap.get(key);
    if (!unitId || !this.holder.unitSet[unitId] || !this.holder.unitSet[unitId].writeMap) return;

    return this.holder.unitSet[unitId].writeMap.get(key);
  },
  ...
  saveDevParam(did, prop, value, ts) {
    this.dm.insertDocs('devparam', [{ _id:did+'_'+prop, val:value, ts }]);
  },

  logDevice(did, logObj ) {
    // this.holder.emit('log', { did, ...logObj }, 'devicelog');
