...
function requireHandler(prop, p, mainProp) {
  try {
...
function getDefHandlerName(prop, { vtype, op }, mainProp) {
  switch (op) {
    ...
    case 'cmd':
      if (prop == 'toggle') return 'def_cmd_' + prop; // def_cmd_toggle
      if (mainProp && (prop == 'on' || prop == 'off')) return 'def_cmd_' +mainProp+'_'+ prop; // def_cmd_state_on
      return '';
...
async function syncHandlers(docs) {
  ...
  try {
    ...
    console.log('ERROR:  handlerutils.syncScripts ' + util.inspect(e));
  }
