  ...
  change(chobj) {
    ...
    Object.keys(chobj).forEach(prop => {
      if (this._raw[prop] != undefined) {
        ...
        if (res.val != undefined) changed.push({ prop, value: res.val, ts });
    ...
    if (this.typeObj.calc.length && (changed.length || errChanged)) {
      this.typeObj.calc.forEach(calcProp => {
        ...
        if (res.val != undefined) changed.push({ prop: calcProp, value: res.val, ts });
    ...
    if (errChanged) {
      ...
      if (this.error != newerror) {
        this.error = newerror;
        changed.push({ prop: 'error', value: newerror, ts });
      }
    }
    return changed;
  }
