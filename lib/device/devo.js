...
class Devo {
  ...
  setChannel(prop, chanObj) {
    if (chanObj.w) {
      ...
      if (chanObj.calc_out) {
        chanObj.calc_outfn = new Function('value', 'return ' + chanObj.calc_out);
      }
    }
  ...
  writeChannel(prop, val) {
    ...
    if (item && item.calc_outfn) {
      val = item.calc_outfn(val);
    }
    console.log('WARN: '+this.dn + ' writeChannel val= ' + val);
    this.agent.writeDeviceChannel(this._id, item, val);
  }
