...
class Devo {
  constructor(devDoc, typestore) {
    ...
    this.typeobj = typestore.getTypeObj(devDoc.type); // Вся информация о структуре, типе данных, ф-и - обработчики - здесь
  ...
  changeTypeProps(addProps, deleteProps) {
    ...
    if (addProps) {
      addProps.forEach(prop => {
        ...
        console.log('changeTypeProps add '+prop+' typePropItem='+util.inspect(typePropItem));
        this.addProp(prop, typePropItem);
      });
    }

    console.log('AFTER changeTypeProps '+util.inspect(this));
  }
