...
class Devo {
  ...
  doCommand(command) {
    this.agent(this, command);
    // Сохранить время выполнения команды и ошибку?
      ...
      if (calcItem.when == 0) {
        if (!chobj) {
          ...
          accepted.forEach(aitem => {
            if (aitem.changed) chobj[aitem.prop] = aitem.val;
          });
        }
        return (
  ...
  acceptNewValue(prop, ts, newval) {
    ...
    if (this[prop] != newval) {
      ...
      res = { prop, ts, value: newval, changed: 1, prev: this._raw[prop].prev };
    } else {
      res = { prop, ts, value: newval }; // значение принято, оно не изменилось
    }
    return res;
  }
  ...
  runCalcFun(prop, paramObj) {
    ...
    if (one) this.agent.emitDeviceDataAccepted([one]);
  }
  ...
  runCommandHandler(prop, virt) {
    ...
    try {
      ...
      console.log('Command handler error! ' + hut.getShortErrStr(e));
      this._raw[prop].err = hut.getShortErrStr(e);
    }
    this._raw[prop].ts = Date.now(); // Момент последнего выполнения команды
  }
  ...
  addExtProps(extPropsForScene, scene) {
    ...
    extPropsForScene.forEach(item => {
      ...
      if (!this.extProps[prop]) {
        ...
        this.extProps[prop] = { ext: { ...item }, scene };
      }
      // else {
      //  this.extProps[prop].src.push(scene);
      // }
