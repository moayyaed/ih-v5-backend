...
class Devo {
  ...
  acceptOne(prop, ts, val, sender) {
    ...
    // console.log('acceptOne prop '+prop+' fn='+(fn ? fn.toString() : ' NO '))
  ...
  comply({ prop, ts, value, changed, prev }) {
  ...
  getHandler(prop) {
    return this.typeobj.getPropHandler(prop);
  }

  getFormatHandler(prop) {
    return this.typeobj.getPropHandler(prop, 'formathandler');
  }
