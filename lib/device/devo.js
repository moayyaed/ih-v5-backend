...
class Devo {
  ...
  mainlog(txt, level = 0) {
    if (!txt) return;
    console.log('MAINLOG: '+util.inspect({ ts: Date.now(), txt, did:this._id, level, sender: 'DEV:'+this.dn, tags:this.tags }))
    this.agent.logMain({ ts: Date.now(), txt, did:this._id, level, sender: 'DEV:'+this.dn, tags:this.tags });
  }
