...
class Devo {
  ...
  constructor(devDoc, typestore, agent, dataObj) {
  ...
  acceptOne(prop, ts, val) {
    ...
    return this.acceptNewValue(prop, ts, fn ? this.runHandler(prop, val) : val);
  }
  ...
  acceptNewValue(prop, ts, newval) {
    ...
    if (this[prop] != newval) {
      ...
      // Параметр сохранить
      if (this.isParam(prop)) this.agent.saveDevParam(this._id, prop, newval, ts);
