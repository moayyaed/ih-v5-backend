...
class Devo {
   ...
   * 
   * @param {Object} extPropsForScene {name:'timerOff', val:10, note:'Время ...'}
   *                 name - имя нового свойства
   * @param {String} sceneId
   * 
   *  Выполняется добавление:
   *   1. в this.extProps[prop] = {ext:<переданный объект>, scenes:[массив sceneId]}
   *     Сохраняются все параметры переданого объекта
   *     Если свойство уже есть -  добавить sceneId в массив
   *        Несколько сценариев могут объявить одно и то же свойство, конфликта не будет 
   *        !!Если второй сценарий определяет свойство по другому, то это не будет учтено
   ...
   * 
   *   2. Будет добавлено новое свойство устройства this[prop] (+_raw[prop])
   */
  addExtProps(extPropsForScene, sceneId) {
    ...
    extPropsForScene.forEach(item => {
      ...
      if (!this.extProps[prop]) this.extProps[prop] = { ext: { ...item }, scenes: [] };
      if (!this.extProps[prop].scenes.includes(sceneId)) this.extProps[prop].scenes.push(sceneId);

      if (this[prop] == undefined) { // Если не задано - задаем дефолтное значение
        this[prop] = item.val;
