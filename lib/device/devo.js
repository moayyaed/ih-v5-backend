...
class Devo {
  ...
  changeOne(prop, ts, val) {
    ...
    if (this.typeobj.props[prop].readfun) {
      const newObj = this.tryReadHandler(prop, val);
  ...
  tryReadHandler(prop, arg3, arg4) {
    ...
    try {
      const res = this.typeobj.props[prop].readfun(this, prop, arg3, arg4); // возвращает значение или объект
  ...
  calcOne(prop, ts, chobj, paramObj) {
    if (this.typeobj.props[prop].readfun) {
      const newObj = this.tryReadHandler(prop, chobj, paramObj);
      return this.setNewValueAndErr(prop, ts, newObj);
    }
