...
class Devo {
    ...
    */
    ...
    if (dataObj) {
      Object.keys(dataObj).forEach(prop => {
        this._raw[prop] = dataObj[prop]; // { raw: val, val, ts, src };
        this[prop] = dataObj[prop].val;
        // Добавляем как первые записи в журнал с флагом first
        // this.innerLog.push({did, prop, val, ts, sender:'first'})
  ...
  addProp(prop, typePropItem, devPropItem) {
    let fn_opt;
    let log = 0; // По умолчанию вЫключено
    if (devPropItem) {
      if (devPropItem.log != undefined) {
        log = devPropItem.log;
      }
      if (devPropItem.fn_opt_str) {
        try {
          fn_opt = JSON.parse(devPropItem.fn_opt_str);
        } catch (e) {
          console.log('ERROR: addProp ' + prop + ' for device ' + this._id + ' fn_opt ' + fn_opt + util.inspect(e));
        }
      }
    }

    this._aux.set(prop, { ...typePropItem, ...devPropItem, fn_opt, log });
    // this._aux.set(prop, { ...typePropItem, ...devPropItem, fn_opt, log:1 });
  ...
  getFn_opt_str(prop) {
    console.log('getFn_opt_str ' + prop + ' typeob=' + util.inspect(this.typeobj.props[prop]));
  ...
  acceptData(inObj, sender) {
    ...
    // console.log('WARN: '+this.dn+' START acceptData '+util.inspect(inObj))
  ...
  addToInnerLog(logObj) {
    if (this.innerLog.length > 100) this.innerLog.splice(0, this.innerLog.length - 110); // Убрать на 10 больше
    this.innerLog.push({ did: this._id, ...logObj });
  }
   ...
   */
  async getInnerLog() {
    if (this.innerLog.length < 90 && this.innerLog[0].mark == 'first') {
