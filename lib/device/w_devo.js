...
class Devo {
   ...
   * @param {Object} devObj - анемичный объект, уже созданный из типа
   *
   * @param {Object} agent - объект для работы с таймерами, логами и отправки команд плагинам
   */

  constructor(devObj, typeMap, agent) {
    Object.assign(this, devObj);
  ...
  hasFormat(prop) {
    const fn = this.getFormatHandler(prop);
  ...
  formatValue(prop, val) {
    ...
    try {
      const fn_format = this.getFormatHandler(prop);
      if (fn_format) {
  ...
  runHandler(prop, arg3, arg4) {
    try {
      ...
      const res = fn(this, prop, arg3, arg4); // возвращает значение или объект
  ...
  getHandler(prop) {
    return this.typestore.getPropHandler(this.type, prop);
  }

  getFormatHandler(prop) {
    return this.typestore.getPropFormatHandler(this.type, prop);
  }
