...
describe('utils/hut', () => {
  ...
  describe('Array function: 	arrayintersection', () => {
    it('first or second array is empty', () => {
      expect(hut.arrayIntersection([], ['test']).length).toEqual(0);
      expect(hut.intersection(['test'], []).length).toEqual(0);
    });
    it('should be result len=1', () => {
      const arr1 = ['one', 'two'];
      const arr2 = ['two', 'five'];

      expect(hut.arrayIntersection(arr1, arr2).length).toEqual(1);
    });

    it('Object keys intersection', () => {
      const obj1 = { val: 1, state: 1 };
      const obj2 = { state: {}, setpoint: 42, level: true };

      expect(hut.arrayIntersection(Object.keys(obj1), Object.keys(obj2)).length).toEqual(1);
    });

    it('Object keys intersection', () => {
      const obj1 = { val: 1, state: 1 };
      const obj2 = { state1: {}, setpoint: 42, level: true };

      expect(hut.arrayIntersection(Object.keys(obj1), Object.keys(obj2)).length).toEqual(0);
    });
  });
  ...
  describe('getFileExt', () => {
    ...
    it('getFileExt - schemeplace.sch', () => {
      let filename = 'schemeplaces.sch';
      let ext = hut.getFileExt(filename);
      expect(ext).toEqual('sch');
    });
  ...
  describe('formArrayReplay', () => {
    it('formArrayReplay simple ', () => {
      let data = [
        { id: '1', name: 'one', comment: 'comment 1' },
        { id: '2', name: 'two' }
      ];
  ...
  describe('merge', () => {
    it('merge two empty objects ', () => {
      let a = {};
      let b = {};
      let res = hut.merge(a, b);
      expect(deepEqual(res, {})).toEqual(true);
    });
    it('merge with empty object ', () => {
      let a = { r1: { x: 1, y: 1 }, r2: { x: 2, y: 2 } };
      let b = {};
      let res = hut.merge(a, b);
      expect(deepEqual(res, { r1: { x: 1, y: 1 }, r2: { x: 2, y: 2 } })).toEqual(true);
    });

    it('merge cross objects - expand1', () => {
      let a = { r1: { x: 1, y: 1 }, r2: { x: 2, y: 2 } };
      let b = { r1: { z: 1 } };
      let res = hut.merge(a, b);
      expect(deepEqual(res, { r1: { x: 1, y: 1, z: 1 }, r2: { x: 2, y: 2 } })).toEqual(true);
    });

    it('merge cross objects - expand2', () => {
      let a = { r1: { x: 1, y: 1 }, r2: { x: 2, y: 2 } };
      let b = { r1: { xx: 1, yy: 1 }, r2: { xx: 2 } };
      let res = hut.merge(a, b);
      expect(deepEqual(res, { r1: { x: 1, y: 1, xx: 1, yy: 1 }, r2: { x: 2, y: 2, xx: 2 } })).toEqual(true);
    });

    it('merge cross objects - override', () => {
      let a = { r1: { x: 1, y: 1 }, r2: { x: 2, y: 2 } };
      let b = { r1: { x: 42, y: 42 }, r2: { xx: 2 } };
      let res = hut.merge(a, b);
      expect(deepEqual(res, { r1: { x: 42, y: 42 }, r2: { x: 2, y: 2, xx: 2 } })).toEqual(true);
    });
  });
  ...
  // arrayToGroupObjectWithElementArray
  describe('arrayToGroupObjectWithElementArray', () => {
    it('not array', () => {
      const res = hut.arrayToGroupObjectWithElementArray('test', 'key', 'element');
      expect(typeof res).toEqual('object');
      expect(hut.isObjIdle(res)).toEqual(true);
    });

    it('true case with 2 group', () => {
      const arr = [
        { key: 'xx', type: 't1', el: 'XX' },
        { key: 'xx', type: 't2', el: 'XY' },
        { key: 'xx', type: 't3', el: 'XZ' },
        { key: 'yy', type: 't1', el: 'YY' }
      ];
      const res = hut.arrayToGroupObjectWithElementArray(arr, 'key', 'el');
      expect(typeof res).toEqual('object');
      expect(hut.isObjIdle(res)).toEqual(false);
      expect(deepEqual(res, { xx: ['XX', 'XY', 'XZ'], yy: ['YY'] })).toEqual(true);
    });
  });

  describe('compareSemVer', () => {
    it('New ver - patch', () => {
      let res = hut.compareSemVer('1.1.1', '1.1.0');
      expect(res).toEqual(3);
    });
    it('New ver - minor', () => {
      let res = hut.compareSemVer('1.2.1', '1.1.0');
      expect(res).toEqual(2);
    });
    it('New ver - major', () => {
      let res = hut.compareSemVer('1.2.1', '0.1.0');
      expect(res).toEqual(1);
    });
    it('New ver - patch 10', () => {
      let res = hut.compareSemVer('1.1.10', '1.1.9');
      expect(res).toEqual(3);
    });

    it('New ver - short ver', () => {
      let res = hut.compareSemVer('1.1', '1.1.9');
      expect(res).toEqual(0);
    });

    it('New ver - prev beta ver', () => {
      let res = hut.compareSemVer('5.1.1', '5.1.0-beta');
      expect(res).toEqual(3);
    });

    it('New ver - new beta ver', () => {
      let res = hut.compareSemVer('5.1.1-beta', '5.1.0');
      expect(res).toEqual(3);
    });
  });
});
