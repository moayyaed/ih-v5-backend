...
describe('utils/hut', () => {
  ...
  describe('fillRelFields', () => {
    it('fillRelFields - place, room', () => {
      let data = [{ id: 1, name: 'Test', place: '1', room: '2' }];
      hut.fillRelFields(data, ['place', 'room'], {
        place: { '1': { name: 'First' }, '2': { name: 'Second' } },
        room: { '1': { name: 'First room' }, '2': { name: 'Second room' } }
      });
      expect(data[0].place).toEqual('First');
      expect(data[0].room).toEqual('Second room');
    });

    it('fillRelFields - {key:place}, {key:room}', () => {
      let data = [{ id: 1, name: 'Test', place: '1', room: '2' }];
      hut.fillRelFields(data, [{ key: 'place' }, { key: 'room' }], {
        place: { '1': { name: 'First' }, '2': { name: 'Second' } },
        room: { '1': { name: 'First room' }, '2': { name: 'Second room' } }
      });
      expect(data[0].place).toEqual('First');
    ...
    it('fillRelFields - {key:place, resfield:place_name}, {key:room, resfield:room_name}', () => {
      let data = [{ id: 1, name: 'Test', place: '1', room: '2' }];
      hut.fillRelFields(data, [{ key: 'place', resfield: 'place_name' }, { key: 'room', resfield: 'room_name' }], {
        place: { '1': { name: 'First' }, '2': { name: 'Second' } },
        room: { '1': { name: 'First room' }, '2': { name: 'Second room' } }
      });
      expect(data[0].place_name).toEqual('First');
      expect(data[0].room_name).toEqual('Second room');
    });

    it('fillRelFields - {key:place, relfield:img, resfield:place_img}, {key:room, resprop:room_name}', () => {
      let data = [{ id: 1, name: 'Test', place: '1', room: '2' }];
      hut.fillRelFields(
        data,
        [{ key: 'place', relfield: 'img', resfield: 'place_img' }, { key: 'room', resfield: 'room_name' }],
        {
          place: { '1': { name: 'First', img: 'first.png' }, '2': { name: 'Second' } },
          room: { '1': { name: 'First room' }, '2': { name: 'Second room' } }
        }
      );
      expect(data[0].place_img).toEqual('first.png');
      expect(data[0].room_name).toEqual('Second room');
    });
  });
  ...
  describe('getFileNameExtLess', () => {
    ...
    it('getFileNameExtLess - place', () => {
      let filename = 'c:/documents/place.sch';
      let res = hut.getFileNameExtLess(filename);
      expect(res).toEqual('place');
    });
  });
  ...
  describe('substitude', () => {
    it('substitude 1', () => {
      let template ='data.place == {place}';
      let data = { id: '1', name: 'one', place: '2' };
      let res = hut.substitude(template, data);
      expect(typeof res).toEqual('string');
      expect(res).toEqual('data.place == 2');   
    });
  ...
  it('substitude 3', () => {
      let template ='data.place == "{place}" && data.name == "{name}"';
      let data = { id: '1', name: 'one', place: '2' };
      let res = hut.substitude(template, data);
      expect(typeof res).toEqual('string');
      expect(res).toEqual('data.place == "2" && data.name == "one"');   
    });
  });
 ...
 describe('test concat ES6', () => {
    it('concat from map ', () => {
      ...
      expect(typeof res).toEqual('object');
      // console.log('concat from map '+util.inspect(res));
  ...
  // isTheSameArray
  describe('isTheSameArray', () => {
    it('arrays are the same', () => {
      let res = hut.isTheSameArray(['1','2'],['1','2']);
      expect(res).toEqual(true);   
    });
    
    it('arrays are the same but order is dif', () => {
      let res = hut.isTheSameArray(['1','2'],['2','1']);
      expect(res).toEqual(true);   
    });

    it('arrays are not the same', () => {
      let res = hut.isTheSameArray(['1','2'],['1','2','3']);
      expect(res).toEqual(false);   
    });

    it('arrays are not the same', () => {
      let res = hut.isTheSameArray(['1','2','3'],['1','2']);
      expect(res).toEqual(false);   
    });

    it('arrays are empty', () => {
      let res = hut.isTheSameArray([],[]);
      expect(res).toEqual(true);   
    });
    it('onearray is empty', () => {
      let res = hut.isTheSameArray(['1'],[]);
      expect(res).toEqual(false);   
    });
  });
