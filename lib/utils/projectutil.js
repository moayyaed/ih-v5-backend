...
const transferBerry = require('../../upgrade2V5/transferBerry');
...
// Создается запись в таблице projects - всегда создается заново на базе папок
...
async function createProject(doc) {
  try {
    const project = 'project_' + String(Date.now());
    const projpath = getNewProjectFolder(project);

    // Создать папку с проектом
    checkAndMakeNewProjectFolder(project);
    console.log('WARN: Create new project folder: ' + projpath);
    doc._id = project;
    doc.projectfolder = project;
    doc.title = project;

    // Создать папки внутри папки проекта
    appconfig.makeProjectFolders({ projectpath: projpath, project });

    // Сразу заполнить из assets
    // Там лежит демо проект, поэтому копируем выборочно
    const fromFolder = path.join(appconfig.get('syspath'), 'assets', 'project');
    if (!fs.existsSync(fromFolder)) {
      console.log('WARN: Not found folder with default project: ' + fromFolder);
      return;
    }

    // Копировать картинки - из папки в папку
    await copyImages(fromFolder + '/images', projpath + '/images');

    // Копировать шаблоны и диалоги
    await copyFolderP(fromFolder + '/jbase/template', projpath + '/jbase/template', '.json');
    await copyFolderP(fromFolder + '/jbase/dialog', projpath + '/jbase/dialog', '.json');

    // Копировать обработчики
    await copyFolderP(fromFolder + '/handlers', projpath + '/handlers', '.js');

    // Типы и другие списки
    const jbaseFiles = ['types.db', 'images.db', 'dialogs.db', 'handlers.db', 'vistemplates.db'];
    await copyFilesP(fromFolder + '/jbase', projpath + '/jbase', jbaseFiles);
    return doc;
  } catch (e) {
    throw { message: 'create project ' + (e.message || ' fail!') };
  }
}

async function copyFromAssets(projpath) {
  // Сразу заполнить из assets
  // Там лежит демо проект, поэтому копируем выборочно
  const fromFolder = path.join(appconfig.get('syspath'), 'assets', 'project');
  if (!fs.existsSync(fromFolder)) {
...
async function copyFolderP(sourceFolder, targetFolder, ext) {
  try {
    if (!fs.existsSync(sourceFolder)) throw { message: 'Source folder not found: ' + sourceFolder };
    if (!fs.existsSync(targetFolder)) throw { message: 'Target folder not found: ' + targetFolder };

    const arr = await fs.promises.readdir(sourceFolder);
    if (!arr) throw { message: 'Error read folder:' + sourceFolder };

    const selectedFiles = arr.filter(item => isFitFile(item));
    ...
    throw { message: 'copyFolderP: ' + e.message };
  }

  function isFitFile(filename) {
    return filename && (!ext || filename.endsWith(ext));
  }
}

async function copyFilesP(sourceFolder, targetFolder, files) {
  try {
    ...
    const promises = files.map(file => fs.promises.copyFile(sourceFolder + '/' + file, targetFolder + '/' + file));
    return Promise.all(promises);
  } catch (e) {
    throw { message: 'copyFilesP: ' + e.message };
  }
}
...
async function transferProjectBerry(fromFolder, project, emitMes) {
  const projectpath = appconfig.get('projectspath') + '/' + project;
  // Создать папку проекта и project.json
  checkAndMakeNewProjectFolder(project);
  ...
  await copyFromAssets(projectpath);

  await transferBerry(fromFolder, projectpath, emitMes);
}
