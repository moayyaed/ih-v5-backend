...
async function updateProjectProps(doc) {
  ...
  if (doc.$set.deps != undefined) {
    appconfig.setTheProjectProp(projectfolder, 'deps', doc.$set.deps);
    createPackageJsonIfNotExists(doc._id);
  }
...
async function getNpmList(folder) {
  const res = [];
  try {
    // createPackageJsonIfNotExists(folder);
    const stdoutStr = await wu.tryRunCmdP(appconfig.getNpmListStr(), { cwd: folder });
    const resObj = JSON.parse(stdoutStr);
    if (resObj && resObj.dependencies) {
