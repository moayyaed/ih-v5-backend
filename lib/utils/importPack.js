...
module.exports = async function(packFolder, watchid, holder) {
  ...
  async function installDbagent() {
    ...
    holder.dm.invalidateCache({  method: 'getmeta', type: 'dbagentinfo', id: info.id });
    holder.dm.invalidateCache({  method: 'getmeta', type: 'form', id: 'formDbagentCommon', nodeid: info.id });

    // Если добавляется новый агент - пользователь сам должен активировать?
    // Но в дерево добавим
  ...
  async function installPlugin() {
    ...
    holder.dm.invalidateCache({  method: 'getmeta', type: 'plugininfo', id: pluginid });
    holder.dm.invalidateCache({  method: 'getmeta', type: 'manifest', id: pluginid });
    holder.dm.invalidateCache({  method: 'getmeta', type: 'form', id: 'formPluginCommon', nodeid: pluginid });
    holder.dm.invalidateCache({  method: 'getmeta', type: 'form', id: 'formPluginChannelsTable', nodeid: pluginid });
    holder.dm.invalidateCache({  method: 'getmeta', type: 'form', id: 'channelview', nodeid: pluginid });
  ...
  async function installProject() {
    ...
    if (appconfig.isV5Project(packFolder)) {
      ...
      // Считать версию проекта из project.json
