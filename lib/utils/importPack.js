...
module.exports = async function(packFolder, watchid, holder) {
  ...
  async function installPlugin() {
    ...
    try {
      await pluginutil.installPlugin(packFolder, pluginInfo, tmes => {
        emitWatch(tmes);
      });
    } catch (e) {
      emitWatch('Install failed! ');
      return;
    }

    // Независимо от результатов инсталляции npm - плагин загружен
    ...
    const mes = `${appconfig.getMessage('Install')} ${pluginid}  => ${toTreeNode}`;
    emitWatch(mes);
