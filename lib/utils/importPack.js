...
module.exports = async function(packFolder, watchid, holder) {
  ...
  async function copyPlugin() {
    ...
    /*
    pluginid = hut.getFileNameExtLess(files[0]).toLowerCase();

    // Создать папку с именем плагина. Если существует - копировать поверх
    let folder = appconfig.getThePluginPath(pluginid);
    hut.checkAndMakeFolder(folder);

    hut.logMsg('Copy from ' + src + ' to  ' + folder, 'UPLOAD');
    return wu.cpP({ src, dest: folder });
  })
  .then(() => {
    // Удалить исходный zip и src
    hut.logMsg('Delete sources' , 'UPLOAD');

    if (hut.delFileSync(zipfile)) {
      hut.logMsg('Delete file '+ zipfile,  'UPLOAD');
    } else  hut.logMsg('ERR Delete file '+ zipfile,  'UPLOAD');

    if (hut.delFolderSync(tempdir)) {
      hut.logMsg('Delete folder '+ tempdir,  'UPLOAD');
    } else  hut.logMsg('ERR Delete folder '+ tempdir,  'UPLOAD');

    // Нужно считать версию из ih и перезагрузить плагин - здесь не делаю? Это первоначальная установка
    resolve({ 
      */
