...
module.exports = async function(packFolder, watchid, holder) {
  ...
  if (appconfig.isProjectPath(packFolder)) {
    ...
    // Если есть любой друглй файл .ih - это плагин
    await installPlugin();
  } else {
    await installPack();
  }
  ...
  async function installDbagent() {
    const info = appconfig.getDbagentInfo(packFolder); // Если при чтении ошибка - будет throw
    if (!info || !info.id) throw { message: 'File .ih missing or invalid!' };

    info.dbagent = true;
    await pluginutil.install(packFolder, info, tmes => emitWatch(tmes));

    // Если обновление dbagenta- нужно его перезапустить?

    // Если добавляется новый - пользователь сам должен активировать другую БД?
    // TODO Если не исп БД - можно сразу ставить??
    if (holder.unitSet.db) {
      if (holder.unitSet.db._id == info.id) {
        emitWatch('Update OK. Restart dbagent');
      } else {
        emitWatch('Install OK. You can activate dbagent ' + info.id + ' now.');
      }
    }
  }
