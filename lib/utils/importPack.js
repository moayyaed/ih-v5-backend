...
module.exports = async function(packFolder, watchid, holder) {
  ...
  async function installProject() {
    ...
    await addToTable('project', { _id, projectfolder: _id, parent: 'projectgroup', order: 100 });
    const toTreeNode = await getParentItemName(parent); // Название корневой папки, в которую поместили
    emitWatch('Install ' + _id + ' => ' + toTreeNode);
  }

  async function getParentItemName(parent) {
  ...
  async function add(elementType, elObj) {
    ...
    for (const item of elObj) {
      ...
      const toTreeNode = await getParentItemName(doc.parent); // Название корневой папки, в которую поместили
      emitWatch('Import ' + doc.name + ' => ' + toTreeNode);
    }
  }
