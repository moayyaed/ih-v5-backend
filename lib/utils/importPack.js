...
module.exports = async function(packFolder, watchid, holder) {
  ...
  async function installDbagent() {
    ...
    info.dbagent = true;
    await plugininstall.doInstall(packFolder, info, tmes => emitWatch(tmes));
    ...
    const doc = await dbagentutil.prepareDocAfterInstall(info, holder);
  ...
  async function installPlugin() {
    const doc = await plugininstall.installPlugin(packFolder, emitWatch, holder);
  ...
  async function afterPluginInstall(table, doc) {
    if (doc) {
      ...
      const name = doc.name || doc.plugin || doc._id;
      const mes = `${appconfig.getMessage('Install')} ${name}  => ${toTreeNode}`;
      emitWatch(mes);
    } else {
      // Нужно перезапустить??
      emitWatch('Update OK');
  ...
  async function installProject() {
    ...
    } else if (isV3Project(packFolder)) {
      v4 = true; // BERRY
      const mes ='Проект версии V3 (BERRY). Запущена процедура миграции...';
      emitWatch(mes);
      _id = projectutil.formNewProjectId(folderName + '_fromBERRY');
      await projectutil.transferProjectBerry(packFolder, _id, tmes => {
        emitWatch(tmes);
      });
    } else {
      v4 = true;
  ...
  function isV3Project(projpath) {
    return fs.existsSync(path.join(projpath, 'jbase', 'devview.json'));
  }
