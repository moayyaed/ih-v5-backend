...
/**
 * Формирование объекта для передачи dbagent-у
 *
...
async function getReportReadObj(qobj, holder) {
  let { id, start, end } = qobj;
    ...
    const columns = tabledata.transfromGenfieldObjToArray(rec.props)
    if (!columns || !columns.length) throw { message: 'No columns for report ' + id };

    // Одно устройство может быть неск раз!!
    const dn_propSet = new Set(); // Выбрать из столбцов названия устройств dn
    ...
    let readobj = {
      id,
      // dbtable: listRec.data_type == 'trend' ? '' : listRec.dbtable || '',
      dbtable: 'records',
      dn_prop,
      start, 
      end,
