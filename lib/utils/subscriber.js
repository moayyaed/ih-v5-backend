...
class Subscriber {
  ...
  // Получить подписки на событие event для всех клиентов (с проверкой параметров из paramObj, кроме filter)
...
function compareParam(paramObj, subobj) {
  ...
  for (const par of parArr) {
    if (par != 'filter' && subobj[par] != undefined && subobj[par] != paramObj[par]) return; // есть параметр, и он не совпадает
  }
  return true;
}
