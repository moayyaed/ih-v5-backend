 ...
 *      - словари для выбранного языка
 *      - перевод строки для выбранного языка
 */
...
module.exports = {
  ...
  getMessage(id) {
    ...
    console.log('getMessage dict.messages '+util.inspect(dict))
...
function getLangPath(lang) {
  return isLangExists(lang) ? config.langdir + '/' + lang : '';
}

function loadDicts(lang) {
  loadOneDict(lang, 'messages');
  loadOneDict(lang, 'pmmessages');
  loadOneDict(lang, 'month');
  loadOneDict(lang, 'daysofweek');
}

function loadOneDict(lang, dictname) {
...
function getConfig(appdir) {
  ...
  try {
    result = fut.readJsonFileSync(`${workpath}/config.json`);
  } catch (e) {
    result = config;
  }
  ...
  result.langdir = fut.checkFolder(path.join(appdir, 'locale'));
  result.syspath = path.join(appdir, '..'); // appdir = backend, sysdir = intrahouse
  result.sysbasepath = fut.checkFolder(path.join(appdir, 'sysbase')); // внутри папки backend
...
function specifyProjectPath() {
  ...
  if (config.project && config.indexOf('/') > 0) {
