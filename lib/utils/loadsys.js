...
function loadManifest(id) {
  ...
  // Если есть manifest.json - берем его, иначе plugin + '.json'
  let filename;
  if (id == 'logagent' || id == 'dbagent') { // config.agentspath
    filename = path.resolve(appconfig.get('agentspath'), 'sqlite', id+'.json'); // TODO - может быть другой агент!!!
    if (!fs.existsSync(filename)) return {};

  } else {
    filename = path.resolve(appconfig.getThePluginPath(plugin), 'manifest.json');
    if (!fs.existsSync(filename)) {
      filename = path.resolve(appconfig.getThePluginPath(plugin), plugin + '.json');
    }
  }
...
async function loadPluginSourceFile(plugin, file) {
  const filename = path.resolve(appconfig.getThePluginPath(plugin), file);
...
async function getCustombaseList() {
  ...
  return Array.isArray(files)
    ? files.filter(file => file.endsWith('.db')).map(file => file.substr(0, file.length - 3))
    : [];
}
