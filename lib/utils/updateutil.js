...
async function getLatest(url) {
  ...
  const cmp = compareSemVer(version, appconfig.get('version'));
  if (cmp) {
 ...
 */
async function updatePlugin(gitHubUrl, name) {
  let res;
  try {
    res = await getLatest(gitHubUrl);
    if (!res) {
  ...
  try {
    ...
    console.log('ERROR:  update ' + util.inspect(e));
    throw { message: 'Ошибка при закачке обновлений!' };
  }
}
