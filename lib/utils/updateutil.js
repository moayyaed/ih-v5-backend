 ...
 */
async function updateSystemPlugins() {
  const systemPlugins = [
    { name: 'sqlite', url: 'https://api.github.com/repos/intrahouseio/ih-dbagent-sqlite/releases/latest' }
  ];

  for (const item of systemPlugins) {
    try {
      await updatePlugin(item.name, item.url);
    } catch (e) {
...
async function getCurrentVersion(name) {
  ...
  switch (name) {
    case 'sqlite':
      info = await appconfig.getDbagentInfo(appconfig.getTheDbagentPath(name));
      break;
    default:
      info = await appconfig.getPluginInfo(name);
  }
  return info && info.version ? info.version : '';
}

async function updatePlugin(name, gitHubUrl) {
  let res;
  try {
    const curVer = await getCurrentVersion(name);
    if (curVer) {
      res = await getLatest(gitHubUrl, curVer);
      if (!res) {
        console.log('INFO: ' + gitHubUrl + ' - Latest OK');
        return;
      }
    }
  } catch (e) {
    console.log('ERROR:  updatePlugin:  ' + gitHubUrl + util.inspect(e));
    return;
  }
  ...
  try {
    ...
    const url = res.zipball_url;
    if (!url) throw { message: 'Expect zipball_url, get ' + util.inspect(res) };
  ...
  try {
    ...
    const backup = getBackupFolder(name + '_v' + res.newversion);
...
async function updateSystem() {
  ...
  try {
    ...
    const workpath = appconfig.get('workpath');
    const backup = getBackupFolder('v' + res.newversion);
    await wu.rsyncP({ src: tempdir, dest: workpath, backup, flags: ' -arc -v' });
    ...
    return { alert: 'info', message: 'Установлена версия ' + getVesionNumbers(res.tag_name), ok: 1, refresh: true };
  } catch (e) {
    console.log('ERROR:  updateSystem  ' + util.inspect(e));
    throw { message: 'Ошибка при закачке обновлений!' };
  }
