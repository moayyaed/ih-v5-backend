...
async function renameP(oldPath, newPath) {
...
/**
 * Чтение файлов из папки - только 1 уровень вложенности
 *    @param {String} folder
...
/**
 * Чтение файлов из папки - все уровни вложенности
 * Рекурсивно
 *    @param {String} startFolder
 *    @param {Object} opt: {ext:'json', dir:1, hidden: 1}
 *              .ext - выбрать файлы с расширением
 *              .dir - выбрать только папки
 *              .hidden -  брать скрытые файлы
 *              .exclude = ['node_modules']  исключать по списку
 *    @return {Array} - [
 *     {filename:'v5', parent: '', folder:1},
 *     {filename:'v5/formCommon', parent: 'v5', folder:0}]
 *
 *    При ошибке чтения возвращает пустой массив на любом уровне вложенности
 */
function readFolderSyncExt(startFolder, opt = {}) {
  const result = [];
  readOneFolderSyncExt('');
  return result;

  function readOneFolderSyncExt(innerFolder) {
    try {
      if (!startFolder) throw new Error('Start folder not defined!');
      const folder = innerFolder ? startFolder + '/' + innerFolder : startFolder;
      if (!fs.existsSync(folder)) throw new Error('Folder not found!');

      const files = fs.readdirSync(folder);
      for (const file of files) {
        if (isSuitable(file)) {
          const isDir = fs.statSync(folder + '/' + file).isDirectory();
          result.push({ filename: file, folder: isDir ? 1 : 0, parent: innerFolder });
          if (isDir) {
            readOneFolderSyncExt(innerFolder ? innerFolder+'/'+file : file);
          }
        }
      }
    } catch (e) {
      console.log('ERR: readOneFolderSyncExt ' + startFolder + '/' + innerFolder + '. ' + e.message);
    }
  }

  function isSuitable(file) {
    // исключить по списку
    if (opt.exclude && opt.exclude.includes(file)) return;

    // исключить скрытые
    if (!opt.hidden && file.startsWith('.')) return;

    // исключить по расширению
    if (opt.ext && !file.endsWith(opt.ext)) return;
    return true;
  }
}
