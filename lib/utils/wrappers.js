...
function installNodeModulesP(folder) {
  ...
  try {
    ...
    let needInstall;
    let dep = hut.readJsonFileSync(packagejson).dependencies;
    if (dep) {
...
function forkModule(module, args) {
  if (!fs.existsSync(module)) {
    hut.logErr('', 'Module not exists:'+module, 'WRAPPER');
    return;
  }

  let ps = fork(module, args);
  if (!ps) {
    hut.logErr('', 'Fork error!', module);
    return;
  }

  ps.on('close', code => {
    if (code) hut.logErr('', 'Exit code=' + code, module);
  });
}
