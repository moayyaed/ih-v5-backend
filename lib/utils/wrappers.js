 ...
 */
function rmP(folder) {
  // TODO - для  Windows команда другая?
  if (sysinfo.isWindows()) return Promise.resolve();

  try {
    if (!folder) throw { name: 'SoftError', message: 'No folder' };
    if (!fs.existsSync(folder)) throw { message: `File not found ${folder}` };
    const files = fs.readdirSync(folder);
    if (!files || !files.length) return Promise.resolve();
  } catch (err) {
    return Promise.reject({ message: 'Before run rm: ' + err.message });
  }

  const cmd = `rm -R  ${folder}/*`; 
  console.log('INFO: ' + cmd);
  return tryRunCmdP(cmd);
}
