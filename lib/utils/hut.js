...
exports.replaceLastWord = replaceLastWord;
exports.replaceBeforeChar = replaceBeforeChar;
...
exports.transfromFieldObjToArray = transfromFieldObjToArray;
exports.getFilteredObject = getFilteredObject;
...
exports.timeFormatHMS = timeFormatHMS;
...
exports.getNextHourly = getNextHourly;
...
exports.isSoundFile = isSoundFile;
...
exports.getNumberOrStr = getNumberOrStr;
...
exports.getVesionNumbers = getVesionNumbers;
...
/** Заменяет последнее слово в строке.
 *  Символы-разделители в конце строке отсекаются
 *  Если пробелов внутри нет - добавляется в конец
 *  @param  {String} astr
 *  @param  {String} word
 *  @return {String}
 */
function replaceLastWord(astr, word) {
  let str = allTrim(astr);
  const i = str.lastIndexOf(' ');
  return (i > 0 ? str.substr(0, i) : str) + ' ' + word;
}

/** Заменяет первые символы перед заданным
 *  Если заданного внутри нет - добавляется в начало
 *  @param  {String} astr
 *  @param  {String} word
 *  @param  {String} char
 *  @return {String}
 */
function replaceBeforeChar(astr, word, char) {
  if (!astr) return word + char;
  const i = astr.indexOf(char);
  return word + char + (i >= 0 ? astr.substr(i + 1) : astr);
}

function replaceAll(astr, search, replacement) {
  if (!astr) return '';
  return astr.split(search).join(replacement);
}
...
function merge(a, b) {
  return Object.entries(b).reduce((o, [key, val]) => {
    o[key] = val && typeof val === 'object' ? merge((o[key] = o[key] || (Array.isArray(val) ? [] : {})), val) : val;
    return o;
  }, a);
}
...
/**
 * Формирование массива из объекта, хранимого как одно поле в записи
 *
 * @param {Object || Array} pObj - {state:{name:'state', min:0, max:100}, value:{}}
 *                             или {"ss7fgh":{name:'state', min:0},:"bhgh2":{}}
 * @return  {Array} - [{_id:"state", id: "state", newid: "state", min:0,...}]
 */
function transfromFieldObjToArray(pObj) {
...
function getFilteredObject(obj, filter) {
  const result = {};
  Object.keys(obj).forEach(prop => {
    if (obj[prop] && isInFilter(obj[prop], filter)) {
...
function timeFormatHMS(sec, pref) {
  let minutes = Math.floor(sec / 60);
  let hours = Math.floor(minutes / 60);
  if (hours > 0) minutes -= hours * 60;
  ...
  return hours + pref[0] + ' ' + minutes + pref[1] + ' ' + seconds + pref[2];
}
...
/**
 * Возвращает временную метку след границы часа
 */
function getNextHourly() {
  let dt = new Date();
  let hh = dt.getHours();
  if (dt.getMinutes() == 59 && dt.getSeconds() == 59) hh += 1;
  return new Date(dt.getFullYear(), dt.getMonth(), dt.getDate(), hh, 59, 59, 0).getTime();
}
...
/**  На основании расширения определяет, является ли файл звуковым
 *
 *    @param  {String} filename
 *    @return {Bool} true - да
 */
function isSoundFile(filename) {
  return filename ? ['.wav', '.mp3', 'ogg'].some(item => filename.endsWith(item)) : false;
}
...
function getErrStrWoTrace(e) {
  ...
  if (str && str.indexOf('\n')) {
    ...
    for (const line of lines) {
      if (line && allTrim(line).startsWith('at')) return str;
      str += line + '\n';
    }
  }
...
/** Возвращает число, если может быть преобразовано в число иначе строку
 *
 */
function getNumberOrStr(val) {
