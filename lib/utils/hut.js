...
function getFunctionArguments(x) {
  ...
  // (device, prop, val, {timeout=5})
  return res[0].substr(1, res[0].length - 2).split(',');
}
...
console.log('getFunctionOptionArgument args='+util.inspect(args));

  const last = allTrim(args[args.length - 1]);
  console.log('getFunctionOptionArgument last='+util.inspect(last));

  if (!last || last.substr(0, 1) != '{' || last.length < 3) return;

  // {timeout=5, pulse=10, xxx}
  const arr = last.substr(1, last.length - 2).split(',');
  const opt = {};
  arr.forEach(el => {
    if (el) {
      if (el.indexOf('=') > 0) {
  ...
  console.log('getFunctionOptionArgument result='+util.inspect(opt));
