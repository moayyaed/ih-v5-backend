...
async function exec(query, holder) {
  ...
  async function exportOneType(item) {
    ...
    */
    ...
    if (doc.scriptOnBoot) hArr.push('_OnBoot');
...
async function copyFiles(arr, fromFolder, toFolder) {
  console.log('copyFiles arr='+util.inspect(arr))
  for (const name of arr) {
    try {
      await fs.promises.copyFile(path.join(fromFolder, name), path.join(toFolder, name));
    } catch (e) {
      console.log('ERROR: Export file ' +path.join(fromFolder, name)+' error: '+ util.inspect(e) + '. Skipped');
    }
  }
}
...
/**
 * Подготовка к выгрузке одного файла изображения или нескольких файлов внутри папки
 *
 * @param {Object} query
...
/**
 * Экспорт одного файла из дерева или нескольких файлов внутри папки
 *    (sound, ...)
 * @param {Object} query
 *   nodeid: имя файла () или id папки
 *   sourcePath: Директорий, содержащий файлы
 *   table: Имя таблицы, содержащей узел дерева (нужно взять название папки)
 *   treeName: Имя дерева
 * @param {Object} holder
 */
async function exportFilesFromTree(query, holder) {
  const {nodeid, sourcePath, table, treeName} = query;
  if (!nodeid) return { error: 'Expected nodeid for export!' };
  if (!sourcePath) return { error: 'Expected sourcePath for export!' };

  // Если это id файла - Выполнить экспорт 1 файла
  if (nodeid.indexOf('.')>0) {
    let name = path.join(sourcePath, nodeid);
    if (!fs.existsSync(name)) return { error: 'File for export not found: ' + name };
    return { folder: '', name };
  }

  try {
    // Это папка - выбрать файлы, переписать их во вспомогательную папку

    const {arr} = await getArrayFromTreeFolder({nodeid, table, treeName}, holder.dm) 
    ...
    // Копировать файлы
    await copyFiles(arr, sourcePath, tempFolder)
...
/**
 * Функция выбирает файлы из папки-узла дерева
 *  Возвращает массив id узлов и название папки
 * @param {Object} 
 *        nodeid - id папки-узла дерева
 *        table - имя таблицы, в которой можно найти папку по id (нужно название папки)
 *        
 * @param {*} dm 
 * @return {arr, folderName}
 */
async function getArrayFromTreeFolder({nodeid, table, treeName}, dm) {
  const doc = await dm.findRecordById(table, nodeid);
  if (!doc) throw { message: 'Not found ' + nodeid+' in table:'+table };

  const arr = await dm.getIdsFromTree(treeName, nodeid);
  console.log('getArrayFromTreeFolder nodeid='+nodeid+' arr='+util.inspect(arr))
  if (!arr || !arr.length) throw { message: doc.name + '. ' + appconfig.getMessage('EmptyFolder') };
  return {arr, folderName:doc.name}
}
...
async function exportPlugin(query, holder) {
  if (!query.nodeid) return { error: 'Expected nodeid for export!' };
  const plugin = query.nodeid;
  let folder = appconfig.getThePluginPath(plugin);

  if (!appconfig.isPlugin(folder)) return { error: 'Invalid plugin - not found file '+plugin+'.ih  in ' + folder };

  return { folder, name: plugin };
}
