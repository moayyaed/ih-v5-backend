...
async function prepareToExportImage(query) {
  ...
  if (hut.isImgFile(nodeid)) return { title: nodeid, filename: nodeid, url };
  ...
  const data = await dataformer.getImagegrid(nodeid);
  const imgArr = data ? data.data : ''; 
  ...
  return { title, url, filename: query.nodeid + '.zip' };
}
...
async function exportImage(query) {
  ...
  try {
    ...
    const imgArr = await dataformer.getImagegrid(nodeid);
