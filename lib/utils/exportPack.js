...
function formPackName( query, prefix) {
  prefix = prefix || appconfig.get('project_prefix');
  return prefix + '@' + query.nodeid;
}
...
async function prepareToExport(query, holder) {
  ...
  const filename = formPackName(query)+'.ihpack';
