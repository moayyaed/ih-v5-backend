...
class Resourcemate {
  ...
  async start() {
    ...
    console.log('WARN: revise projectspath ' + projectspath);
    ...
    /*
    try {
      // Проверить, что есть системная папка imgsystem + noimage.svg
      const docs = [];
      const imgsystem = await this.dm.dbstore.findOne('images', { _id: 'imgsystem' });
      if (!imgsystem) {
        docs.push({ _id: 'imgsystem', parent: 'imagegroup', order: 10, list: 'imagegroup', name: 'System', folder: 1 });
      }

      const noimage = await this.dm.dbstore.findOne('images', { _id: 'noimage.svg' });
      if (!noimage) {
        docs.push({ _id: 'noimage.svg', parent: 'imgsystem', order: 10, name: 'noimage.svg' });
      }

      if (docs.length) await this.dm.dbstore.insert('images', docs);
    } catch (e) {
      console.log('ERROR: add  noimage.svg: ' + JSON.stringify(e));
    }
    */
