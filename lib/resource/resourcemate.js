...
class Resourcemate {
  ...
  async start() {
    ...
    try {
      // Проверить, что есть системная папка + noimage
      const imgsystem = await this.dm.dbstore.findOne('images', { _id: 'imgsystem' });
      if (!imgsystem) {
        // Добавить
        const docs = [
          { _id: 'imgsystem', parent: 'imagegroup', order: 10, list: 'imagegroup', name: 'System', folder: 1 },
          { _id: 'noimage.svg', parent: 'imgsystem', order: 10, name: 'noimage.svg' }
        ];

        await this.dm.dbstore.insert('images', docs);
      }
    } catch (e) {
