...
class Resourcemate {
  ...
  async start() {
    ...
    await this.dm.reviseTableWithFolder('project', async docs => projectutil.syncProjects(docs, projectspath));

    await this.dm.reviseTableWithFolder('image', async docs => resutil.syncImages(docs, this.folder));
    this.revising = false;
  ...
  async processWatchResult(eventType, filename) {
    ...
    if (!hut.isImgFile(filename)) return;
