...
const integrationmethods = require('../integration/integrationmethods');
...
class Subsmanager {
  ...
  async formDeviceSubSetForIntegration(unitId, id) {
    ...
    // выделить свойства устройства которые мапятся для интеграции
    let getMappedPropFun;
    if (integrationmethods[unitId] && integrationmethods[unitId].getDeviceMappedPropsArray) {
      getMappedPropFun = integrationmethods[unitId].getDeviceMappedPropsArray;
    }
    ...
    docs.forEach(doc => {
      ...
      if (getMappedPropFun) {
        const propArr = getMappedPropFun(unitId, doc, this.holder);
        propArr.forEach(prop => {
          setItem[doc.did][prop] = id;
        });
      } else {
        const prop = '*'; // любое свойство
        setItem[doc.did][prop] = id;
      }
    });

    // console.log('formDeviceSubSetForIntegration ' + util.inspect(this.unitSubSet.devices, null, 4));
  }
