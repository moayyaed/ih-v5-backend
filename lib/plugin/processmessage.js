...
module.exports = async function(m, uobj, holder) {
  ...
  switch (mes.type) {
    ...
    case 'archive':
        return processArchive(mes.data, uobj);
  ...
  async function getPersistent() {
    ...
    //     { id: 3, type: 'get', name: 'persistent', filter: '14:51:06:F4:AC:82' }
    // или { id: 3, type: 'get', name: 'persistent', filter: {file:'14:51:06:F4:AC:82'} }
    const { filter } = mes;
    let file;

    // const file = typeof filter == 'object' ? filter.file : filter;
    try {
      ...
      if (!file) throw { message: 'Expected file for type:getpersistent' };
      if (file == 'all') {
        data = await pluginutil.getAllFromPersistent(unitId);
      } else {
        data = await pluginutil.getFromPersistent(unitId, file);
      }
    } catch (e) {
      console.log('ERROR:  Failed setpersistent: ' + util.inspect(e) + ' Plugin message: ' + util.inspect(mes));
    }
    return data;
  }
  ...
  async function processSet() {
    try {
      const { name, data } = mes;
      if (!name) throw { message: 'Missing name.' };
      if (!data) throw { message: 'Missing data.' };
      switch (mes.name) {
        case 'persistent':
          return setPersistent(data);
        default:
          throw { message: 'Unknown name.' };
      }
    } catch (e) {
      return 'IH: Failed set. ' + e.message + ' Plugin message:' + util.inspect(mes);
    }
  }

  async function setPersistent(dataObj) {
    // Сохранить данные в постоянном хранилище плагина
    // { id: 4, type: 'set', name: 'persistent', data: {file:'77:51:06:F4:AC:77', data:'xxx'} }
    let res;
    try {
      const { file, data = '' } = dataObj;
      if (!file) throw { message: 'Expected file for type:setpersistent' };
      // if (data == undefined) throw { message: 'Expected data for type:setpersistent' };
      await pluginutil.saveToPersistent(unitId, file, data);

      res = 'IH: Save to persistent OK';
    } catch (e) {
      res = 'IH: Failed setpersistent: ' + util.inspect(e) + ' Plugin message: ' + util.inspect(mes);
      console.log('ERROR:  Failed setpersistent: ' + util.inspect(e) + ' Plugin message: ' + util.inspect(mes));
    }
    return res;
  }
  ...
  function processArchive(data) {
    if (!data) return;
    const arr = uobj.chano.readArchive(data, holder); // Если привязок нет - может ничего и не быть!!
    if (arr && arr.length) {
      // Нужно добавить dn
      arr.forEach(item => {
        const dobj = holder.devSet[item.did];
        if (dobj) item.dn = dobj.dn;
        item.val = item.value;
      })
      holder.emit('received:device:archive', arr);
    }
    // readObj =  { d0138: { temp: -41 } }
    return 'IH: get archive' + util.inspect(data) + '\nset ' + util.inspect(arr);
  }
  ...
  function procInfo(data) {
    ...
    if (unitId == 'p2p' && data.key && data.key.length > 1) {
      console.log('INFO: P2p has started: ' + data.key);
    }
    // return 'IH: get ' + util.inspect(data) + '\nset ' + util.inspect(readObj);
  }
  ...
  async function sendGetResponse() {
    if (mes.tablename) {
      ...
      const res = { id: mes.id, type: 'get', data: await getDataForName(mes.name), response: 1 };
      uobj.send(res);
    }
  }
  ...
  async function getDevices() {
    try {
      ...
      docs.forEach(doc => {
        ...
        if (dobj && dobj.type) {
          // добавить мин мах значения
          const minMaxObj = getMinMaxObj(dobj);
          arr.push({ ...doc, ...minMaxObj, type: dobj.type });
        }
      });
  ...
  function getMinMaxObj(dobj) {
    const res = {};
    const propArr = dobj.getPropsForHardlink();

    propArr.forEach(prop => {
      const min = dobj.getMin(prop);
      const max = dobj.getMax(prop);
  ...
  function receiveCommandToDo() {
    ...
    // {"type":"command","command":"device","did":"d0136","prop":"on"}
    ...
    if (mes.command && typeof mes.command == 'object') {
      ...
      res = 'do ' + res;
      commander.execCommand(sender, mes, holder);
    } else {
    ...
    const resObj = { id: mes.id, type: 'command', ...res };
    uobj.send(resObj);
