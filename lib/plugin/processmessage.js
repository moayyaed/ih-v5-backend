...
const receiveсhannels = require('./receivechannels');
...
module.exports = async function(m, uobj, holder) {
  ...
  async function receiveChannels(data) {
    ...
    try {
      ...
      const ro = await receiveсhannels(data, uobj, holder);
  ...
  async function getDevices() {
    ...
    // Для интеграций должен быть список плагинов??
    // Выбрать устройства из файла integrations
    /*
    if (integrationmethods[unitId]) {
      const res = await integrationmethods[unitId].getDevices(unitId, holder);
      // console.log('getDevices for ' + unitId + ' ' + util.inspect(res, null, 4));
      return res;
    }
    */
  }
