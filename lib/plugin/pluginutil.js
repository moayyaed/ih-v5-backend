...
async function getV5PluginTable(holder) {
  ...
  arr.forEach(item => {
    if (pluginMap.has(item.id)) {
      ...
      item.newversion = pitem.version;
      item.yurl = !!pitem.url; // Есть ссылка для скачивания
      item.status = item.newversion && item.version < pitem.version ? 2 : 1;
  ...
  for (const [id, item] of pluginMap) {
    const yurl = !!item.url;
    arr.push({ id, version: '', newversion: item.version, yurl, status: 3, description: item.description });
  }
  return arr;
}
...
async function installPlugin(packFolder, emitWatch, holder) {
  ...
  // const doc = await prepareUnitDocAfterInstall( pluginInfo, holder);
