...
async function prepareUnitDocAfterInstall(pluginid, holder) {
  // Считать манифест - он уже на месте. Грузим заново, из кэша не берем
  const filename = path.resolve(appconfig.getThePluginPath(pluginid), pluginid + '.json');
  const manifest = fut.readJsonFileSync(filename, true);
  const _id =  getUnitDocId(pluginid, manifest.single);

  const unitDoc = holder.dm.findRecordById('units', _id);
