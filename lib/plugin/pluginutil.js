...
function getPluginFiles(pluginName) {
  const res = [];
  try {
    const folder = appconfig.getThePluginPath(pluginName);
    const files = fut.readFolderSync(folder);
    if (files.length) {
      files.forEach(file => {
        if (!file.startsWith('.') && file != 'node_modules') {
          const item = {id: file, title: file, parent:0}
          if (fut.isDirectory(folder+'/'+file)) {
            item.folder = 1;
            // Собрать первую вложенность
          } else {
            item.component = 'formModuleEditor';
          }
          res.push(item);
        }
      })
    ...
    console.log('ERROR: getPluginFiles '+pluginName+' '+util.inspect(e));
  }
  return res;
}
