  ...
  start() {
    ...
    this.dm.on('updated:units', docs => {
      docs
        ...
        .forEach(doc => {
          if (doc.$set) {
            // TODO Включить только свойства, которые относятся к параметрам 
            const {_id, order, parent, plugin, suspend, active, ...params} = doc;
            this.engine.sendOnUnitSub('tableupdated', doc._id, { tablename: 'params', op: 'update' }, [params]);
