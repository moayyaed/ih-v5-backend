  ...
  */
  ...
  processMessage(m, unitId) {
    ...
    switch (mes.type) {
      ...
      case 'sub':
        result = this.doSub(mes.data, unitId);
        break;

      case 'unsub':
        result = this.doUnsub(mes.data, unitId);
        break;
      default:
  ...
  isModuleRunning(unitId) {
    return this.unitId && this.unitSet[unitId].ps;
  }
  ...
  sendOnUnitSub(event, unitId, paramObj, data) {
    ...
    subArr.forEach(subItem => {
      // Также возможно нужно фильтровать data?? с использованием  subItem.subobj.filter
