...
class Pluginengine {
  ...
  start() {
    ...
    this.holder.on('transferdata_in', ({uuid, unit, payload}) => {

      if (!uuid || !unit || !payload) return;
     
  
      // Отправить команду плагину - если он активен!!?
      if (this.isModuleRunning(unit)) {
       
        this.unitSet[unit].send({ type: 'transferdata', id: uuid, uuid, payload });
  ...
  async processmessageFromUnit(m, unitId) {
    ...
    if (m && m.type && m.type == 'transferdata') {
      console.log('WARN: transferdata '+util.inspect(m))
      this.houser.emit('transferdata_out', Object.assign({ unit:unitId }, m));
      return;
    }
  ...
  debug(unitId, msg) {
    console.log('DEBUG: '+unitId+' msg='+msg)
