...
class Pluginengine {
  ...
  async processmessageFromUnit(m, unitId) {
    ...
    if (m && m.type && m.type == 'scan') {
      // Возможно, здесь надо кешировать для одновременных запросов?? mqtt?
      this.holder.emit('scan', Object.assign({ unit: unitId }, m));
      this.unitSet[unitId].scanResult = m.data;
      return;
    }
  ...
  moduleOnClose(unitId, code) {
    ...
    if (uobj.sigterm) {
      ...
      errStr = uobj.exitError ? uobj.exitError : 'Plugin exit with code ' + code;
      uobj.exitError = '';
      this.logUnit(unitId, errStr, 1);
    }
    uobj.ps = '';
  ...
  scanPlugin(unitId, params) {
    ...
    // Это ответ - обработка в processMessage?
    // this.holder.emit('scan', { uuid: params.uuid, unit:unitId });
  }
}
