...
class Pluginengine {
  ...
  start() {
    ...
    this.holder.on('finish', () => {
      ...
      Object.keys(this.unitSet).forEach(unit => this.unitSet[unit].sendSigterm());
    });
  }
  ...
  forkModule(unitId, modulepath, args) {
    ...
    if (uobj.connector) {
      ...
      ps.on('message', async m => {
        // this.processMessage(m, unitId); // Вынести в отдельный модуль, передавать туда uobj

        const result = await processmessage(m, uobj, this.holder); // Вынести в отдельный модуль, передавать туда uobj
        if (result) this.logUnit(unitId, result);
      });
    }
  }
  ...
  debugctl(mode, unitId) {
    console.log('EE debugctl ' + unitId);
    if (!this.unitSet[unitId]) return;
    const uobj = this.unitSet[unitId];
    uobj.debug = mode; // 0 || 1
    if (mode) {
