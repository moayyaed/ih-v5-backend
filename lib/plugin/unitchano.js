  ...
  */
  ...
  getChannels() {
    return this.smart
      ...
      : this.charr.filter(item => !item.folder && item.r).map(item => Object.assign({ id: item.chan }, item));
  }
  ...
  getChannelsForSmart() {
    ...
    return this.charr
      ...
      .map(item => {
        const { _id, folder, parent, did, prop, order, title, chan, ...robj } = item;
        if (folder) return { id: _id, ...robj };

        // Сформировать  actions если есть action_props
        if (this.action_props) {
          const action = item.w ? {} : '';
          const obj = {parentid: parent};
          // console.log(chan+' 1-OBJ='+util.inspect(obj)+' item='+util.inspect(item))
          Object.keys(robj).forEach(xprop => {
            if (this.action_props[xprop]) {
              if (action) action[xprop] = robj[xprop];
            } else {
              obj[xprop] = robj[xprop];
            }
          });
          obj.dn = chan;
          if (!hut.isObjIdle(action)) obj.actions = [action];
          // console.log(chan+' 2-OBJ='+util.inspect(obj))
          return obj;
        }

        return { ...robj, parentid: parent, dn: chan };
      });
  }
  ...
  getWriteObj(chan, value) {
    ...
    if (this.smart) {
      ...
      return {dn:item.chan, prop: item.act || 'set', val:value};
    }

    item.id = item.chan;
    if (item.pubtopic) {
      item.topic = item.pubtopic;
    }
    if (item.pubmessage) {
      item.message = item.pubmessage;
    }
