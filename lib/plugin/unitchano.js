...
class Unitchano {
  ...
  getChannelsCommon() {
    ...
    const res = this.charr.filter(item => !item.folder && item.r).map(item => Object.assign({}, item, { id: item.chanId }));
    return res;
  }
  ...
  formReadWriteMap() {
    ...
    this.charr.forEach(item => {
      ...
      if (item.r) {
        item.r = 1;
        if (item.calc) item.calcfn = this.makeFun(item.calc, item.Id);
        this.readMap.set(item.chanId, item);
      }
      // else item.r = 0;

      if (item.w) {
        if (item.calc_out) item.calc_outfn = this.makeFun(item.calc_out, item.chanId);
  ...
  getWriteObj(chanId, value, testValue) {
  ...
  addShareFields() {
    ...
    // Для всех каналов с parent папкой этого типа
    // - добавлять поля из папки по списку this.share_node_folder_fields - д б массив
    const nodeFolders = this.charr.filter(item => item.folder && item.foldertype == 'node');
    if (!nodeFolders.length) return;

    const nodeFolderObj = hut.arrayToObject(nodeFolders, '_id'); 
    this.charr.forEach(item => {
      if (item.parent &&  nodeFolderObj[item.parent]) {
        this.share_node_folder_fields.forEach(field => {
