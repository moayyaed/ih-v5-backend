...
class Unitchano {
  ...
  readData(data) {
    ...
    data.forEach(item => {
      ...
      // Если есть привязка
      if ((item.value != undefined || item.chstatus != undefined) && this.readMap.has(id) && this.readMap.get(id).did) {
   ...
   /** readArchive
   * Чтение архивных данных каналов
   *  - прогнать через маппинг, если есть привязка к устройству
   *     - если есть обработка - обработать (если есть привязка?)
   *     - вернуть массив значений относительно устройства
   *
   * @param {Array}  [{id: id_канала, value:плоское значение, ts},..]
   * @return {Object} {DD1:{value:1, ts}}
   *
   */
  readArchive(data) {
    if (!data || !Array.isArray(data)) return;
    ...
    data.forEach(item => {
      ...
      if ((item.value != undefined) && this.readMap.has(id) && this.readMap.get(id).did) {
        ...
        res.push({did:linkobj.did, prop:linkobj.prop, value:item.value, ts: item.ts}) 
      }
    });
    return res;
  }
