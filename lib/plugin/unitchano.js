...
class Unitchano {
 ...
 getChannels() {
    return this.smart
      ...
      : this.getChannelsCommon()
      // : this.charr.filter(item => !item.folder && item.r).map(item => Object.assign({}, item, { id: item.chan }));
  }

  getChannelsCommon() {
    // Если просто без папок
    // if (this.share) 
    return this.charr.filter(item => !item.folder && item.r).map(item => Object.assign({}, item, { id: item.chan }));
  }
