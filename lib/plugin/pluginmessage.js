 ...
 */
function process(mes, uobj, houser) {
    /*
  const unit = uobj.id;
  ...
  if (typeof mes == 'object') {
    switch (mes.type) {
      ...
      case 'data':
        return receiveData(mes.data);

      case 'histdata':
        return processHistdata();

      case 'transferdata':
        houser.emit('transferdata_out', Object.assign({ unit }, mes));
        return;

      case 'startscene':
        if (mes.id) houser.emit('startscene', mes.id, mes.arg || '');
        return 'IH: startscene ' + mes.id;

      case 'log':
      case 'debug':
        return unit + ': ' + (typeof mes.txt == 'object' ? JSON.stringify(mes.txt) : mes.txt);
  ...
  function getDataForName(name) {
    switch (name) {
      ...
      case 'devhardlinks':
        return virtdata.getVirtData('devhardlinks', { filter: { unit } }, houser);
      ...
      default:
        return virtdata.isVirtData(name) ? virtdata.getVirtData(name, { filter }, houser) : jdb.get({ name, filter });
    }
  }
  ...
  function getDevprops() {
    ...
    return Object.keys(houser.devSet)
      .filter(dn => !filter || hut.isInFilter(houser.devSet[dn], filter))
      .map(dn => (props ? houser.devSet[dn].getCurrentPropValues(props) : houser.devSet[dn].getCurrent(true))); // вернуть props v4
  }

  function getDevicesV4() {
    return Object.keys(houser.devSet)
      .filter(dn => !filter || hut.isInFilter(houser.devSet[dn], filter))
      .map(dn => Object.assign(houser.devSet[dn].getDeviceV4(), houser.devSet[dn].getCurrent(true)));
  }

  function getModuleChannels() {
    return uobj.smart
      ? jdb.get({ name: 'devhard', filter: { unit } })
      : channels.getBindChannels(uobj, houser.pp.getUnitChannels(unit), houser);
  }
  ...
  function getSystemParams() {
    return {
      port: Number(houser.getConfig('port')),
  ...
  function receiveChannels(data) {
    ...
    if (values.length) channels.saveCurrentChannelsValue(uobj, values, houser);
    ...
    if (newchan && newchan.length > 0 && uobj.gendevices) {
      gendev.genLinkedDevices(newchan, uobj, houser);
    ...
    setTimeout(() => {
      ...
      if (values.length) {
        const readObj = uobj.readData(values, houser);
        houser.setDevPropsFromUnit(readObj);
      }
    }, 500);
    return 'IH: ' + res;
  }
  ...
  // //////////////////////////////////////////////////////////////////////
  // Обработка запросов type:data
  function receiveData(data) {
    if (data) {
      channels.saveCurrentChannelsValue(uobj, data, houser);
      const readObj = uobj.readData(data, houser);
      houser.setDevPropsFromUnit(readObj);
      return 'IH: get ' + util.inspect(data) + '\nset ' + util.inspect(readObj);
      // return 'IH: get '+util.inspect(data)+'\nset ' + util.inspect(readObj)+'\n chanVals='+util.inspect(houser.chanVals[unit]);
      // return 'IH: set ' + JSON.stringify(readObj);
    }
  }
  ...
  function processHistdata() {
    ...
    houser.emit('histdata', uobj.readHistData(mes.data, houser));
  }
  ...
  function receiveCommandResponse() {
    ...
    try {
      ...
      if (mes.clid && mes.send && util.isArray(mes.send)) {
        houser.emit('sendclid', mes.clid, mes.send);
  ...
  function receiveCommandToDo() {
    ...
    if (mes.command && typeof mes.command == 'object') {
      ...
      if (mes.command.filter) {
        ...
        res = 'do ' + res;
        commander.doCommands(mes.command, sender, houser);
      }
    }
    return 'IH: ' + res;
  }
