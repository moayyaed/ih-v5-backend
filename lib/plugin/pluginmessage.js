...
module.exports = function (mes, uobj, holder) {
  console.log('PLUGIN MESSAGE '+util.inspect(mes));
  ...
  function getModuleChannels() {
    return uobj.charr.map(item => Object.assign({id:item.chan}, item));
  }
  ...
  function receiveData(data) {
    if (data) {
      channels.saveCurrentChannelsValue(uobj, data, holder);
      const readObj = uobj.readData(data, holder);
      holder.setDevPropsFromUnit(readObj);
      return 'IH: get ' + util.inspect(data) + '\nset ' + util.inspect(readObj);
      // return 'IH: get '+util.inspect(data)+'\nset ' + util.inspect(readObj)+'\n chanVals='+util.inspect(holder.chanVals[unit]);
