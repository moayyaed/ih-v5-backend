 ...
 */
module.exports = async function (mes, uobj, holder) {
 

  const unit = uobj.id;
  const plugin = uobj.plugin;
  console.log('FROM PLUGIN '+unit+' MESSAGE: '+util.inspect(mes));
  ...
  async function sendGetResponse() {
    if (mes.tablename) {
      ...
      uobj.send({ id, type: 'get', data: await getDataForName(mes.name), response: 1 });
    }
  }
  ...
  async function getModuleParams() {
    let result = {};
    const doc = await dm.dbstore.findOne('units', { _id:unit });
