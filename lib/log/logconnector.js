...
const logFields = {
  ...
  pluginlog: { unit: '', txt: '', level:0, sender: '' },
  authlog: { tags: '', location: '', txt: '', level:0,  sender: '' },
  mainlog: { tags: '', location: '', txt: '', level:0,  sender: '' }
};
...
module.exports = {
  ...
  async getLog(table, filter = {}, opt = {}) {
    ...
    const sql = logutil.fromReadSql(table, filter, opt);
  ...
  delete(table, filter) {
    const sql = logutil.formDeleteSql(table, filter);
