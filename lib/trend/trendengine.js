...
class Trendengine {
  constructor(holder) {
    ...
    holder.timelineSet = this.timelineSet;
  ...
  start() {
    ...
    this.holder.on('received:device:archive', data => {
      // Пока просто сохранить
      if (data.length) dbconnector.write(data);
    });
  ...
  addItem(doc, onlyTrend) {
    ...
    const id = getTrendId(doc.dn, doc.prop);
  ...
  onAcceptDeviceData(data) {
    ...
    data.forEach(item => {
      ...
      const trendId = getTrendId(item.dn, item.prop);
  ...
  processTimeline(data) {
    ...
    data.forEach(item => {
      ...
      if (this.timelineSet[trendId]) {
        ...
        const { wriObj, curObj } = this.timelineSet[trendId].onChange(item.value, item.ts);
        if (wriObj) {
          toWriteDb.push(wriObj);
        }

        if (curObj) {
          toWriteCurrent.push(curObj);
        }
      }
    });

    this.saveTimeline(toWriteDb, toWriteCurrent);
    // if (toWriteDb.length) dbconnector.write(toWriteDb, 'timeline', ['dn', 'prop', 'start', 'end', 'state']); //
    // if (toWriteCurrent.length) this.dm.upsertDocs('timelinecurrent', toWriteCurrent);
  }

  saveTimeline(toWriteDB, toWriteCurrent) {
    try {
      if (toWriteDB.length > 0) {
        dbconnector.write(toWriteDB, 'timeline', ['dn', 'prop', 'start', 'end', 'state']); //
      }
      if (toWriteCurrent.length) {
        this.dm.upsertDocs('timelinecurrent', toWriteCurrent);
      }
    } catch (e) {
      console.log(
        'ERROR: trendengine.saveTimeline toWriteDB=' +
          util.inspect(toWriteDB) +
          ' toWriteCurrent=' +
          util.inspect(toWriteCurrent) +
          ' err:' +
          util.inspect(e)
      );
    }
