...
async function processMessage(client, message, holder) {
    ...
    switch (mes.type) {
      ...
      case 'get':
        if (mes.route) {
          ...
          if (mes.uuid && mes.sub && mes.name == 'devices') {
            client.subs.set(mes.uuid, { type: 'mobile_devices' });
            console.log('SUB FOR MOBILE: ' + util.inspect(client.subs));
          }
        }
        break;
      ...
      case 'action':
        ...
        if (res && res.error) throw { message: res.error };
        break;
  ...
  async function devicesettingsV5(route) {
    ...
    const dobj = holder.dnSet[dn];
    if (!dobj) throw { message: 'Device not found:' + dn };
    return {data: widgetdata.getMobileWidgetdata(dobj, holder)};
  }
  ...
  async function getResult(name) {
    ...
    switch (name) {
      ...
      case 'scenes':
        // result.data = holder.dm.getFromCache('mobile_scenes').data;
        result.data = await getScenes();
        break;
