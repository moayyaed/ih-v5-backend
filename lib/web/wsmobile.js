...
async function processMessage(client, message, holder) {
  ...
  async function updatePnsKeys() {
    ...
    // client.user.login ??
    console.log('updatePnsKeys client.user='+util.inspect(client.user))
    ...
    if (docs.length) {
      ...
      await holder.dm.dbstore.update('infoaddr', { infotype: 'pushnotification', hwid }, { $set: { addr: token } }, { multi: true });
      docs.forEach(doc => {
  ...
  async function authResult() {
    console.log('authResult mes='+util.inspect(mes, null, 4))
    ...
    console.log('authResult client.token='+util.inspect(client.token))
    return result;
  }
