...
async function processMessage(client, message, holder) {
  ...
  async function updatePnsKeys() {
    /*
    const { login, hwid, token, model } = mes;
  
    if (!login) throw { message: 'Expected login for type:register' };
    ...
    if (jdb.get({ name: 'infoaddr', filter: { infotype: 'pushnotification', hwid } }).length > 0) {
      jdb.replaceAll({ name: 'infoaddr', filter: { infotype: 'pushnotification', hwid }, replace: { addr: token } });
    }
  
    const arr = jdb.get({ name: 'infoaddr', filter: { infotype: 'pushnotification', user: userId, hwid } });
  
    // Если нет еще записи для этой учетной записи - добавить 
    return arr && arr.length
      ? Promise.resolve()
      : jdb.addP('infoaddr', [{ infotype: 'pushnotification', user: userId, group: user.group, hwid, addr: token, allowed: 1, text: '', sign: '', model }]);
    */
  }

  /*
   function getMtime() {
      if (mes.name == 'pushnotifications') {
        const pnarr = jdb.get({ name: 'pushnotifications', filter: { dest: clients[clid].user.login } });
        return pnarr && pnarr.length > 0 ? pnarr[pnarr.length - 1].ts : 0;
      }

      return jdb.getTsForReading(mes.name) || Date.now();
    }
    */
  ...
  async function getActionResult() {
    ...
    if (prop && value != undefined) {
      ...
      result = commander.execSet(sender, { did: dobj._id, prop, value }, holder);
      return result;
    }
  }
