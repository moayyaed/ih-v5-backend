...
module.exports = function(holder) {
  return async (req, res, next) => {
    ...
    async function adminResponse(user) {
      ...
      if (!alevel) throw { message: appconfig.getMessage('ACCESSDENIED') };

      if (alevel < 2) {
        if (!holder.am.isPMQueryAllowed(query, user._id))
          throw { message: appconfig.getMessage('ACCESSDENIED') };

        // POST - это редактирование - проверить флаг pm_readwrite
        if (req.method != 'GET') {
          if (!holder.am.hasPMWritePermission(user._id))
...
async function authResult(query, headers, holder) {
  const res = {};
  // const {username, password, token} = headers;
  const { username, password, method } = query;
