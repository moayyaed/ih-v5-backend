...
module.exports = function(holder) {
  return async (req, res, next) => {
    ...
    try {
      ...
      const dataObj = query.method == 'auth' ? await authResult(req.headers) : await getDataObj(part1);
    ...
    async function getDataObj(part) {
      ...
      switch (part) {
        case 'admin':
        case 'user':
          return (await req.method) == 'GET' ? getApi(query, user, holder) : postApi(query, user, holder);
...
async function authResult(headers) {
  const res = {};
