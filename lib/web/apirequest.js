...
async function getApi(query, token, holder) {
  ...
  if (data) return data; // Удалось считать из кэша
  ...
  switch (method) {
    case 'get':
      ...
      return spec.includes(type) ? getSpec(query) : getapi.get(query, holder);
