...
module.exports = function(holder) {
  return async (req, res, next) => {
    ...
    try {
      ...
      if (part1 == 'export') return doExport(res, query);
...
async function getApi(query, user, holder) {
  if (query.method == 'export') return exportPack.prepareToExport(query, user, holder);
...
function getUserData(user) {
  ...
  return { layout: user.mainlay, name: user.name, savewarning };
}

async function doExport(res, query) {
  // Подготовить файл для выгрузки
  // const { folder, name, error } = query.param == 'project' ? await exportProject(query) : await exportPack.exec(query);
  const { folder, name, error } = await prepare(query);
  ...
  try {
    ...
    if (folder) {
      // Упаковать папку
      filepath = path.resolve(folder, '..', name + '.ihpack');
      console.log('Folder ' + folder + 'zip to ' + filepath);
      await wu.zipP({ src: folder, dest: filepath });
    } else {
      // Отправить один файл - image
      filepath = name;
    }
    res.download(filepath);

    // streamZip(res, folderToZip);
  } catch (e) {
...
async function prepare(query) {
  switch (query.param) {
    case 'image':  return exportImage(query);
    case 'project':  return exportProject(query);
    default: return  exportPack.exec(query);
  } 
}
...
async function exportProject(query) {
  ...
  // let name = appconfig.getTheProjectProp(project, '');
  // if (!name) appconfig.setTheProjectProp(project, 'name', project);
