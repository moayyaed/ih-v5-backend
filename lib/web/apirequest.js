    ...
    async function getDataObj(part) {
      ...
      switch (part) {
        ...
        case 'import': // только для ТЕСТА!!!
          return { data: await importPack(query.folder) };
...
async function doExport(res, query) {
  ...
  const { folder, name, error } = await exportPack.exec(query);
  if (folder && name) {
    // Упаковать
    // const filepath = appconfig.get('worktemppath') + '/templatePack.zip';
    ...
    res.status(404).send('ExportPack error: ' + error);
  }
}
