...
module.exports = function(holder) {
  return async (req, res, next) => {
    ...
    async function getDataObj(part) {
      ...
      // К query добавить userId = user._id
...
function getUserData(user) {
  ...
  return { layout: user.mainlay, name: user.name, savewarning, lang: appconfig.get('lang'), project: appconfig.get('project_title'), conf: appconfig.get('conf') || 0 };
}

async function doExport(res, query, holder) {
  // Подготовить файл для выгрузки
  const { folder, name, error } = await holder.dm.datagetter.exportOne(query, holder);
