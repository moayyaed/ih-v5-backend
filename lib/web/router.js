...
async function processApi(body, token) {
  ...
  switch (method) {
    ...
    case 'copyto':
        if (!type) throw { error: 'ERR', message: 'Type not defined for method:copyto!' };
        if (!id) throw { error: 'ERR', message: 'Id not defined for method:copyto!' };
  
        return copyToResult(body, token);
...
async function copyToResult(body, token) {
  // Проверить token!!
