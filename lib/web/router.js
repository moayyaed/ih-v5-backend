...
async function processApi(body, token) {
  ...
  switch (method) {
    ...
    case 'getdefault':
        if (!type) throw { error: 'ERR', message: 'Type not defined for method:getdefault!' };
        if (!id) throw { error: 'ERR', message: 'Id not defined for method:getdefault!' };
  
        return getDefaultResult(body, token);  
...
async function getDefaultResult(body, token) {
  const { type, id, nodeid } = body;

  // Проверить token!!
  return dm.getDefault(type, id, nodeid);
}
