...
const postapi = require('../dbs/postapi');
...
async function processGetApi(query, token) {
  ...
  switch (method) {
    case 'get':
      ...
      return spec.includes(type) ? dataformer.getByQuery(query) : dataformer.get(type, id, nodeid);
      // return dataformer.getByQuery(query);

    case 'getmeta':
      return dataformer.getMeta(type, id, nodeid);
    
    case 'clear':
      return postapi.link.clear(query, dm); 
