...
module.exports = function(holder) {
  return async (req, res) => {
    ...
    try {
      ...
      console.log('UPLOAD ' + body.param);
      try {
        param = JSON.parse(body.param);
      } catch (e) {
        throw { error: 'BADREQ', message: 'Unexpected param ' + body.param + util.inspect(e) };
      }

      if (typeof param == 'object') {
        if (!param.format) throw { error: 'BADREQ', message: 'Expected param:{format, content}' };
        if (param.format != 'csv') throw { error: 'BADREQ', message: 'Unexpected format ' + param.format }; // Пока только csv
        ...
        const resObj = await holder.dm.datamaker.importDocs({ ...param, nodeid: body.previd, data }, holder);
        res.json(resObj);
