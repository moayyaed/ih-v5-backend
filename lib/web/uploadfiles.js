...
module.exports = function(holder) {
  return async (req, res) => {
    ...
    function getFiles() {
      ...
      if (!req.files.files || typeof req.files.files != 'object') throw { error: 'BADREQ', message: 'Expected files.files for upload!' };
    ...
    function saveFiles() {
      ...
      files.forEach(file => {
        ...
        if (body.replace || !fileExists) {
          arr.push(saveOne(file, fileExists));
        }
      });
      return arr;
    }

    async function saveOne(file, fileExists) {
      try {
