...
module.exports = function(holder) {
  return async (req, res) => {
    ...
    // TODO Удалить исходные файлы и папки
    // Результат только логируется
    if (toDelete.length) {
      try {
        for (const pathToDelete of toDelete) {
          console.log('WARN: remove ' + pathToDelete);
          await fs.promises.rm(pathToDelete, { force: true, recursive: true });
        }
      } catch (e) {
        console.log('ERROR: Remove Failed !' + util.inspect(e));
      }
    }
    ...
    async function selectByParam(arr, param) {
      switch (param) {
        case 'image':
