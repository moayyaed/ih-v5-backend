  ...
  return async (req, res) => {
    ...
    try {
      ...
      if (files[0].mimetype == 'application/zip') {
        ...
        if (!sourceFolder) throw { message: 'Empty zip archive!' };
      }
      ...
      if (body.param.indexOf(':') > 0) {
        ...
        if (!importmethods[format] || !importmethods[format][param])
          throw { error: 'BADREQ', message: 'Unexpected format ' + format }; // Пока только csv
    ...
    async function formPromises(param, sFolder) {
      console.log('formPromises ' + param + ' sFolder=' + sFolder);
      if (sFolder) {
        ...
        console.log('formPromises arr=' + util.inspect(arr));
        ...
        console.log('formPromises selectedFiles=' + util.inspect(selectedFiles));
    ...
    async function extractFromZip(file) {
      try {
        ...
        return fileCount > 0 ? tempdir : innerfolder ? tempdir + '/' + innerfolder : '';
