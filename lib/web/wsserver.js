...
const dataonsubscribe = require('../domain/dataonsubscribe');
const commander = require('../domain/commander');
const visscript = require('../domain/visscript');
...
exports.start = function(server, holder) {
  ...
  */
  ...
  holder.on('playsound', message => {
    // {txt, dest:'admin', sender:'scene_scen042'}
    // {txt, dest:['admin','u002'], sender:'scene_scen042'}

    const { txt, dest, sender } = message;
    if (!txt || !dest || typeof txt != 'string') return;

    const files = txt.split(/\s*,\s*/);
    const userIds = Array.isArray(dest) ? dest : [dest];

    Object.keys(clients).forEach(clid => {
      if (clients[clid].user && userIds.includes(clients[clid].user._id)) {
        send(
          clid,
          formServerMessage({
  ...
  */
      ...
      for (let clItem of clientArr) {
        if (clItem.subsItem) {
          ...
          if (logname == 'devicelog') {
            ...
            if (res ) {
              data = { data: res };
              const ts = logObj.data ? logObj.data[0].ts : '??' ;

              send(clItem.clid, formResponse({ uuid: clItem.uuid, widgettype: logname, ts }, data));
            }
            ...
            if (clItem.subsItem.ajarr) {
              ...
              if (data) {
                ...
                send(
                  clItem.clid,
                  formResponse({ uuid: clItem.uuid, widgettype: logname, sid: clItem.subsItem.sid }, data)
                );
              }
            }
          }
  ...
  async function doScandata(clid, mes) {
    // Клиент прислал данные по сканированию
    // { uuid, method:'scandata', type:'scan', params:{unit, ...}};
    ...
    if (mes.type == 'scan') {
      ...
      holder.emit('scan:plugin', mes.params.unit, { method: mes.method, id: uuid, uuid, ...mes.params });
    }
  }
...
function formResponse(inobj, outobj, err) {
  ...
  if (inobj && inobj.ts) robj.ts = inobj.ts;
  if (inobj && inobj.sid) robj.sid = inobj.sid;
  if (inobj && inobj.widgettype) robj.widgettype = inobj.widgettype;
