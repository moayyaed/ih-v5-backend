  ...
  */
  ...
  async function sendOnSub_Log(event, logObj) {
    ...
    for (let [key, clientArr] of curSubMap) {
      ...
      // clientArr - массив подписчиков на диалоги, key - id контента (dialog-а)
      // для каждого диалога - если в нем есть журнал:
      ...
      for (let clItem of clientArr) {
        if (clItem.subsItem) {
          const data = await dataonsubscribe.formDeviceLogOnSub(event, key, logObj, holder);
          if (data) {
