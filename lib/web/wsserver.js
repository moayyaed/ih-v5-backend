...
exports.start = function(server, holder) {
  ...
  */
  ...
  holder.on('changed:device:data', changed => {
    ...
    sendOnSub('layout', {}, changed); // Измененные устройства находятся внутри контейнера
  });
   ...
   * {"method":"action", "type":"command", "id":"d003", "command":"on", "uuid":"xyzjjk"}
   *  */
  function doAction(clid, mes) {
    console.log('doAction ' + util.inspect(mes));

    const dobj = holder.devSet[mes.id];
    let err = '';
    if (!dobj) {
      err = 'Not found device with id: ' + mes.id;
    } else if (mes.command) {
      if (!dobj.hasCommand(mes.command)) {
        err = 'No command "' + mes.command + '" for device: ' + dobj.dn + '  (' + dobj.name + ')';
      } else {
          // holder.emit('log', { did:mes.id, cmd:mes.command, src:'login:admin' }, 'devicelog');
          dobj.doCommand(mes.command, {src:'login:admin'});
      }
      
    } else {
      err = 'Expected command!';
    }
    send(clid, formResponse({ uuid: mes.uuid }, '', err));
  }
