...
exports.start = function(server, holder) {
  ...
  const interval = setInterval(() => {
    Object.keys(clients).forEach(clid => {
      if (clients[clid].subs.has('debug')) {
        const subsArr = clients[clid].subs.get('debug');
        subsArr.forEach(subsItem => {
  ...
  function sendOnSub(event, paramObj, changed) {
    ...
    curSubMap.forEach((clientArr, key) => {
      ...
      const data = formMessageOnSub(event, key, changed);
      if (data) {
        // Отправить сообщение каждому клиенту-подписчику
        clientArr.forEach(clid => {
          send(clid, formResponse({}, { data }));
        });
      }
    });
  }

  // @return Map: key=vc003:[cid1, cid2], key=vc007:[cid1, cid2, cid3]
  function getSubMap(event) {
    const res = new Map();
    Object.keys(clients).forEach(clid => {
      if (clients[clid].subs.has(event)) {
        const subArr = clients[clid].subs.get(event);
        if (subArr && Array.isArray(subArr)) {
          subArr.forEach(item => {
            if (!res.has(item)) res.set(item, []);
            res.get(item).push(clid);
          });
        }
      }
    });
  ...
  //
  function formMessageOnSub(event, id) {
    switch (event) {
      case 'container':
        return { template_1: { state1: 1 } };
      default:
    }
  }
