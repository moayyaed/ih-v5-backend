  ...
  */
  ...
  async function doAction(clid, mes) {
    ...
    if (!mes.type) {
      ...
      } else if (commander.hasUserRightsToUIAction(user,{ action:  mes.command, did: mes.did, prop: mes.command }, holder)) {
        // Проверить возможность выполнения команды
        res = await commander.execCommand(sender, mes, holder);
      } else res = { err: appconfig.getMessage('InsuffRightsForAction') };
    } else if (mes.type == 'devicecommand') {
      if (commander.hasUserRightsToUIAction(user,{ action: mes.type, did: mes.id, prop: mes.command }, holder)) {
