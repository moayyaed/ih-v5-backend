...
exports.start = function(server, holder) {
  ...
  */
  ...
  holder.on('debug', (uuid, message) => {
    console.log('WARN: DEBUG uuid=' + uuid + ' ' + message);
  ...
  holder.on('watch', watchObj => {
    console.log('WARN: on Watch' + util.inspect(watchObj));
    console.log('WARN: watchSet ' + util.inspect(watchSet));
  ...
  */
  ...
  holder.on('devicelog', logObj => {
    sendOnSub_Log('dialog', logObj);
    sendOnSub_Log('container', logObj);
    sendOnSub_Log('layout', logObj);
  });

  // Отправить по подписке
  async function sendOnSub_Log(event, logObj) {
    // console.log('WARN:  sendOnSub_Log ' + util.inspect(logObj));
    ...
    for (let [key, clientArr] of curSubMap) {
      // console.log('WARN:  sendOnSub_Log key=' +key + ' clientArr'+util.inspect(clientArr));
      // clientArr - массив подписчиков, key - id контента (dialog-а, ...

      for (let clItem of clientArr) {
        if (clItem.subsItem) {
          const data = await dataonsubscribe.formDeviceLogOnSub(event, key, logObj, clItem.subsItem.contextId);
          if (data) {
      ...
      if (paramObj.contextId) {
        ...
        for (let clItem of clientArr) {
          if (clItem.subsItem && clItem.subsItem.contextId) {
            data = await dataonsubscribe.formChangedForContext(changed, clItem.subsItem);
            if (data) {
              send(clItem.clid, formResponse({ uuid: clItem.uuid }, data));
            }
          }
          /*
            const dataByContext = dataonsubscribe.formChangedForContext(changed, clItem.subsItem.contextId);
            if (dataByContext) {
              send(clItem.clid, formResponse({ uuid: clItem.uuid }, { data: dataByContext }));
            }
            */
        }
        // });
  ...
  function doAction(clid, mes) {
    ...
    if (!mes.type) {
      ...
      res = commander.execCommand(clid, mes, holder);
    } else if (mes.type == 'devicecommand') {
      res = commander.execDeviceCommand(clid, { did: mes.id, prop: mes.command }, holder);
    } else if (mes.type == 'set') {
      // Установка значений
      res = commander.execSet(clid, mes, holder);
    } else {
