...
function formWhere(filter) {
  ...
  function getObjItem(prop, valObj) {
    ...
    switch (compOper) {  // $lt, $lte, $gt, $gte, $in, $nin??
      case '$lt': return getOneItem(prop, '<', compVal);
      case '$lte': return getOneItem(prop, '<=', compVal);
      case '$gt': return getOneItem(prop, '>', compVal);
      case '$gte': return getOneItem(prop, '>=', compVal);
      case '$in': return getInItems(prop, compVal);
      default: 
  ...
  function getInItems(prop, compVal) {
    if (!Array.isArray(compVal)) {
      console.log('ERROR: Logconnector: Expected array in filter exp for prop:'+util.inspect(compVal)+'. Skipped..')
      return;
    }
