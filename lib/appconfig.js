...
module.exports = {
  ...
  isPlugin(pluginpath) {
    return fs.existsSync(path.join(pluginpath, 'plugin.json'));
  },
...
function checkAndMakeProjectFolders() {
  ...
  const projectJson = fut.readJsonFileSync(`${config.projectpath}/project.json`, true);
  config.project_title = projectJson.title || config.project;
  config.project_version = projectJson.version || '5.0'; // Если не прописано - значит, старый
  config.project_prefix = projectJson.prefix || 'user';
  config.project_dbname = projectJson.dbname || '';
  /*
