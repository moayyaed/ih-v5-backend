...
const config = { name_service: 'ih-v5', lang: 'en', npm: '', header: 'intraHouse', oem: '' };
const dict = {};
let noimage = '';
let appcrypto;
...
module.exports = {
  ...
  */
  ...
  getRegistryState() {  
    if (!config.hwid) return 'TryAgain'; // Не успел рассчитаться ключ
  
    const filename = path.join(config.mainbasepath, 'registry.json');
    const regObj = fut.readJsonFileSync(filename, true);
   
    if (!regObj  || !regObj.email || !regObj.hwid) return 'NeedReg';
  ...
  setRegistry(registryObj) {
    if (!registryObj || !registryObj.email) return;
    const filename = path.join(config.mainbasepath, 'registry.json');
