...
module.exports = {
  ...
  getPluginPersistentStorePath(plugin) {
    const folder = path.join(config.mainbasepath, plugin);
    if (!fs.existsSync(folder)) {
      fut.checkAndMakeFolder(folder);
    }
    return folder;
  ...
  // Скрипты каналов хранятся в проекте в папке channelscripts
  // Для каждого экземпляра плагина создается папка <modbus1>/
  getChannelscriptFilepath(unit, nodeid) {
    const folder = path.join(config.channelscriptpath, unit);
    if (!fs.existsSync(folder)) fut.checkAndMakeFolder(folder);
    return path.join(folder, nodeid+'.js');
  },
  ...
  getIhFilePath(type, name) {
    ...
    const folder = this.getFolderForPlugin(type, name);
    return path.join(folder, type == 'dbagent' ? 'dbagent.ih' : name + '.ih');
  },

  getFolderForPlugin(type, name) {
    switch (type) {
      case 'dbagent': return path.join(config.agentspath, name);
      case 'tool': return path.join(config.toolspath, name);
      default: return path.join(config.pluginspath, name);
    }
  },
  ...
  getV5Plugins() {
    try {
      ...
      // Некоторые строки могут придти как null
  ...
  getToolModulePath(toolName, module = 'index.js') {
    const toolfolder =  path.join(config.toolspath, toolName);

    return path.join(toolfolder, module);
  },
  ...
  getChannelPropsFromV5Form(unit) {
    const fiedtypes = ['number', 'cb', 'input', 'droplist', 'text'];
    const excludefields = ['testValue'];
    ...
    try {
      ...
      const res = {};
      const pArray = data.grid.filter(gridItem => gridItem.table == 'devhard').map(gridItem => gridItem.id);
      pArray.forEach(p => {
        if (data[p] && Array.isArray(data[p])) {
          data[p].forEach(item => {
            if (item.prop && fiedtypes.includes(item.type) && !excludefields.includes(item.prop)) {
...
function getConfig(appdir, configdir) {
  ...
  result.toolspath = path.join(result.vardir, 'tools');
  fut.checkAndMakeFolder(result.toolspath);
...
function checkAndMakeProjectFolders(xconfig) {
  ...
  fut.checkAndMakeFolder(xconfig.jbasepath + '/report');
  ...
  xconfig.channelscriptpath = `${xconfig.projectpath}/channelscripts`;
  fut.checkAndMakeFolder(xconfig.channelscriptpath);
  ...
  */
  ...
  xconfig.soundpath = `${xconfig.projectpath}/sounds`;
  fut.checkAndMakeFolder(xconfig.soundpath);
