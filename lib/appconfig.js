...
module.exports = {
  ...
  getIndex(url) {
    let frontendPath = config.syspath+'/frontend';
    ...
    if (config.project_frontend && folder != 'admin') {
      ...
      folder = config.project_frontend;
    }
    const filename = frontendPath + '/'+folder+'/index.html';
    try {
  ...
  getRestapihandlerFilename(id) {
    return path.join(config.restapihandlerpath, id + '.js');
  },
  ...
  checkApiSymlink(appdir) {
    ...
    if (!fs.existsSync(to_main_node_modules_path)) {
      ...
      fs.symlink(target, to_main_node_modules_path, 'dir', err => {
        if (err) {
          console.log('ERROR: node_modules symlink  ' + to_main_node_modules_path + ' creation error: ' + util.inspect(err));
        }
      });
    }
...
function checkAndMakeProjectFolders(xconfig) {
  ...
  console.log('PFILE '+`${xconfig.projectpath}/project.json`+'   projectJson ='+util.inspect(projectJson))
