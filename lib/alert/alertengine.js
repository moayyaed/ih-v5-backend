...
class Alertengine {
  ...
  addAlert(aleData) {
    ...
    this.alertSet[_id] = { _id, ...aleData, tsStart, txt, longtxt, propTitle };
  ...
  async closeAlerts({ _id, owner }, all) {
    ...
    console.log('BEFORE removeDocs alerts '+util.inspect(docs));
    await this.holder.dm.removeDocs('alerts', docs);
    this.emitAlertsChanged('delete', docs.map(item => ({id:item._id})));
  }
