...
class Alertengine {
  constructor(holder) {
    ...
    holder.on('ack:alert', data => {
      console.log('ACK:ALERT')
      // Нужно: записать в devicelog, mainlog
      // Пытаться закрыть алерт(ы)
      // Генерировать событие по результату
      this.ackAlert(data);
    });

    holder.on('deack:alert', data => {
      console.log('DEACK:ALERT '+util.inspect(data))
      // Нужно: записать в devicelog, mainlog
      // Генерировать событие отказ от квитирования
