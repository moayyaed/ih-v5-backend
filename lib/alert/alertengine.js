...
class Alertengine {
  ...
  startAlert(aleData) {
    ...
    this.alertSet[_id] = { ...aleData, _id, tsStart, txt: aleData.txt + ' ' + aleData.devTitle };
    ...
    const longtxt = txt + ' ' + devTitle;
    const rowbutton = {title :'Квитировать', command : 'api_ack_alert', hide: false };
   

    this.emitAlertsChanged('add', this.chooseJournals(aleData), [
      { id: _id, ts: tsStart, tsStart, level, txt: longtxt, state: 1, rowbutton, _command: 'api_ack_alert' }
    ]);
  }
   ...
   */
  chooseJournals(inObj) {
    let { owner, tags, location } = inObj;
    ...
    this.alertJournals.forEach((filter, key) => {
      if (!filter.location && (!filter.tags || !filter.tags.length)) {
        // Нет фильтра - это общий журнал тревог
        ajournals.push(key);
      } else {
        ...
        if (arrTag && arrTag.length && filter.tags && filter.tags.length) {
          // Это массивы

          // byTag = arrTag.every(elem => filter.tags.includes(elem));
          byTag = filter.tags.every(elem => arrTag.includes(elem));
        }
  ...
  tryStopAlert(aleData) {
    ...
    if (this.ownerMap.has(owner)) {
      ...
      if (this.alertSet[aid]) {
        ...
        const normtxt = alertutil.formNormTxt(aleData, aleObj.devTitle);
  ...
  async ackAlert(aleData) {
    ...
    if (this.alertSet[_id]) {
      ...
      this.alertSet[_id].username = username;
    }
    ...
    if (canClose) {
      ...
      // this.emitAlertsChanged('update', this.chooseJournals(aleData), [{ id: _id, tsAck, userId, username, _command:'api_deack_alert' }]);
      // В UI (по подписке) снять подтверждение нельзя, только через PM
      const resObj = { id: _id, tsAck, userId, username, rowbutton: { title: 'Hide', hide: true } };
      this.emitAlertsChanged('update', this.chooseJournals(aleData), [resObj]);
    }
  }
  ...
  async deackAlert(aleData) {
    ...
    if (this.alertSet[_id]) {
      ...
      this.alertSet[_id].username = '';
    }

    // Нужно фиксировать эту операцию в журналах
    alertutil.addLogs(aleData, 'deack', Date.now());

    this.emitAlertsChanged('update', this.chooseJournals(aleData), [
      { id: _id, tsAck: 0, userId: '', username: '', _command: 'api_ack_alert' }
    ]);
  }
