...
class Alertengine {
  ...
  addAlert(aleData) {
    ...
    this.holder.dm.insertDocs('alerts', [
      { _id, owner, toClose, did, prop, propTitle, txt: longtxt, level, tsStart, tags, location, ajarr }
    ]);

    // также нужно записать в журналы
    // console.log('BEFORE devicelogger did='+did+' txt='+txt)
    devicelogger.addLog(did, { did, prop, ts: tsStart, txt, level, sender:'alert' });
    // console.log('ADD MAINLOG '+util.inspect({ did, prop, ts: tsStart, txt: longtxt, level, tags, location }))
    logconnector.addLog('mainlog', { did, prop, ts: tsStart, txt: longtxt, level, tags, location, sender:'alert' });
  ...
  // stop может быть по разным причинам Отправить, у себя сразу удалить, ибо не активный
  async stopAlert(aid, reason) {
    if (!this.alertSet[aid]) {
    ...
    const normtxt = formNormTxt(aleObj);

    // Записать в журналы
    const { did, prop, tags, location } = aleObj;
    const dobj = this.holder.devSet[did];
    const txt = normtxt;

    const longtxt = dobj ? dobj.name + ' ' + dobj.dn + '. ' + normtxt : did + '. ' + normtxt;

    // console.log('BEFORE devicelogger did='+did+' txt='+txt)
    devicelogger.addLog(did, { did, prop, ts: tsStop, txt, level: 0 });
    // console.log('ADD MAINLOG '+util.inspect({ did, prop, ts: tsStart, txt: longtxt, level, tags, location }))
    logconnector.addLog('mainlog', { did, prop, ts: tsStop, txt: longtxt, level: 0, tags, location });
