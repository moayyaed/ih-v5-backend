...
class Alertengine {
  ...
  tryStartAlert(aleData) {
    ...
    aleData.ajarr = this.chooseJournals(aleData);
  ...
  startAlert(aleData) {
    const _id = getNewAid();
    const { owner, level, devTitle, txt, ajarr } = aleData;
    ...
    this.emitAlertsChanged('add', [{ id: _id, ts: tsStart, tsStart, level, txt: longtxt, state: 1, ajarr }]);
  }
  ...
  async deackAlert(aleData) {
    ...
    this.emitAlertsChanged('update', [{ id: _id, tsAck: 0, userId: '', ajarr }]);
  }
