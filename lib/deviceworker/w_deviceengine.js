...
class Deviceengine {
  ...
  getHandlerForType(type, event) {
    if (this.typeMap.has(type)) {
      ...
      // console.log('getHandlerForType type='+type+' typeObj='+util.inspect(typeObj))
  ...
  runDeviceHandler(did, hanid, fn, triggers) {
    console.log('runDeviceHandler '+did+' fn='+util.inspect(fn)+' triggers='+util.inspect(triggers))
    try {
      if (fn) {
        const ts = Date.now();
  ...
  formTypeHandlerSetsFromTypeItem(type) {
    ...
    /*
    if (item.scriptOnInterval && item.par_OnInterval) {
      const interval = item.par_OnInterval;
      addToSet(this.onIntervalByType, type, interval);
      this.tm.startTimer(interval, { owner: 'type', tname: interval });
    } else {
      // сбросить
  ...
  updateInterval(flag, par, type) {
    ...
    // Прежний сбросить - когда таймер сработает, он будет пропущен
    if (included) {
      removeFromAllSet(this.onIntervalByType, type);
    }
    // Новый добавить
