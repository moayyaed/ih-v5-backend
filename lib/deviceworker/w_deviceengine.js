...
class Deviceengine {
  ...
  start() {
    ...
    this.wCore.on('update:global', data => {
      ...
      this.updateGlobal(data.did, data.chobj);
    });

    this.wCore.on('remove:global', data => {
      if (!this.checkData(data, 'remove:global', ['did'])) return;
      this.removeGlobal(data.did);
    });
  ...
  updateGlobal(did, doc) {
    this.wCore.global.updateItem(did, doc);
  }
