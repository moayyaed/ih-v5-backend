...
class Deviceengine {
  ...
  start() {
    ...
    this.wCore.on('received:globals', getObj => {
      Object.keys(getObj).forEach(did => {
        this.setValueGlobal(did, getObj[did]);
      });
    });
    ...
    this.wCore.on('exec:global:setvalue', data => {
      ...
      this.setValueGlobal(data.did, data.value);
      /*
      const did = data.did;
    ...
    this.wCore.on('update:type:alerts', data => {
      if (!this.checkData(data, 'update:type:alerts', ['typeId', 'chobj'])) return;
      const typeObj = this.typeMap.get(data.typeId);
      if (!typeObj) return;

      typeObj.changeAlerts(data.chobj);
      // console.log('update:type:alerts  typeObj='+util.inspect(typeObj, null, 4))
      this.formAlertsByTypeFromTypeItem(data.typeId);
    });
  ...
  runAlertsByType(changed) {
    changed.forEach(item => {
      const type = this.devSet[item.did].type;
      if (this.alertsByType[type]) {
        if (this.alertsByType[type].has(item.prop)) {
          console.log('ALERT ' + item.did + ' ' + item.prop);
          ...
          if (typeObj.props[item.prop].ale && typeObj.alerts[item.prop]) {
            ...
            if (res) {
              // Добавить данные устройства и генерировать алерт, если он еще не сгенерирован?
              const dobj = this.devSet[item.did];
              if (dobj._alertslevel[item.prop] == undefined ||  dobj._alertslevel[item.prop] != res.level) {
                dobj._alertslevel[item.prop] = res.level;
                const aleObj = {did: item.did, prop:item.prop, dn:dobj.dn, name:dobj.name, ...res};
                console.log('EMIT = ' + util.inspect(aleObj));

                this.wCore.emit('alert:device:call', aleObj);
  ...
  boolAlert( val, rules) {
    return (rules.B_1 && val == Number(rules.B_1.theval)) ? rules.B_1 : rules.B_0;
   
  }

  hiLoAlert(val, rules) {
    if (rules.N_LoLo && rules.N_LoLo.use && val < Number(rules.N_LoLo.theval)) return rules.N_LoLo;
    if (rules.N_HiHi && rules.N_HiHi.use && val > Number(rules.N_HiHi.theval)) return rules.N_HiHi;
    if (rules.N_Lo && rules.N_Lo.use && val < Number(rules.N_Lo.theval)) return rules.N_Lo;
    if (rules.N_Hi && rules.N_Hi.use && val > Number(rules.N_Hi.theval)) return rules.N_Hi;
    return rules.N_0;
  }
  ...
  formAlertsByTypeFromTypeItem(type) {
    ...
    // console.log(' formAlertsByTypeFromTypeItem this.alertsByType=' + util.inspect(this.alertsByType, null, 4));
  }
  ...
  setValueGlobal(did, value) {
    if (did && this.wCore.global.getItem(did)) {
      this.wCore.global.setValue(did, value);
    } else {
      console.log('ERROR: WORKER: NOT FOUND ' + did);
    }
  }
