...
class Deviceengine {
          ...
          if (typeObj.props[item.prop].ale && typeObj.alerts[item.prop]) {
            ...
            if (res) {
              ...
              if (dobj._alertslevel[item.prop] == undefined ||  dobj._alertslevel[item.prop] != res.level) {
                ...
                const aleObj = {did: item.did, prop:item.prop, propTitle:dobj.getPropTitle(item.prop), ...res};
  ...
  boolAlert( val, rules) {
    return (rules.B_1 && val == Number(rules.B_1.theval)) ? {aruleId:'B_1', ...rules.B_1} : {aruleId:'B_0',...rules.B_0};
   
  }

  hiLoAlert(val, rules) {
    if (rules.N_LoLo && rules.N_LoLo.use && val < Number(rules.N_LoLo.theval)) return {aruleId:'N_LoLo',...rules.N_LoLo};
    if (rules.N_HiHi && rules.N_HiHi.use && val > Number(rules.N_HiHi.theval)) return {aruleId:'N_HiHi',...rules.N_HiHi};
