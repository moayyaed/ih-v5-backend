...
class Deviceengine {
  ...
  runDeviceHandler(did, hanid, fn, triggers) {
    ...
    try {
      if (fn) {
        const ts = Date.now();
        if (this.getRunningPerSek(did, hanid, ts) > 4) {
          // 4 раза в сек onChange по устройству - зациклилось!!
          const dn = this.devSet[did] ? this.devSet[did].dn : '';
          throw 'Handler made 5 launches for ' + dn+' ('+did + ') per second! Possibly looping, blocked' ;
        }
  ...
  getRunningPerSek(did, hanid, ts) {
    ...
    if (!itemPerSek.lastsek || ts - itemPerSek.lastsek > 1000) {
