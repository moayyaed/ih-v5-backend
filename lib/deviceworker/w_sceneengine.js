...
class Sceneengine {
  ...
  start() {
    ...
    this.wCore.on('changed:device:data', data => {
      ...
      // if (this.activeSet.size > 0)  this.listenersOnChange(data); //
    });
   ...
   * @param {Array of Objects} changed -  массив изменений  [{dn prop, value},...]
   */
  listenersOnChange(changed) {
    console.log('listenersOnChange START')
    for (const id of agent.activeSet) {
      if (this.sceneSet[id] && this.sceneSet[id].listeners) {
        for (const listenerKey of Object.keys(this.sceneSet[id].listeners)) {
          console.log('listenersOnChange listenerKey='+listenerKey)
          if (this.sceneSet[id].listeners[listenerKey]) {
            const triggers = [];
            changed.forEach(item => {
              if (listenerKey == item.dn || listenerKey == item.dn + '.' + item.prop) {
                triggers.push(item.dn + '.' + item.prop);
              }
            });
            if (triggers.length) {
              console.log('agent tryExec')
  ...
  startScene(id, toRunItem) {
    ...
    if (agent.tryStart(id, toRunItem)) {
      // Сценарий запустился и остался активным
