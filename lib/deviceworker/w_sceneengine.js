...
class Sceneengine {
  ...
  start() {
    ...
    this.wCore.on('update:scene', data => {
      ...
      if (data.sceneId) {
        ...
        // И добавить заново
        this.addScene(data.sceneId, data);
        // Флаг отладки не трогаем совсем
      }
    });
  ...
  getActualParams(id, devs, def) {
    ...
    const res = { globals: this.globals };
    // console.log(id+' getActualParams start devs='+devs+' def='+util.inspect(def))
    if (devs && def) {
      ...
      arr.forEach(dev => {
        ...
        // res[dev] = dobj;
        if (dobj) {
          // console.log(id+' getActualParams dn='+dn+' dobj='+util.inspect(dobj))
         
          if (!this.scenedevs[dn]) this.scenedevs[dn] = new Scenedevo(dobj, agent);

          res[dev] = this.scenedevs[dn];
          // res[dev] = new Scenedevo(dobj, agent);
        } else {
          console.log(id+' getActualParams NOT FOUND dev='+dev)
        }
      });
     //  console.log(id+' getActualParams res='+util.inspect(res))
    } else {
      console.log(id+' getActualParams NO devs or def!!')
    }
