...
class Sceneengine {
  ...
  listenersOnChange(changed, wasRun) {
    ...
    for (const id of agent.activeSet) {
      console.log('activeSet id=' + id+' this.sceneSet[id].listeners='+util.inspect(this.sceneSet[id].listeners));
      if (!wasRun[id] && this.sceneSet[id] && this.sceneSet[id].listeners) {
        for (const listenerKey of Object.keys(this.sceneSet[id].listeners)) {
          console.log('listenersOnChange listenerKey=' + listenerKey+ ' fun='+this.sceneSet[id].listeners[listenerKey]);
          if (this.sceneSet[id].listeners[listenerKey]) {
            ...
            // console.log('changed='+util.inspect(changed)+' triggers='+triggers.join('.'));
            if (triggers.length) {
  ...
  updateScene(id, scenObj) {
    this.removeScene(id);
    this.addScene(id, scenObj);
  }
