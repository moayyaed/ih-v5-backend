...
class Sceneengine {
  constructor(wCore, scenes) {
    ...
    this.listenersOnChange = {}; // {<dn>:{<prop>: new WeakSet [scen001.listener[dn][prop], scen004]}}
  ...
  */
  ...
  addToTriggers(scenObj, sceneid) {
    ...
    if (!scenObj.triggers) return;

    const arr = scenObj.triggers
      .split(',')
  ...
  startScene(id, toRunItem) {
    console.log(id + ' toRunItem=' + util.inspect(toRunItem));
    ...
    agent.debug(id, '---Started--- ' + getTriggersStr());
    try {
      this.postTrace(id, traceObj);
      // this.sceneSet[id].triggers = triggers;
    ...
    function getTriggersStr() {
      ...
      let str = ' Triggers: ';
      toRunItem.triggers.forEach((trigger, idx) => {
        str += trigger + '=' + toRunItem.values[idx] + ' ';
      });
      return str;
    }
  }
