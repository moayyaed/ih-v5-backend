...
class Sceno {
  ...
  */
  ...
  execOS(cmd, cb) {
    let respId;
    if (cb) {
      respId = shortid.generate();
      this.responsefn[respId] = cb;
    }
    this.agent.execOS(this.id, cmd, respId);
  }

  pluginCommand(command, cb) {
    let respId;
    if (cb) {
      respId = shortid.generate();
      this.responsefn[respId] = cb;
    }
    this.agent.pluginCommand(this.id, command, respId);
  }

  pluginRestart(unit) {
    this.agent.pluginRestart(this.name, unit);
  }
  ...
  snap(fobj, cb) {
    ...
    this.agent.snap(this.id, fobj, respId);
  }
  ...
  info(infotype, dest, txt) {
    ...
    this.agent.sendInfo(this.id, infotype, sobj);
  }

  /*
  showDialog(dialogId, dest, sobj) {
    this.agent.showDialog(this.id, {dialogId, dest, sobj})
  }
  */
