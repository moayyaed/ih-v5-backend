...
class Sceno {
  ...
  addListener(trigger, func) {
    ...
    if (this.def[trigger]) {
      this.listeners[this.def[trigger]] = func;
    } else {
      this.log('ERROR addListener: not found device for trigger '+trigger)
    }
    // console.log('this.listeners='+util.inspect(this.listeners))
  }
