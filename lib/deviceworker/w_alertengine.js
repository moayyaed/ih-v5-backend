...
class Alertengine {
  ...
  /**
   *
   * @param {Object} aleObj = {owner:'d003.state', txt, level,...}
   */
  ...
  addAlert(owner, aleData) {
    const _id = getNewAid();
    this.alertSet[_id] = { _id, tsStart: Date.now(), ...aleData };
  ...
  tryStopAlert(aleData) {
    ...
    if (this.ownerMap.has(owner)) {
      const aid = this.ownerMap.get(owner);
      this.stopAlert(aid, 'norm');
      this.ownerMap.set(owner, 0);
    }
  }

  // Отправить, у себя сразу удалить
  stopAlert(aid, reason) {
    console.log('stopAlert ' + aid + ' reason ' + reason);
    if (!this.alertSet[aid]) {
      console.log('Not FOUND alert ' + aid);
      return;
    }

    this.sendStatus({ ...this.alertSet[aid], reason, tsStop: Date.now() }, 'stop');
