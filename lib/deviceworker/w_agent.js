...
module.exports = {
  start(wCore, placeList) {
    this.wCore = wCore;
    this.isWorker = true;

    this.placeMap = new Map();
    this.loadPlaceMap(placeList);

    this.wCore.on('updated:list:placeList', data => {
      // Изменен список placeList - просто заменяю
      this.loadPlaceMap(data);
      console.log('W updated:list:placeList=' + util.inspect(this.placeList));
    });
  ...
  loadPlaceMap(dataArr) {
    this.placeMap.clear();
    dataArr.forEach(item => {
      this.placeMap.set(item.id, item);
    });
  },

  getPlaceArr(dobj) {
    if (!dobj) return [];
    const itemProp = 'title';
    let key = dobj.parent;
    const arr = [];
    while (key && key != 'place') {
      if (this.placeMap.has(key)) {
        const item = this.placeMap.get(key);
   ...
   * @param {*} arg4
   */
  runUserHandler(dobj, handlerObj, prop, arg3, chstatus) {
    if (!dobj || !dobj._id || !handlerObj) return;

    const hanid = dobj.type + '_' + prop;
    const args = [dobj, prop, arg3, chstatus];
  ...
  logDevice(did, logObj) {
    // console.log('W_AGENT logDevice did='+did+' logObj='+util.inspect(logObj))
  ...
  getExtprop(dn, prop, sceneId) {
    return this.wCore.sceneExtprops[sceneId] &&
      this.wCore.sceneExtprops[sceneId][dn] &&
  ...
  // Записать в БД, массив д б уже готов
  dbwrite(toWrite) {
    this.wCore.postMessage('dbwrite', { toWrite });
  }
};
