...
class Globalmanager {
  constructor(wCore) {
    ...
    // this.handlersOnChange = {}; // Сам обработчик внутри объекта gl = handler:{filename, fn, blk, error}
    // {<gl_did>:   {DD1:<dobj>, ..}
  }
  ...
  setValue(id, value, sender) {
    // console.log('SET VALUE '+id+' value='+value)
  ...
  removeItem(id) {
    ...
    if (this.handlersOnChange[id]) {
      hut.unrequire(this.handlersOnChange[id].filename);
      delete this.handlersOnChange[id];
    }
