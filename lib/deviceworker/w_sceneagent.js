...
module.exports = {
  ...
  doAll(id, filter, act, value) {
    ...
    let devsubset = this.wCore.getDevSubset(filter);
    if (!devsubset) return [];
  
    // Отобрать устр-ва, которые могут выполнить команду и сразу выполять?
    devsubset.forEach(dn => {
      let dobj = this.wCore.devSet[dn];
      if (dobj.hasCommand(act)) {
        dobj.doCommand(act, this.running);
      }
    });
    return devsubset; // Возвращаем массив dn устройств, для которого сработала групповая команда
  },
