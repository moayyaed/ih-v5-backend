...
class Devo extends Basedevo {
  ...
  runHandler(prop, arg3, arg4) {
    ...
    try {
      ...
      */
      ...
      return { error: 'Handler error: ' + hut.getShortErrStr(e) };
  ...
  acceptNewValue(prop, ts, inval, sender) {
    if (this._id && !this._id.startsWith('__')) {
      console.log('acceptNewValue inval=' + util.inspect(inval));
    }
    if (inval == undefined) return;

    const resObj = { did: this._id, dn: this.dn, prop, ts };
    ...
    if (typeof inval != 'object') {
      ...
      // { value, error} - результат работы функции - обработчика
      if (inval.value != undefined) newval = inval.value;
      newerr = inval.error || '';
    }

    newval = this.applyPropType(prop, newval);
    ...
    if (this._raw[prop].err != newerr) {
      this._raw[prop].err = newerr; // Ошибка свойства пишется только в _raw
      resObj.err = newerr;
    }
    if (this._id && !this._id.startsWith('__')) {
      console.log('acceptNewValue resObj=' + util.inspect(resObj));
    }
    return resObj;
