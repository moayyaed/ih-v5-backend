...
class Devo extends Basedevo {
  ...
  getChstatus(prop) {
    if (this._raw[prop]) return this._raw[prop].chstatus;
  }
  ...
  acceptData(inObj, sender) {
    ...
    Object.keys(inObj).forEach(prop => {
      if (this._raw[prop] != undefined && !this.isCommand(prop)) {
        ...
        const one = this.acceptOne(prop, ts, inObj[prop], chstatus, sender);
        if (one) accepted.push(one);
        if (chstatus != undefined) this._raw[prop].chstatus = chstatus;
      }
    });
   ...
   */
  acceptOne(prop, ts, val, chstatus, sender) {
    this._raw[prop].raw = val; // Полученное значение сохраняем как raw
    const newval = this.hasHandler(prop) ? this.runHandler(prop, val, chstatus) : val;
  ...
  runHandler(prop, arg3, chstatus) {
    ...
    try {
      if (!handlerObj.sys) {
        res = this.agent.runUserHandler(this, handlerObj, prop, arg3, chstatus);
      } else {
        res = handlerObj.fn(this, prop, arg3, chstatus);
      }
