...
module.exports = async function checkUpdateSystem(usebetasystem) {
  let refresh = false;
  let message = 'Установлена последняя версия ';
  // const res = await updateutil.getLatest();

  const res = await updateutil.getUpdateInfo(false);
  if (res) {
    if (res.newversion) {
      message = 'Доступна версия ' + res.newversion;
      appconfig.setNewversion('system', res.newversion);
      refresh = true;
    } else  if (res.newbetaversion) {
      if (usebetasystem) {
        message = 'Доступна beta-версия ' + res.newbetaversion;
        appconfig.setNewversion('betasystem', res.newbetaversion);
        refresh = true;
      }
    } else {
