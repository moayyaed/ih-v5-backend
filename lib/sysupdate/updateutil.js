 ...
 *
 *  @param {Object}  {version, url} - если требуется обновление!!
 * 
 */
async function getUpdateInfo(name, currentVersion) {
  if (!name) {
    name = appconfig.getConfName();
  }
  if (!currentVersion) {
    currentVersion = appconfig.get('version');
  }

  const updateReq = 'https://update.ih-systems.com/version/' + name;
  try {
    const res = await nu.httpGetJsonP(updateReq);
    if (!res || !res.version || !res.url) {
      throw {
        message: 'Req: ' + updateReq + ' Res: ' + util.inspect(res) + ' Expected "version" and "url" in response'
      };
    }

    const cmp = compareSemVer(res.version, currentVersion);
    if (cmp) {
      console.log('INFO: ' + currentVersion + '<' + res.version);
      return { url: res.url, newversion: res.version };
    }

  } catch (e) {
    console.log('ERROR:  updateSystem Req: ' + updateReq );
    throw e;
  }
}
