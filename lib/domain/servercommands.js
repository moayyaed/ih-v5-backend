      ...
      if (Array.isArray(frames)) {
        frames.forEach(item => {
          if (item.target_frame && item.target_frame.id && item.container_id && item.container_id.id) {
            ...
            if (item.journal_id && item.journal_id.id) journal_id = item.journal_id.id;
            if (item.alertjournal_id && item.alertjournal_id.id) alertjournal_id = item.alertjournal_id.id;

            resObj.context.frames[item.target_frame.id] = {
              container_id: item.container_id.id,
              device_id,
              multichart_id,
              timelinechart_id,
              journal_id,
        ...
        Object.keys(frames).forEach(frame => {
          if (!resObj.context.frames) resObj.context.frames = {};
          resObj.context.frames[frame] =
            typeof frames[frame] == 'object' ? { ...frames[frame] } : { container_id: frames[frame] };
        });
      }
    }
    return resObj;
