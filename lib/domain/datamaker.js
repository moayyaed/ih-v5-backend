...
async function createOneRecord({ table, filter, item, parentid, body }, dm) {
  ...
  if (item.foldertype) doc.foldertype = item.foldertype;
...
async function copypasteDoc({ table, doc, target, seqObj }, dm) {
  ...
  switch (table) {
    ...
    case 'devhard':
      ...
      doc.prop = '';
      */
      await devhardutil.copyDevhardDoc(oldId, doc, dm);
      break;
    ...
    case 'device':
      ...
      // TODO копировать настройку для мобильного и другие интеграции
...
async function customValidate({ prop, doc, _id, table }, dm) {
  console.log(
    'WARN: customValidate START prop=' + prop + ' doc=' + util.inspect(doc) + ' id=' + _id + ' table=' + table
  );
  let res;
  switch (table) {
    case 'devhard':
