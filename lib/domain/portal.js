...
async function activation_license(query, holder) {
  try {
    ...
    console.log(
      'INFO: portalResult.data ' +
        portalResult.data +
        ' LEN=' +
        portalResult.data.length +
        ' type=' +
        typeof portalResult.data
    );
...
async function deactivate_license(query, holder) {
  try {
    if (!query || !query.key) throw { message: 'Missing key in query:' + util.inspect(query) };

    const key = query.key;
    const rec = await holder.dm.findRecordById('licenses', key); // rec {_id:key, id, userid}
    if (!rec) {
      
      throw { message: 'Not found record with key ' + key };  
    }
    ...
    const data = { id: rec.id, userid: rec.userid, hwid: appconfig.get('hwid') };
