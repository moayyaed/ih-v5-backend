...
async function getDataVis(cmd, query, holder) {
  ...
  const dm = holder.dm;
  const frames = query.frames ? parseFrames(query.frames) : ''; // [{frame:frame_4, vc: vc002, device:d007, multichart_id:xx, timelinechart_id:yy}]
  ...
  switch (cmd) {
    ...
    case 'templates':
      ...
      if (query.layoutid) {
        ...
        return joinTemplatesForLayout(query.layoutid, frames, holder);
      }
    ...
    case 'containers':
      ...
      if (query.layoutid) {
        return joinContainersForLayout(query.layoutid, frames, holder);
      }
  ...
  arr
    ...
    .forEach(item => {
      ...
      if (elems.length > 1 && elems[0] && elems[1]) {
        // frame_1,vc002,d042
        const [frame, vc, device, multichart_id, timelinechart_id, journal_id, alertjournal_id] = elems;
        if (!res) res = {};
        res[frame] = { vc, device, multichart_id, timelinechart_id, journal_id, alertjournal_id };
      }
    });
  return res;
}
  ...
  if (dataObj && dataObj.elements) {
    Object.keys(dataObj.elements).forEach(name => {
      if (
        ...
        if (frameObj[name].device) {
          if (!dataObj.elements[name].widgetlinks.link.value) {
            dataObj.elements[name].widgetlinks.link.value = {};
          }
          if (!dataObj.elements[name].widgetlinks.link.value.device) {
            dataObj.elements[name].widgetlinks.link.value.device = {};
          }
          dataObj.elements[name].widgetlinks.link.value.device.id = frameObj[name].device;
        }
...
async function getRtData(cmd, query, holder) {
  ...
  switch (cmd) {
    ...
    case 'containers':
      if (query.layoutid) {
        return joinContainersRtForLayout(query.layoutid, frames, holder);
      }
      throw { err: 'SOFTERR', message: "Expected 'layoutid' in query!" };
...
async function joinContainersForLayout(layoutId, frames, holder) {
  const resObj = {};
  const dataObj = await getCachedProjectObj('layout', layoutId, holder.dm);
  const ids = gatherContainerIdsFromLayoutData(dataObj, frames); // ids контейнеров c учетом frames!!
...
async function joinContainersRtForLayout(layoutId, frames, holder) {
  let resObj = {};
  const dataObj = await getCachedProjectObj('layout', layoutId, holder.dm);

  const ids = gatherContainerIdsFromLayoutData(dataObj, frames);
  ...
  const vc_didMap = gatherContainersDevContextMapFromLayoutData(dataObj, frames);
 ...
 * В случае если переменные расположены на (экране, контейнере, диалоге)
 */
...
function getStaticObj(holder, did, prop) {
  // Для переменных - передать значение!!
  if (isGlobalVar(did)) {
    return { [prop]: getPropVal(holder, did, prop) };
  }
...
function gatherFramesWithWidgetlinksFromLayoutData(dataObj) {
  const arr = [];
  ...
  if (dataObj && dataObj.elements) {
    Object.keys(dataObj.elements).forEach(name => {
      if (
        ...
        const title = dataObj.elements[name]._label || dataObj.elements[name].title || name;
        // arr.push({ id: name, title, vcId: });
        arr.push({ id: name, title,  ...getFrameLink(dataObj.elements[name].widgetlinks.link)});
      }
    });
  }
  return arr;

  function getFrameLink(item) {
    const res =  {containerId:item.id};
    if (item.value) {
      res.device = item.value.device;
      res.multichart_id = item.value.multichart_id ? item.value.multichart_id.id : '';
      res.timelinechart_id = item.value.timelinechart_id ? item.value.timelinechart_id.id : '';
      res.journal_id = item.value.journal_id ? item.value.journal_id.id : '';
      res.alertjournal_id = item.value.alertjournal_id ? item.value.alertjournal_id.id : '';
    }
    return res;
  }
...
function gatherContainerIdsFromLayoutData(dataObj, frames = {}) {
  const ids = [];
...
function gatherContainersDevContextMapFromLayoutData(dataObj, frames = {}) {
  ...
  if (dataObj && dataObj.elements) {
    Object.keys(dataObj.elements).forEach(name => {
      if (
        ...
        let did;
        if (frames[name] && frames[name].device) {
          did = frames[name].device;
        } else {
          did = getDidFromWidgetlinks(dataObj.elements[name].widgetlinks.link);
        }
        if (did) vc_contextDevice.set(name, did);
      }
    });
  }
  return vc_contextDevice;

  function getDidFromWidgetlinks(link) {
    if (link && link.id && link.value && link.value.device && link.value.device.id) return link.value.device.id;
  }
}

async function joinTemplatesForLayout(layoutId, frames, holder) {
  const dataObj = await getCachedProjectObj('layout', layoutId, holder.dm);
  const cids = gatherContainerIdsFromLayoutData(dataObj, frames);
...
async function getProjectObj({ id, nodeid }) {
  ...
  const folders = ['layout', 'container', 'template', 'dialog','reportmakeup'];
...
module.exports = {
  ...
  isProp,
  parseFrames,
  gatherFramesFromLayoutData,
  gatherFramesWithWidgetlinksFromLayoutData
};
