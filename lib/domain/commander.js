...
async function exec(user, action, mes, holder) {
  let sender = user ? user.name : 'Unknown';
  sender = 'user: ' + sender;
  mes.userId = user ? user._id : 'admin';
  const did = mes.type == 'devicecommand' ? mes.id : mes.did;

  if (!hasUserRightsToUIAction(user, action, did, holder)) {
    return { err: appconfig.getMessage('InsuffRightsForAction') };
  }

  if (mes.type == 'command') return execCommand(sender, mes, holder);
  if (mes.type == 'devicecommand') return execDeviceCommand(sender, { did: mes.id, prop: mes.command }, holder);
  if (mes.type == 'set') return execSet(sender, mes, holder);
}
...
function execDeviceCommand(sender, { did, dn, prop }, holder) {
  if (dn) {
    did = findDeviceDid(dn, holder);
  }
...
function findDeviceDid(dn, holder) {
...
function hasUserRightsToUIAction(user, action, did, holder) {
 // console.log('hasUserRightsToUIAction action='+action+' did='+did+' user='+util.inspect(user));

  if (action != 'devicecommand' && action != 'device' && action != 'setval') return true;
  if (!user || !user._id) return;

  const rules = holder.am.getUserDevctlRules(user._id);
  // console.log('hasUserRightsToUIAction rules='+util.inspect(rules));
