...
const specForms = ['formSceneCalls', 'formCustomtableCommon', 'formIntegration', 'formTypeIntegration'];
const recordDependentForms = { scene: ['formSceneCalls'], customtable: ['formCustomtableCommon'] };
...
function getPluginFormName(id) {
  ...
  return plspec.includes(formName) || formName.startsWith('formPlugin')  ? formName : '';
}

async function getForm(query, dm) {
  const { id } = query;

  if (id == 'formIntegration') return getFormIntegration(query, dm);
  if (id == 'formTypeIntegration') return getFormTypeIntegration(query, dm);
  if (id == 'formSceneCalls') return getFormSceneCalls(query, dm);
...
async function getFormIntegration(query, dm) {
  const { id, nodeid } = query;
  if (nodeid == 'mobile') return loadsys.loadMeta('form', 'formDeviceMobileSetting');
  
  const data = await loadsys.loadPluginMetaData('form', 'formIntegration', nodeid);
  // Заменить имена таблиц?
  // console.log('getFormIntegration data='+util.inspect(data))
  replaceTableName(data, nodeid);
  return data;
}

function replaceTableName(data, app) {
  if (data && data.grid && data.grid.length) {
    data.grid.forEach(item => {
      if (item.table && item.table.startsWith('_')) {
        item.table = 'i_'+app+item.table;
      }
    })
  }
}

async function getFormTypeIntegration(query, dm) {
  const { id, nodeid } = query;
  if (nodeid == 'mobile') return loadsys.loadMeta('form', 'formTypeMobileSetting');
  const data = await loadsys.loadPluginMetaData('form', 'formTypeIntegration', nodeid);
  replaceTableName(data, nodeid);
  return data;
}
...
async function getFormWithPlink(id, unit) {
  ...
  // Если форма уже имеет plink - ничего не делать
  if (channelform.grid.find(item => item.id == 'plink')) return hut.clone(channelform);
  ...
  if (doc && doc.props && data[px] && data[px][0] && data[px][0].columns) {
    ...
    if (keys.length > 0) {
      data[px][0].columns = keys.map(key => ({
        ...
        width: doc.props[key].width > 0 ? Number(doc.props[key].width) : 150,
        filter: true
      }));
    }
  }
...
async function getFormSceneCalls(query, dm) {
  const { id, nodeid } = query;
  const data = await loadsys.loadMeta('form', id);
  if (!data.grid) throw { message: 'Invalid form:' + id + '. Exected grid property!' };
