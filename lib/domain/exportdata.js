...
async function exportOne(query, holder) {
  ...
  switch (query.param) {
    ...
    case 'sound':
      return exportPack.exportFilesFromTree(
        { nodeid: query.nodeid, sourcePath: appconfig.get('soundpath'), table: 'sound', treeName: 'sounds' },
        holder
      );
...
async function prepareToExport(query, user, holder) {
  ...
  if (query.param == 'sound') {
    return prepareToExportFilesFromTree(query, holder);
  }
...
/**
 * Подготовка к выгрузке одного файла из дерева или нескольких файлов внутри папки
 *
 * @param {Object} query
 ...
 * @param {Object} holder
 */
async function prepareToExportFilesFromTree(query, holder) {
  const { nodeid, table, treeName } = query;
  const url = getExportUrl(query);
  ...
  if (nodeid.indexOf('.') > 0) {
    ...
    // Собрать из папки
    const { arr, folderName } = await exportPack.getArrayFromTreeFolder({ nodeid, table, treeName }, holder.dm);
    const title = `${arr.length} ${appconfig.getMessage('filesFromFolder')} ${folderName}`;
    data = { title, url, filename: query.nodeid + '.zip' };
  }
  return { data };
}
