...
async function getPopupPlugin(query, holder) {
  ...
  async function getMainPluginPopup() {
    ...
    arr.push({ id: 'delete', type: 'item', title: appconfig.getMessage(deleteTitle) + ' ' + nodeid, command: 'delete' });
    return { data: arr };

    function addCopyPaste() {
      arr.push({ id: 'copy', type: 'item', title: appconfig.getMessage('Copy'), command: 'copy' });
      arr.push({ id: 'paste', type: 'item', title: appconfig.getMessage('Paste'), command: 'paste', check: 'disablePaste' });
