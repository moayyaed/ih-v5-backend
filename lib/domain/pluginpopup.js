...
async function getPopupPlugin(query, holder) {
  ...
  async function getMainPluginPopup() {
    ...
    if (unitDoc.folder && unitDoc._id.startsWith('plugin_')) {
      ...
      const addTitle = appconfig.getMessage('AddInstance') + ' ' + newId;

      arr.push({ id: newId, title: addTitle, command: 'addNodeByContext' });
      addPluginExport(plugin);
      arr.push({ id: 'divdelete', type: 'divider' });
      arr.push({ id: '4', type: 'item', title: appconfig.getMessage('DeleteAllInstances'), command: 'delete' });
      return { data: arr };
    }

    // Single плагин -  можно деактивировать Экземпляр мультиплагина - удалить По сути одно и то же
    const deleteTitle = !isSingle() ? 'DeleteInstance' : 'DeactivatePlugin';
    ...
    addLogExport();
    addDeletePluginlog();
    if (isSingle()) addPluginExport(nodeid);
    ...
    function isSingle() {
      return !unitDoc.parent.startsWith('plugin_');
    }

    function addPluginExport(plugin) {
      arr.push({ id: 'divexport_plugi', type: 'divider' });
      arr.push({
        id: 'export_plugin',
        type: 'item',
        title: appconfig.getMessage('ExportPlugin'),
        command: 'export',
        param: 'plugin'
      });
    }

    function addDeletePluginlog() {
      arr.push({
        id: 'deletelog',
        type: 'item',
        title: 'Очистить журнал плагина',
        command: 'send',
        param: { emit: 'deletelog:plugin' }
      });
    }
