...
async function devicelog(data, table, nodeid, pitem, holder) {
  ...
  if (didArr.length > 1 || domaindata.isGlobal(didArr[0])) {
    ...
    if (dobj) {
      arr = await dobj.getInnerLog(true); // reverse = true
    }
    */
...
async function typehandlersTrace(data, table, nodeid, pitem, holder) {
  ...
  Object.keys(holder.traceSet).forEach(id => {
    ...
    item.did_link = domaindata.getDeviceLinkObj(item.did);
  ...
  function getDuration(item) {
    return (item.startTs > 0 && item.stopTs >= item.startTs)  
      ? (item.stopTs > item.startTs ? item.stopTs - item.startTs : '<1')
      : '';
  }
  ...
  function getLink(str) {
    ...
    return type && typeList.has(type)
      ? { title: typeList.get(type).title+'.'+prop, path: domaindata.getTypeHandlerPath(type, prop) }
      : { title: str, path: '' };
  }
}
