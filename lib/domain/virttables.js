...
async function scenestree(data, table, nodeid, pitem, holder) {
  // console.log('holder.sceneSet='+util.inspect(holder.sceneSet))

  const arr = [];
  data[table].forEach(rec => {
    const item = holder.sceneSet[rec.id];
    // if (item.multi) {
...
async function alertlog(data, table, nodeid, pitem, holder) {
  // Пока все беру из  alerts без фильтра TODO - нужно фильтровать по nodeid - это папка
  const filter = {};
  const rows = await holder.dm.datagetter.getAlertLogRows(filter, '', holder);
...
async function journallastrecs(data, table, nodeid, pitem, holder) {
  try {
    const fromDb = await logconnector.getLog('mainlog',{},  { limit:100 });
    //  { start, end, dn_prop: dn });
...
async function plugins(data, table, nodeid, pitem, holder) {
  const status = ['?', 'Установлена последняя версия', 'Есть обновление', 'Доступно для установки'];
