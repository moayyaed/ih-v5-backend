...
async function typeprops(dataFromTable, table, nodeid, pitem, holder) {
  ...
  /*
  function getPropItem(prop) {
    const name = prop.startsWith('_format') ? prop.substr(8) : name;
    arr.find(item => item.id == prop);
    { id: prop, errstr: getOnErrstr(prop), fuse: 2 }
  }
  */
...
async function pluginlog(data, table, nodeid, pitem, holder) {
  ...
  // const arr = await holder.dm.bigstore.get('pluginlog', { unit: { $in: unitArr } }, { order: 'ts' });
  const arr = await holder.dm.getLog('pluginlog', { unit: { $in: unitArr } });
...
async function devicelog(data, table, nodeid, pitem, holder) {
  ...
  if (didArr.length > 1 || domaindata.isGlobal(didArr[0])) {
    // arr = await holder.dm.dbstore.get('devicelog', { did: { $in: didArr } }, { order: 'ts,_id' });
    arr = await holder.dm.getLog('devicelog', { did: { $in: didArr } });
  } else {
    // Начало журнала одного устройства всегда запрашивается здесь
    const dobj = holder.devSet[didArr[0]];
    if (dobj) {
      arr = await dobj.getInnerLog();
    }
  }
