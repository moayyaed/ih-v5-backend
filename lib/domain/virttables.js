...
const widgetdata = require('./widgetdata');
const pluginutil = require('../plugin/pluginutil');
const channelutil = require('../plugin/channelutil');
...
async function typeprops(dataFromTable, table, nodeid, pitem, holder) {
  ...
  if (datautil.isLink(nodeid)) {
    ...
    if (prop) {
      ...
      if (propItem) {
        ...
        propItem.when_str = misc.getWhenStr(rec, propItem.op, propItem.id);
      }
    }
    return propItem || {};
  }
  ...
  return holder.dm.tabledataFit(table, pitem.columns, arr);

  function getBlkstr(type, hprop) {
    const hanid = type + '_' + hprop;
    if (holder.traceSet[hanid]) {
      if (holder.traceSet[hanid].blk) return getMes('BLOCKED') + ': ' + holder.traceSet[hanid].error;
    }
  }
...
async function devicecommon(dataFromTable, table, nodeid, pitem, holder) {
  ...
  result.forEach(item => {
    ...
    if (dataFromTable.realtime[item.prop]) {
      ...
      if (item.prop == 'error') item.valstr = dItem.realtime_dev_err;
  ...
  function getType_fuse(prop, typeItem) {
    return {
      ...
      path: misc.getFormUrl('typeprophandler', dobj.type, prop)
    };
  }
  ...
  async function formDeviceProps() {
    ...
    const typename = getMes('Type') + ' ' + typeItem.item.name;
    const typePath = misc.getFormUrl('TypeProps', dobj.type);
    ...
    // error - системное свойство
    res.push(getItem('error', getMes('Error'), 'System', { title: '', path: '' }));
    return res;
  }
}
...
function getTypeOpTitle(propItem) {
  ...
  const opList = { rw: 'Data', r: 'Data', calc: 'Calculate', cmd: 'Command', par: 'Parameter', evnt: 'Event'};
...
async function typestree(data, table, nodeid, pitem, holder) {
  ...
  const arr = data[table]
    ...
    .map(item => Object.assign({ _id: item.id, name: item.title }, item, typeList.get(item.id)));
  return holder.dm.tabledataFit(table, pitem.columns, arr);
  // return tabledata.formRows(table, arr, pitem.columns, holder);
}
...
async function scenestree(data, table, nodeid, pitem, holder) {
  ...
  data[table].forEach(item => {
    ...
    if (sceneListItem) {
      item.multi_str = sceneListItem.multi ? getMes('MultiScene') : '';
      item.blk_str = sceneListItem.blk ? getMes('Blocked') : '';
    }
    arr.push(item);
  });
  return arr;
...
async function schedrulestree(data, table, nodeid, pitem, holder) {
  ...
  return arr.sort(hut.byorder('w_ts')).map(item => {
    ...
    const sched_link = { title: item.title, path: misc.getFormUrl('SchedruleCommon', item.id) };
...
async function sysdevicestree(data, table, nodeid, pitem, holder) {
  ...
  arr.push({
    title: { title: getMes('Total'), path: '' },
    stateStr: getMes('RunningProcesses') + ': ' + state_sum,
    memrss: memrss_sum,
    memheap: memheap_sum,
    memhuse: memhuse_sum,
  ...
  function getRowButton(state) {
    // { title, hide, command }
    if (state == 1) return { title: 'Stop', command: 'stopplugin' };
    if (state > 1) return { title: 'Start ', command: 'startplugin' };
...
async function soundstree(data, table, nodeid, pitem, holder) {
  const arr = data[table].map(item => Object.assign({ _id: item.id, name: item.title }, item));
  // const imgExclude = ['unset', 'noimage.svg'];

  // miss и другие поля - взять из таблицы
  const docs = await holder.dm.dbstore.get('sounds', {});
...
async function unitext(dataFromTable, table, nodeid, pitem, holder) {
  ...
  return holder.dm.tabledataFit(table, pitem.columns, arr.sort(hut.byorder('did,prop')));
...
async function unitchannels(dataFromTable, table, nodeid, pitem, holder) {
  ...
  if (!nodeid || nodeid == 'unitgroup' || nodeid.startsWith('plugin_')) {
    ...
    // Добавить данные из узла (папки с foldertype = 'node'), если есть такие
    const plugin = hut.removeLastNumFromStr(nodeid);
    const share_fields = await pluginutil.getManifestProp(plugin, 'share_node_folder_fields', holder.dm);
    if (share_fields) {
      channelutil.addShareFields(arr, share_fields);
    }
  }
...
async function onechannelRt(dataFromTable, table, nodeid, pitem, holder, id) {
  ...
  const rtChan = unitItem.chano.getRtChannel(unitchanItem.chanId); // val, ts
  let realtime_chan_str = '';
  if (rtChan) {
    realtime_chan_str = getTimeStr(rtChan.ts) + ' => ' + rtChan.val;
    if (rtChan.chstatus != undefined) realtime_chan_str += ' (status '+rtChan.chstatus+')';
  }
...
function unitstree(data, table, nodeid, pitem, holder) {
  ...
  data[table].forEach(rec => {
    ...
    const res = datautil.getStatusObj(item, 'unitStateList', true);
    if (res) arr.push(res);
  });
  return holder.dm.tabledataFit(table, pitem.columns, arr);
  // return tabledata.formRows(table, arr, pitem.columns, holder);
...
async function sysdevicegroup(data, table, nodeid, pitem, holder) {
  ...
  res.forEach(item => {
    if (item.folder && item.parent == 0) item.name = getMes('SystemIndicators');
  });
  return res;
}
...
async function devicelog(data, table, nodeid, pitem, holder) {
  ...
  if (didArr.length > 1) {
    ...
    // arr = await domaindata.getInnerLog(didArr[0], holder, true);
    arr = await devicelogger.getLog(didArr[0], true); // reverse = true
  }
...
async function dbsavetimeline(data, table, nodeid, pitem, holder) {
  // Собрать все что пишется в БД
  const docs = await holder.dm.dbstore.get('devicedb');
  const arr = [];
  let i = 0;
  docs.forEach(doc => {
    if (doc.did && doc.prop && doc.timeline) {
      i++;
      arr.push({
        id: i,
        did: { title: domaindata.getDeviceTitle(doc.did), path: misc.getFormUrl('DeviceDb', doc.did) },
        prop: doc.prop,
        timeline: 'да'
      });
    }
...
async function licensesList(data, table, nodeid, pitem, holder) {
  ...
  arr.forEach(item => {
    ...
    item.status =
      getMes('LicIsActive') +
      (item.endActivation > 0 ? getMes('_until_') + hut.getDateTimeFor(new Date(item.endActivation), 'reportdt') : '');
    item.title = item.platform + ' ' + (item.product == 'tags' ? ' (' + item.qt + ')' : item.product);
    if (item.demo) {
      item.rowbutton = { title: getMes('Delete'), command: 'deactivate_demo' };
      item.title += ' DEMO';
    } else {
      item.rowbutton = { title: getMes('Deactivate'), command: 'deactivate' };
    }
  });
  return arr;
}
...
async function globalhandler(data, table, nodeid, pitem, holder) {
  ...
  if (!doc.scriptOnChange) {
    ...
    doc.when_str = str ? getMes('OnChange') + ': ' + str : getMes('NoTriggersDefined');
  }
  return doc;
}
...
async function jlevels(data, table, nodeid, pitem, holder) {
  ...
  const arr = hut.transfromFieldObjToArray(pObj).sort(hut.byorder('level'));
  arr.forEach(item => {
...
async function customdata(data, table, nodeid, pitem, holder) {
  ...
  if (doc.fuse) {
    ...
    if (!Array.isArray(customDocs)) throw { message: getMes('HandlerShouldReturnArray') };
  } else {
...
function getSceneLink(id) {
  ...
  const scenePath = misc.getFormUrl('SceneCodeEditor', id);
  ...
  function getLink(str) {
    ...
    if (prop) {
      if (prop.startsWith('_On')) {
        desc = ' ' + getMes('Event') + ' ' + prop;
      } else if (prop.startsWith('_format_')) {
        desc = ' ' + getMes('Property') + ' ' + prop.substr(8) + '#string';
        // prop = '_' + prop;
      } else {
        desc = ' ' + getMes('Property') + ' ' + prop;
      }
    }

    return type && typeList.has(type)
      ? {
          title: getMes('Type') + ' ' + typeList.get(type).title + '. ' + desc,
          path: misc.getFormUrl('typeprophandler', type, prop)
          // path: domaindata.getTypeHandlerPath(type, prop)
        }
      : { title: str, path: '' };
  }
}
...
async function infoaddrCommon(data, table, nodeid, pitem, holder) {
  const arr = (await holder.dm.get('infoaddr', { userId: nodeid })).filter(item => item.infotype != 'pushnotification');
  const title = getMes('Send');
  arr.forEach(item => {
    const infotype = item.infotype || '-';
    item.infotype = { id: infotype, title: infotype };
    item.rowbutton = { title, command: 'sendinfo' };
    item.id = item._id;
  });
  return arr;
}

async function infoaddrPushnot(data, table, nodeid, pitem, holder) {
  const arr = await holder.dm.get('infoaddr', { userId: nodeid, infotype: 'pushnotification' });
  const title = getMes('Send');
  arr.forEach(item => {
    item.rowbutton = { title, command: 'sendinfo' };
    item.id = item._id;
...
async function plugins(data, table, nodeid, pitem, holder) {
  const status = [
    '?',
    getMes('UPTODATE'), // 'Установлена последняя версия'
    getMes('UpdateFound'), //  'Есть обновление'
    getMes('InstallationAvailable'), //  'Доступно для установки'
    getMes('LicenseRequired') //  'Для установки требуется лицензия
  ];
  ...
  return arr.sort(hut.byorder('id')).map(item => {
    ...
    if (item.status == 2) {
      item.rowbutton_up = { title: getMes('UPDATE'), command: 'updateplugin' };
    } else if (item.status == 3) {
      item.rowbutton_up = { title: getMes('Install'), command: 'installplugin' };
    } else {
  ...
  arr.forEach(item => {
    ...
    item.rowbutton =
      item.lastmodif > 0 && (!item.lastpub || item.lastmodif > item.lastpub)
        ? { title: getMes('Publish'), command: 'publishpage' }
        : { title: '', command: '' };
  });
  return arr;
}
...
async function mobilesettingfromtype(dataFromTable, table, nodeid, pitem, holder) {
  ...
  const typeItem = domaindata.getTypeobj(dobj.type);
  if (!typeItem || !typeItem.item) return [];
...
async function devhardWithParentNode(dataFromTable, table, nodeid, pitem, holder) {
  const rec = await holder.dm.findRecordById('devhard', nodeid);

  if (rec && rec.parent) {
    // Найти папку для этого канала
    const parentDoc = await holder.dm.findRecordById('devhard', rec.parent);
    if (parentDoc) {
      rec.parent = parentDoc._id;
      rec.parentname = parentDoc.chan || parentDoc.title;
      rec.parentunitid = parentDoc.unitid;
      rec.parentoffset = parentDoc.parentoffset;
      rec.foldertype = parentDoc.foldertype;
    }
  }
  return [rec];
}
...
module.exports = {
  ...
  dbsavetimelineTable: dbsavetimeline,
  ...
  infoaddr_common: infoaddrCommon,
  infoaddr_pushnot: infoaddrPushnot,
