...
async function infoaddrCommon(data, table, nodeid, pitem, holder) {
  ...
  const title = appconfig.getMessage('Send');
  arr.forEach(item => {
...
async function plugins(data, table, nodeid, pitem, holder) {
 ...
 const status = [
  '?',
  appconfig.getMessage('UPTODATE'),
  appconfig.getMessage('UpdateFound'),
  appconfig.getMessage('InstallationAvailable'),
  appconfig.getMessage('LicenseRequired')
];
  ...
  return arr.sort(hut.byorder('id')).map(item => {
    ...
    if (item.status == 2) {
      item.rowbutton_up = { title: appconfig.getMessage('UPDATE'), command: 'updateplugin' };
      //  } else if (item.yurl && item.status == 3) {
    } else if (item.status == 3) {
      item.rowbutton_up = { title: appconfig.getMessage('Install'), command: 'installplugin' };
    } else {
