...
async function snippetsRun(data, table, nodeid, pitem, holder) {
  ...
  Object.keys(holder.snippetSet).forEach(did => {
    ...
    const startStr =item.lts > 0 ? hut.getDateTimeFor(new Date(item.lts), 'reportdt') : '';
    const stopStr =item.qts > 0 ? hut.getDateTimeFor(new Date(item.qts), 'reportdt') : '';
    arr.push({snippet_link, target_link, period:item.period, startStr, stopStr, err:item.err})
  });
  return arr;
}
