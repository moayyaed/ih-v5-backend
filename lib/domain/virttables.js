    ...
    if (prop.startsWith('_On')) {
      if (prop == '_OnChange') {
        let str = rec.par_OnChange
          ? (rec.par_OnChange != '*' ? 'свойств ' + rec.par_OnChange : ' любого свойства устройства') + ', '
          : '';

        if (rec.par2_OnChange) {
          str += ' глобальной переменной ' + rec.par2_OnChange;
        }
...
async function typepropalert(dataFromTable, table, nodeid, pitem, holder) {
  ...
  /*
  const arr =  [
    {id:'T_0', message:'Норма'},
    {id:'T_1', message:'Авария'}
  ];
  */

 const arr =  [
  {id:'LoLo', thval:-42, message:'Нижнее аварийное значение'},
  {id:'Lo', thval:0, message:'Нижнее предупредительное значение'},
  {id:'Norma', message:'Норма'},
  {id:'Hi', thval:7, message:'Верхнее предупредительное значение'},
  {id:'HiHi', thval:115, message:'Верхнее аварийное значение'}
];

  console.log('V typepropalert pitem='+util.inspect(pitem))
  return tabledata.formColumnVals(pitem.columns, arr, holder);
    ...
    Object.keys(extProps).forEach(prop => {
      if (!typeItem.props[prop]) {
        ...
        const item = getItem(prop, title, 'Scene Parameter Number', {
          ...
          path: domaindata.getScenescriptPath(scene)
        });
        result.push(item);
      }
    });
...
async function devicescene(dataFromTable, table, nodeid, pitem, holder) {
  ...
  function form(devsceneSet, child) {
    devsceneSet.forEach(scene => {
      const sceneListItem = liststore.getItemFromList('sceneList', scene);
      const scenePath = domaindata.getScenescriptPath(scene);
...
async function pluginlog(data, table, nodeid, pitem, holder) {
  ...
  const arr = await holder.logconnector.getLog(
    'pluginlog',
    { unit: { $in: unitArr } },
    { limit: 1000, sort: { ts: -1 } }
  );
  arr.forEach(item => {
...
async function devicelog(data, table, nodeid, pitem, holder) {
  ...
  if (didArr.length > 1 || domaindata.isGlobal(didArr[0])) {
    arr = await holder.logconnector.getLog('devicelog', { did: { $in: didArr } }, { limit: 1000, sort: { ts: -1 } });
  } else {
  ...
  arr.forEach(item => {
    ...
    // item.valstr = item.prop ? getPropValueStr(item.did, item.prop, item.val) : '';
    item.realtime_ts = getTsStr(item);
  });
  return arr;

  /*
  function getPropValueStr(did, prop, val) {
    const dobj = holder.devSet[didArr[0]];
    return dobj ? dobj.formatValue(prop, val) : '';
  }
  */
...
async function snippetsRun(data, table, nodeid, pitem, holder) {
  ...
  Object.keys(holder.snippetSet).forEach(did => {
    ...
    arr.push({snippet_link, target_link, period:item.period, startStr, stopStr, qtsStr, errstr:item.error, resets: item.resets})
  });
  return arr;
}
