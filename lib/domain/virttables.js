...
async function sysdevicestree(data, table, nodeid, pitem, holder) {
  ...
  const arr = data[table].map(item => {
    ...
    const log_link = {title:'Лог процесса', path:'dev/sysdevices/sysdeviceview/'+item.id+'/tabProcessLog'}
    // dev/sysdevices/sysdeviceview/__UNIT_mainprocess/tabProcessLog
    return { id: item.id, module:m, title, stateStr, memrss, memheap, memhuse, laststartStr, log_link };
  });
...
async function devicelog(data, table, nodeid, pitem, holder) {
  ...
  if (didArr.length > 1 || domaindata.isGlobal(didArr[0])) {
    arr = await logconnector.getLog('devicelog', { did: { $in: didArr } }, { limit: 1000, sort: { ts: -1 } });
  } else {
...
async function alertlog(data, table, nodeid, pitem, holder) {
  // Пока все беру из  alerts
  const docs = await holder.dm.dbstore.get('alerts', {});
  const arr = [];

  docs.sort(hut.byorder('tsStart')).forEach(item => {
    item.id = item._id;
    item.stateStr = item.tsStop > 0 ? 'stopped: ' + item.reason : 'active';
    item.tsStartStr = getTsStr(item.tsStart);
    item.tsStopStr = getTsStr(item.tsStop);
    item.tsAckStr = getTsStr(item.tsAck);
    item.rowbutton = item.tsAck
      ? { title: 'Снять квитирование', command: 'api_deack_alert' }
      : { title: 'Квитировать', command: 'api_ack_alert' };
