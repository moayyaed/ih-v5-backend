...
async function pluginlog(data, table, nodeid, pitem, holder) {
  ...
  const arr = await holder.dm.getLog('pluginlog', { unit: { $in: unitArr } }, {limit:1000, sort:{ts:-1}});
  arr.forEach(item => {
...
async function devicelog(data, table, nodeid, pitem, holder) {
  ...
  if (didArr.length > 1 || domaindata.isGlobal(didArr[0])) {
    ...
    arr = await holder.dm.getLog('devicelog', { did: { $in: didArr }} , {limit:1000, sort:{ts:-1}});
  } else {
