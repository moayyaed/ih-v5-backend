...
async function schedrulestree(data, table, nodeid, pitem, holder) {
  ...
  for (const doc of data[table]) {
    ...
    let laststart = 0;
    if (rec) {
      willstart = rec.willstart_ts;
      laststart = rec.laststart_ts;
    }
    arr.push({ id, title: doc.title, willstart, laststart });
  }

  return arr.sort(hut.byorder('willstart')).map(item => {
    const willstartStr = getTimeStr(item.willstart);
    const laststartStr = getTimeStr(item.laststart);
    const sched_link = { title: item.title, path: domaindata.getSchedrulePath(item.id) };
    return { ...item, sched_link, willstartStr, laststartStr };
  });
}
...
async function devicelog(data, table, nodeid, pitem, holder) {
  ...
  console.log('VIRT devicelog didArr=' + util.inspect(didArr));
  ...
  arr.forEach(item => {
    ...
    item.sender = item.sender && typeof sender == 'object' ? JSON.stringify(item.sender) : item.sender || '';
