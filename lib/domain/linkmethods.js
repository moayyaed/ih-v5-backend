...
async function get(query, holder) {
  ...
  switch (query.id) {
    case 'devicelink':
      ...
      // if (query.dialogid == 'visitemsAndAny') return getElementlink(query, dm, holder);
      if (query.dialogid == 'visitemsAndAny')
    ...
    case 'elementlinkforset':
        // return query.root == 'anydev' ? getAllProps(query, dm, holder) : getElementlink(query, dm, holder);  
        return getElementlinkForSet(query, dm, holder);
    case 'setvalue':
    ...
    case 'imagegrid':
      return dm.getImagegrid(query.nodeid);
...
async function getElementlinkForSet(query, dm, holder) {
  const res = query.root == 'anydev' ? await getAllProps(query, dm, holder) : await getElementlink(query, dm, holder);  
  return res.filter(item => item.prop.indexOf('#')<0);  // state#string - исключить такие поля
  // return res.filter(item => item.prop.indexOf('#')<0);  // state#string - исключить такие поля
}
...
async function getAllProps(query, dm, holder) {
  ...
  const properties = arr.sort().map(prop => getItem(did, prop, 'Свойство ' + prop));
...
async function getAllCmd(dm, holder) {
  ...
  const did = '__device';
  const properties = cmdProps.sort().map(prop => getItem(did, prop, 'Команда ' + prop));
  return { data: { properties } };
}
...
async function getCmdlink({ nodeid }, dm, holder) {
  ...
  const properties = dobj.getCommands().sort().map(prop => getItem(did, prop, domaindata.getDeviceDn(did) + '.' + prop, dobj.dn, dobj.getPropTitle(prop)));
  return { data: { properties } };
}
...
async function getElementlink({ nodeid, root, anchor }, dm, holder) {
  ...
  if (nodeid.startsWith('gl')) {
    ...
    const prop = item.dn;
    properties = [getItem(did, prop, 'globals.' + prop, item.dn, item.name)];
  } else if (nodeid.startsWith('__syslocal_')) {
    const prop = nodeid.substr(11);
    properties = [getItem(nodeid, prop, 'syslocal.' + prop, nodeid)];
    ...
    const prop = item.dn;
    properties = [getItem(did, prop, 'locals.' + prop, item.dn, item.name)];
    ...
    did = nodeid;
    // const dn = domaindata.getDeviceDn(did);
    const arr = dobj.getPropsForVislink();

    properties =arr
      .sort()
      .map(prop => getItem(did, prop, dobj.dn + '.' + prop, dobj.dn, dobj.getPropTitle(prop)));

    domaindata.getListAsArray('deviceStaticPropList').forEach(item => {
      const prop = item.id;
      properties.push( getItem(did, prop, dobj.dn + '.' + prop, dobj.dn, 'Статическое: '+item.title));
    });  
  }
  return { data: { properties } };
}
...
function getItem(did, prop, title, dn, title2) {
