...
const specsubtree = {
  ...
  docs: getDocTree,
  pluginfiles: getPluginFilesTree
};

async function getPluginFilesTree(query, dm) {
  // {"method":"get","type":"subtree","id":"pluginfiles","nodeid":"email","userId":"admin","__expert":1}
  // if (!query || !query.__expert) throw {message: appconfig.getMessage('InsuffRights')};

  //  query.nodeid='basic' || 'plugin_modbus'
  let pluginName = query.nodeid;
  if (pluginName.startsWith('plugin_')) pluginName = pluginName.substr(7);

  // Считать папку плагина с вложенными файлами
  const folder = appconfig.getThePluginPath(pluginName);
  const fileItems = fut.readFolderSyncExt(folder, { exclude: ['node_modules'] }); //

  // { filename: 'v6', folder: 1, parent: 'v5' },
  // { filename: 'test.json', folder: 0, parent: 'v5/v6' }

  // В дереве для id / => * (слеш не проходит)
  const b_array = [];
  const l_array = [];
  fileItems.forEach(item => {
    const parent = hut.replaceAll(item.parent ? pluginName + '/' + item.parent : pluginName, '/', '*');
    const id = parent + '*' + item.filename;
    const tObj = { id, title: item.filename, parent, folder: item.folder };
    if (item.folder) {
      b_array.push(tObj);
    } else l_array.push(tObj);
  });
