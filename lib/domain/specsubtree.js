...
async function getPluginFilesTree (query, dm) {
  ...
  const pluginName = query.nodeid;
  const folder = appconfig.getThePluginPath(pluginName);
 
  const fileItems = fut.readFolderSyncExt(folder,  {exclude:['node_modules']}); // 

  //  { filename: 'v6', folder: true, parent: 'v5' },
  // { filename: 'test.json', folder: false, parent: 'v5/v6' }

  const b_array = [];
  const l_array = [];
  
  fileItems.forEach(item => {
    // const parent = (pluginName+'/'+item.parent).replaceAll('/', '*');  replaceAll(astr, search, replacement)
