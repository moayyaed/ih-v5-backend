...
async function getPluginFilesTree (query, dm) {
  ...
  // 
  const fileItems = pluginutil.getPluginFiles(query.nodeid) ; // {id, title, parent, folder}
  console.log('fileItems = '+util.inspect(fileItems))
  const b_array = [];
  const l_array = [];
  fileItems.forEach(item => {
    if (item.folder) {
      b_array.push(item)
    } else l_array.push(item)
  })

  console.log('b_array = '+util.inspect(b_array))
  console.log('l_array = '+util.inspect(l_array))

  const data = treeutil.makeTreeWithLeaves(b_array, l_array);
  console.log('getPluginFilesTree data='+util.inspect(data))
  return data;
  // return files.map(file => ({id:file, title:file}));
  

  /*
  return [
    {id:'index.js', title:'index.js '+JSON.stringify(query), parent:0},
    {id:'app.js', title:'app.js', parent:0}
  ]
  */
