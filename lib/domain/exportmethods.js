...
async function exportChannels(query, holder) {
  ...
  // Если есть share_node_folder_fields для папок - их тоже добавить
  const plugin = hut.removeLastNumFromStr(nodeid);
  let node_fields = appconfig.getPluginManifestProp(plugin, 'share_node_folder_fields');
  ...
  for (const item of treeItems) {
    ...
    if (onedoc) {
      if (!item.folder) {
        docs.push({ ...formOneObj(onedoc), parent_title: item.parent_title });
      } else if (onedoc.foldertype == 'node') {
          docs.push({ ...formNodeObj(onedoc), folder_title: item.title, parent_title: item.parent_title });
        } else {
          docs.push({ folder_title: item.title, parent_title: item.parent_title });
        }
    } else {
      console.log('WARN: exportChannels: Not found devhard record id=' + item.id + '. Skipped');
    }
    /*
    if (!item.folder) {
      // Для записей - листьев вытащить данные из таблицы
      ...
      // Для папок - только folder_title, foldertype, parent_title
      console.log('FOLDER: item='+util.inspect(item))
      docs.push({ folder_title: item.title, foldertype:item.foldertype, parent_title: item.parent_title });
      // Возможно, еще share_ !!! 
    }
    */
  }

  // Добавляется позднее, так как эти поля для листьев не заполняются!
  if (Array.isArray(node_fields)) {
    node_fields.forEach(field => {
      if (!columns.includes(field)) columns.unshift(field);
    });
  }
  columns.unshift('foldertype');
  ...
  function formNodeObj(dataItem) {
    const newo = {foldertype:'node'};
    if (Array.isArray(node_fields)) {
      node_fields.forEach(field => {
        const val = dataItem[field] != undefined ? dataItem[field] : '';
        newo[field] = valueByType(field, val);
