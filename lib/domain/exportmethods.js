...
function docsFromTree(tree) {
  ...
  function traverse(tarr) {
    for (const item of tarr) {
      if (item.children) {
        result.push({ folder: 1, id: item.id, title: item.title, parent, parent_title });
        parent = item.id;
        parent_title = item.title || item.id;
        traverse(item.children);
      } else {
        result.push({ id: item.id, title: item.title, parent, parent_title });
      }
    }
  }
}
