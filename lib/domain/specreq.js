...
function isSpecPopup(id) {
  ...
  return id == 'channels' || id.startsWith('plugin') || id.startsWith('dyn_');
}
...
async function getSpecPopup(query, holder) {
  const { id } = query;
  if (id == 'channels') return getChannelsPopup(query, holder);
...
async function getChannelsPopup(query, holder) {
  const { navnodeid } = query;
  let channelsPopup;
  // popup в зависимости от плагина
  if (navnodeid) {
    const plugin = appconfig.getPluginIdFromUnitId(navnodeid);
    // console.log('getChannelsPopup navnodeid=' + navnodeid + ' plugin=' + plugin);
    channelsPopup = await holder.dm.getManifestItem(plugin, 'channelsPopup');
  }
  if (!channelsPopup) {
    channelsPopup = [
      { id: 'folder', title: appconfig.getMessage('NewFolder') },
      { id: 'node', title: appconfig.getMessage('NewChannel') }
    ];
  }
  return { data: channelsPopup };
}
