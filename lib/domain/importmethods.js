...
async function processChannels(docs, unit, holder) {
  ...
  docs.forEach(doc => {
    ...
    if (doc.folder_title) {
      ...
      newdocs.push({ ...formFolderObj(doc), _id, unit, parent, order, folder: 1, chan: doc.folder_title });
    } else {
      // Канал
  ...
  const newLinks = [];
  newdocs.forEach(doc => {
    if (!doc.folder && doc.did && doc.prop) {
      newLinks.push({ ...doc, $set: { did: doc.did, prop: doc.prop } });
    }
  });
  ...
  // Если добавлены привязки - обновить
  // if (newLinks.length) this.dm.emit('updated:devhard', newLinks);

  return { response: 1, refresh: true };

  function formFolderObj(dataItem) {
    const newo = {};
    const foldertype = dataItem.foldertype;
    if (foldertype) newo.foldertype = foldertype;

    if (foldertype == 'node' && Array.isArray(node_fields)) {
