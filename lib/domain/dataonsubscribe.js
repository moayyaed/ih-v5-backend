...
function formChangedDeviceLog(logobj, uppobj, contextId) {
  ...
  // 09.12.2021  - отправляется с указанием устройства - контекста, а не id виджета
  // {d0389: [{title: "09.12 16:05:48.383", message: "Команда: Включить user: Admin"}]}
  // widgettype: "devicelog"  - добавляется на верхнем уровне (wsserver)

  if (uppobj.devicelog[did]) return { [did]: logobj.data };

  if (contextId && did == contextId) {
    return { [did]: logobj.data };
...
async function formContainerSubConditionWithContext(mes, holder) {
  ...
  if (needContext) {
    ...
    if (mes.frames) {
      const frame = await getFirstFrame(mes.frames); // { vc, device, multichart_id, timelinechart_id, journal_id, alertjournal_id }
  ...
  // console.log('WARN:ajs.size=' + ajs.size+ ' res='+util.inspect(res));
  return res;
}
