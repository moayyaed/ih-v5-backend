...
class Datamanager extends EventEmitter {
  ...
  async copyTo(body) {
    ...
    if (body.type == 'tree') {
      const res = await updater.copyToTree(body);

      // Нужно еще emit и сброс кэша, если есть зависимости
      // Сброс кэша для таблиц дерева, имена внутри payload:{devices:{nodes:[],folders[]},}
 ...
 */
  async insert(body) {
    ...
    if (body.type == 'tree') {
      const res = await updater.insertTree(body);
  ...
  invalidateCacheForRequest(body) {
    ...
    console.log('nvalidateCacheForRequest start ' + util.inspect(body));
 ...
 */
  ...
  async remove(body) {
    if (body.type == 'tree') {
      ...
      const notRemoved =   await updater.removeFromTree(body);
      this.invalidateCacheForRequest(body);
      if (notRemoved.length > 0) {
        throw {err:'ERR', message:'Folder was not removed: '+notRemoved.join(',')}
      }
      return;
    }
...
async function checkParentBranch(body, needParent) {
  ...
  const filter = datautil.createIdsInFilter(Array.from(parentSet));
