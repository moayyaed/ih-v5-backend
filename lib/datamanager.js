...
class Datamanager extends EventEmitter {
  ...
  async copyTo(body) {
    if (body.type == 'tree') {
      const {res, data, reorder} = await updatetree.copyTo(body); // возвращается включая reorder
  ...
  async insert(body) {
    if (body.type == 'tree') {
      const res = await updatetree.insert(body);  // вернули массив добавлений
      dataformer.updateTreeguide(body, res); // Передаем изменения в словарь дерева
 ...
 */
  ...
  async remove(body) {
    if (body.type == 'tree') {
      ...
      console.log('removed = '+util.inspect(removed))
