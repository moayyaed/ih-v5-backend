...
async function getRecordByForm(id, nodeid) {
          ...
          for (const item of formMetaData[cell.id]) {
            if (item.type == 'table') {
              if (item.prop == 'devicecommonTable') {
                ...
                data[cell.id][item.prop] = await getTypeTable();
            ...
            } else if (item.prop == 'test') {
                data[cell.id][item.prop] = {"id":"2", "title":"Два"};
  ...
  async function getTypeTable() {
    ...
    return hut.objectToArray(pObj, 'prop');
  }
...
async function getDroplistItem(listname, key) {
  console.log('getDroplistItem listname '+listname);
  if (Array.isArray(listname)) return {"id":"2", "title":"Два"};
