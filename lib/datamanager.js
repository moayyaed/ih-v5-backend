...
const meta = require('../sysbase/meta_tree_devices.js');
...
async function remove(body) {
  ...
  // Здесь нужно создать filter c модификатором $in?
  // Если одна запись -  просто фильтр по _id
  const filter = {};
  let multi = false;
  ...
  //
  const numRemoved = await remove(desc.collection, filter, { multi });

  if (numRemoved != arr.length)  throw {error:'ERRREMOVE', message:'Expected "id" prop in payload!'};
