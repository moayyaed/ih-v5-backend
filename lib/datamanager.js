...
async function getList(id) {
  if (liststore.hasList(id)) {
    console.log('from liststore');
    return liststore.getListAsArray(id);
  }

  // Нужно загрузить
  const listdesc = descriptor.getDescItem('list', id);
  const desc = descriptor.getTableDesc(listdesc.table);
  const data = await dbstore.get(desc.collection, {}, { fields: { _id: 1, name: 1 } });
...
async function getRecordByForm(id, nodeid) {
    ...
    for (const cell of formMetaData.grid) {
      if (cell.id && formMetaData[cell.id]) {
        ...
        for (const item of formMetaData[cell.id]) {
          if (item.type == 'table') {
            data[cell.id][item.prop] = await getTableValue();
          } else if (cell.table && dataFromTable[cell.table] && dataFromTable[cell.table][0][item.prop] != undefined) {
            const val = dataFromTable[cell.table][0][item.prop];
  ...
  async function getTableValue() {
    ...
    const arr = trec[0].aux ? trec[0].aux.map(aitem => aitem) : [];
