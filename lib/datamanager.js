...
async function getRecordByForm(id, nodeid) {
  ...
  const data = await dbstore.get(desc.collection, { _id: nodeid });
  return { data };
}
...
async function loadMeta(type, id) {
  let data;
  if (type == 'components') {
    let folder = path.resolve(appconfig.get('sysbasepath'), type);
    data = await loadAndTranslateJsonFromFolder(folder);
  } else if (type == 'form') {
    let filename = path.resolve(appconfig.get('sysbasepath'), 'forms', id + '.json');
    data = await loadAndTranslateJsonFile(filename);
  } else {
