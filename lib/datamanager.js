...
async function getRecordByForm(id, nodeid) {
  ...
  if (formMetaData.grid) {
    ...
    for (const cell of formMetaData.grid) {
      if (cell.id && cell.table && formMetaData[cell.id]) {
        if (dataFromTable[cell.table]) {
          // Выбрать props, если в записи нет - добавить
          data[cell.id] = {};
          formMetaData[cell.id].forEach(item => {
            if (item.prop) {
              if (dataFromTable[cell.table][0][item.prop] != undefined) {
                const val = dataFromTable[cell.table][0][item.prop]
                if (item.type == 'droplist') {
                  data[cell.id][item.prop] = liststore.getItemFromList(item.data, val);
                } else {
