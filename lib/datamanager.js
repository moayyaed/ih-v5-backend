...
class Datamanager extends EventEmitter {
  ...
  async qRecords(table) {
    ...
    try {
      if (this.datagetter.isVirttable(table)) {
        ...
        docs = await this.get(table, {}, { fields: { _id: 1 } });
      }
    } catch (e) {
      console.log('ERROR: qRecords table=' + table + ': ' + util.inspect(e));
    }
    console.log('DM QRECORDS  ' + table + ' LEN=' + docs.length);
    return docs.length;
  }
  ...
  async getLog(table, filter) {
    if (!table) table = 'userlog';
    return this.bigstore.get(table, filter, { order: 'ts' });
  }
