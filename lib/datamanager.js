...
class Datamanager extends EventEmitter {
  constructor() {
    ...
    this.logstore = require('./dbs/tingodb/dbstore');
  ...
  async start({ datagetter, datamaker, isNoCache }) {
    ...
    // tables: store:'db' - nedb; store:'logdb' - tingo; store:'none' - virt
    ...
    for (const table of Object.keys(tables)) {
      ...
      } else if (tables[table].store == 'logdb') {
        logdb[table] = tables[table];
      }
    }
    ...
    // this.logstore - tingodb --------------------
    // console.log('BEFORE logstore start '+util.inspect(logdb))
    // this.logstore.start(logdb, appconfig.get('logbasepath') );
  }
   ...
   */
  async getMeta(query) {
    console.log('dm.getMeta '+util.inspect(query))
    let { type, id } = query;
    // return type == 'form' && specform.isSpecForm(id) ? specform.getForm(query, this) : loadsys.loadMeta(type, id);
    return this.datagetter.isSpec(type, id) ? this.datagetter.getSpecMeta(query, this) : loadsys.loadMeta(type, id);
  }
  ...
  insertToLog(table, mesObj) {
    ...
    // this.logstore.insert(table, docs);
  ...
  getMetaCacheIds(query) {
    console.log('dm.getMetaCacheIds '+util.inspect(query))
    let { type, id } = query;
    // return id && specform.isSpecForm(id) ? specform.getMetaCacheIds(query) : ['META', type, id];
    return id && type == 'form' && this.datagetter.isSpec(type, id) ? this.datagetter.getMetaCacheIds(query) : ['META', type, id];
  }

  getCacheKey(query) {
    return this.getCacheIds(query).join('_');
  }
