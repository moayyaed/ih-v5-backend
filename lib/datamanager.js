...
class Datamanager extends EventEmitter {
  ...
  async removeDocs(table, docs, beforeRemove) {
    ...
    if (beforeRemove) {
      for (const doc of docs) {
        // await datamaker.checkDocCanBeRemoved(table, doc);
...
async function getMeta(query) {
  console.log('getMeta( ' + util.inspect(query));
  ...
  if (!data) {
    console.log('Plugin NOT MetaData' +id)
    console.log('getMeta loadsys ' + type + ' ' + id + ' ' + nodeid);
...
function isPluginMetaData(id) {
  ...
  const spec = ['formPluginCommon', 'channelview', 'formPluginChannelsTable'];
