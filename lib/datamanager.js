...
async function loadTree(id) {
  ...
  if (data.length > 1) {
    ...
    // Создать папку только виртуально 
...
async function insert(body) {
  ...
  if (body.type == 'tree') {
    await checkParentBranch(body, docsToWrite, true);
  }
...
async function update(body) {
  ...
  if (body.type == 'tree') {
    await checkParentBranch(body, docsToWrite, false);
  }
 ...
 */
async function checkParentBranch(body, docs, needParent) {
  ...
  if (needParent || doc.parent != undefined) {
    ...
    const filter = Object.assign({}, tableDesc.filter || {}, { id: doc.parent });
