...
function processBodyForUpdateMethods(body) {
  if (!body.data) throw { error: 'ERRDATA', message: 'No data!' };
...
function getUpdateDesc(body) {
  const { type, id } = body;
  const partid = body.partid || id; // partid - поддерево
  const desc = getDescItem(type, partid);
  if (Array.isArray(desc)) throw { error: 'ERRINSERT', message: 'Need partid for composite tree!' };
  return desc;
}

function getUpdateTableDesc(desc) {
  // Найти таблицу для записи
  const tableDesc = getDescItem('table', desc.table);
  if (!tableDesc.collection) throw { error: 'SOFTERR', message: 'Not found collection name for table=' + desc.table };
}
