...
async function remove(body) {
  ...
  if (body.options && body.options.leaf) {
    ...
    const subtree = treeutil.findNodeById(tree, id); 
    console.log('SUBTREE by id=' + id);
    console.log(util.inspect(subtree, null, 4));

    // Разделить на ветки и листья, так как это разные таблицы
    const b_arr = [id]; // Удаляемая ветка
    const l_arr = [];
    if (subtree) {
      const siblingArr = treeutil.findAllDescendants(subtree);
...
async function checkParentBranch(body, needParent) {
  ...
  // TODO Для массива нужен Promise.all
  // } else
