...
class Datamanager extends EventEmitter {
  ...
  async getCachedData(query, prepFn) {
    console.log('getCachedData query='+util.inspect(query))
    const key = this.getCacheKey(query);
  ...
  async getContent(name, query) {
    console.log('DM getContent '+name)
  ...
  getCacheIds(query) {
    ...
    if (query.method == 'getdocs') return this.getDocsCacheIds(query);
  ...
  getDocsCacheIds(query) {
    const ids = ['type', 'id', 'lang', 'conf'];
    const res = [];
    for (let i = 0; i < ids.length; i++) {
      if (!query[ids[i]]) return res;
      res.push(query[ids[i]]);
