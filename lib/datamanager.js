...
class Datamanager extends EventEmitter {
  ...
  async start({ datagetter, datamaker, isNoCache }) {
    ...
    // tables: store:'db' - nedb; store:'bigdb' - tingo; store:'none' - virt
    ...
    // this.bigstore - tingodb --------------------
    try {
      this.bigstore = require('./dbs/tingodb/dbstore');
      this.bigstore.start(
        {
          devicelog: { ensureIndex: 'tsid', expireAfterSeconds: 86400 },
          mainlog: { ensureIndex: 'tsid' },
          pluginlog: { ensureIndex: 'tsid', expireAfterSeconds: 86400 }
        },
        appconfig.get('bigbasepath')
      );

      this.logformer.start(this.bigstore);
    } catch (e) {
      console.log('ERROR: datamanager ' + util.inspect(e));
    }
  ...
  async insertToLog(table, mesObj) {
    if (!table) table = 'mainlog';
    /*
    ...
    docs.forEach(item => {
      ...
      item.ats = 0;
      // ts++; // сдвиг на 1 мсек  Нельзя все записи с одним ts??
    });
    */
   this.logformer.exec(table, Array.isArray(mesObj) ? mesObj : [mesObj]);

    // this.bigstore.insert(table, docs);
    // this.insertDocs(table, docs);
  }

  async getLog(table, filter = {}, opt = {}) {
    if (!table) table = 'mainlog';
    console.log('DM: getLog opt.limit='+opt.limit)
    return opt.limit ?  this.bigstore.getLimited(table, filter, opt) : this.bigstore.get(table, filter, { order: 'ts' }) ;
  }
