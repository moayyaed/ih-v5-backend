...
const treeformer = require('./apptools/treeformer');
...
async getCachedTree(query) {
  console.log('DM getCachedTree query ='+util.inspect(query))
  const {id} = query;
  const cachedObj = await this.getCachedData({ type: 'tree', id }, treeformer.getTree);
  console.log('cachedObj = '+util.inspect(cachedObj))
...
async getCachedSubTree(query) {
  ...
  const cachedObj = await this.getCachedData({ type: 'subtree', id, nodeid }, treeformer.getSubTree);
  if (!cachedObj) throw { error: 'SOFTERR', message: 'No cached subtree ' + id };
  return cachedObj.data; // Это массив
}

async getImagegrid(nodeid) {
  const treeData = await treeformer.getDataFromTree('images', nodeid, this);
  ...
  async getTree(treeId, filter = {}, opt = {}) {
    ...
    const data = await this.getCachedData({ method, type: 'tree', id, ...filter }, treeformer.getTree);
