...
async function start() {
  ...
  // Списки храним в отдельном объекте liststore, но desc - вместе
}
...
async function getList(id) {
  if (liststore.hasList(id)) {
    return { data: liststore.getListAsArray(id) };
  }

  // Нужно загрузить
  const listdesc = descriptor.getDescItem('list', id);
  const desc = descriptor.getTableDesc(listdesc.table);
  const projection = getFieldProjection(listdesc.propmap);
...
async function getRecordByForm(id, nodeid) {
  ...
  try {
    if (formMetaData.grid) {
      ...
      for (const cell of formMetaData.grid) {
        if (cell.id && formMetaData[cell.id]) {
          data[cell.id] = {};

          for (const item of formMetaData[cell.id]) {
            if (item.type == 'table') {
              if (item.prop == 'devicecommonTable') {
                data[cell.id][item.prop] = await getDeviceTable();
              } else if (item.prop == 'typepropsTable') {
                data[cell.id][item.prop] = await getTypeTable(item.columns);
              } else if (item.prop == 'channelTable') {
                data[cell.id][item.prop] = await getChannelTable();
    ...
    throw { error: 'SOFTERR', message: 'Unable prepare data for ' + id + util.inspect(e) };
  }
  return { data };
  ...
  async function getTypeTable(columns) {
    console.log('columns '+util.inspect(columns));

    const dropLists = columns ? columns.filter(el => el.type == 'droplist') : [];
    console.log('dropLists '+util.inspect(dropLists));
    ...
    arr.forEach(item => {
      dropLists.forEach((col, idx) => {
        const prop = col.prop;
        if (item[prop] != undefined) item[prop] = dropLists[idx].data.find(el => el.id == item[prop]);
      });
    });

    /*
    return hut.objectToArray(pObj, 'prop').forEach(item => {
      dropLists.forEach((col, idx) => {
        if (item[col] != undefined) item[col] = dropLists[idx].data.find(el => el.id == item[col]);
      });
    });
    */
   return arr;
...
async function getRecordByForm(id, nodeid) {
  ...
  const fields = formMetaData.data.default || {};

  let data = [];
  // М б table или table+reltable - если нужна связь с подчиненной таблицей
  if (formMetaData.data.table) {
    const desc = descriptor.getTableDesc(formMetaData.data.table); 
    data = await dbstore.get(desc.collection, { _id: nodeid }, { fields: getFieldProjection(fields) });

  } else if (formMetaData.data.keytable && formMetaData.data.reltable) {
    // Получить ключевые данные из keytable, а основные из reltable
    const kdesc = descriptor.getTableDesc(formMetaData.data.keytable);
    const key = formMetaData.data.key;
    const keyrec = await dbstore.get(kdesc.collection, { _id: nodeid }, { fields: {} });
    if (keyrec.length>0 ) {
      const rdesc = descriptor.getTableDesc(formMetaData.data.reltable);
      // data = await dbstore.get(rdesc.collection, { [key]: keyrec[0][key] }, { fields: getFieldProjection(fields) });
    
      data = await dbstore.get(rdesc.collection, { [key]: nodeid }, { fields: getFieldProjection(fields) });
    }
  } else {
    throw {error:'SOFTERR', message: 'Not found table for form:' + id }
  }
