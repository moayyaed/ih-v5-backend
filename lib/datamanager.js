...
class Datamanager extends EventEmitter {
  ...
  async start() {
    ...
    // ВРЕМЕННО - ПЕРЕНОС ПАПОК из lists в images.db
    const listdocs = await this.dbstore.get('lists', {});
    const imgdocs = listdocs.filter(doc => doc._id.startsWith('img'));
    for (const doc of imgdocs) {
      // Добавить в images
      await this.dbstore.update('images', { _id: doc._id }, { ...doc, folder: 1 }, { multi: false, upsert: true });
    }
    // Убрать из list??
    //
  }
  ...
  async checkAndCreateRequiredRecords(table) {
    ...
    const docsToWrite = [];
    if (!Array.isArray(reqRecs)) {
      reqRecs = [reqRecs];
    }

    for (const rec of reqRecs) {
      if (rec.check && rec.add) {
        const doc = await this.dbstore.findOne(desc.collection, rec.check);
        if (!doc) docsToWrite.push(rec.add);
      } else {
        console.log(
          'ERROR: table:' + table + '. requiredRecords must have "check" and "add" props. requiredRecords skipped.'
        );
      }
    }
