...
class Datamanager extends EventEmitter {
  ...
  async getCachedData(query, prepFn) {
    ...
    // if ((query.method == 'getmeta' && nocacheMeta[query.id]) || nocache[query.id] || query.type == 'subtree') {
  ...
  async insertDocs(table, docs) {
    ...
    try {
      ...
      console.log('ERROR: datamanager.insertDocs table='+table+', docs = ' + util.inspect(docs) + '. ' + util.inspect(e));
    }
  ...
  /**
   * Синхронизация таблицы с папкой (сценарии, обработчики)
   *
   * @param {String} table - имя таблицы
   * @param {function} syncFunc - функция синхронизации - выполняет обработку файлов и их сличение
   *
   *  Результат syncFunc - массив измененных док-тов - разделяется на добавленные и измененные
   *  Выполняется добавление и изменение документов в таблицу
   */
