...
class Datamanager extends EventEmitter {
  ...
  async start() {
    ...
    // customtables - также загрузить
    const customtables = await this.dbstore.get('customtables');
    customtables.forEach(item => this.createCustom(item._id));
  }
  ...
  async createCustomTable(name) {
    if (this.dbstore.hasCollection(name)) return;
    // Добавить в таблицу customtables - чтобы загрузить на старте
    this.createCustom(name);
    return this.insertDocs('customtables', [{ _id: name }]);
  }
