...
const logformer = require('./dbs/logformer');
...
class Datamanager extends EventEmitter {
  constructor() {
    ...
    this.cache = cache;
    this.logformer = logformer;
  }

  async start({ datagetter, datamaker, isNoCache, logconnector}) {
    ...
    this.logconnector = logconnector && logconnector.isActive() ? logconnector : ''; // Объект, обеспечивающий свзь с БД для записи логов
    // this.logconnector = ''; // Объект, обеспечивающий свзь с БД для записи логов. Цепляется позже, может и не быть

    this.logformer.start(logconnector);
  ...
  async qRecords(table) {
    ...
    try {
      ...
      const desc = descriptor.getDescItem('table', table);
      return this.dbstore.count(desc.collection, {});
    } catch (e) {
  ...
  async insertToLog(table = 'mainlog', mesObj) {
    if (!this.logconnector || !this.logconnector.isActive()) return;
  
    const docs = await this.logformer.exec(table, Array.isArray(mesObj) ? mesObj : [mesObj]);
    this.logconnector.write(table, docs);
  }


  async getLog(table = 'mainlog', filter = {}, opt = {}) {
    if (!this.logconnector || !this.logconnector.isActive()) return [];

    return this.logconnector.read(table, filter, opt);
    // return opt.limit ?  this.dbstore.getLimited(table, filter, opt) : this.dbstore.get(table, filter, { order: 'ts' }) ;
  }
  ...
  createInvalidateList() {
    ...
    singleTrees.forEach(treeId => {
      ...
      const addItem = (table, id) => {
        ...
        result[table].push(this.getCacheKey({ type: 'tree', id }));
      };
