...
const treeGet = require('./apptools/treeGet');
...
class Datamanager extends EventEmitter {
  ...
  createICollections(app) {
    const cTypes = this.dbstore.createICollection(app, 'types');
    descriptor.addITableDesc(cTypes, cTypes);

    const cDevices = this.dbstore.createICollection(app, 'devices');
    descriptor.addITableDesc(cDevices, cDevices);
  }
  ...
  async getCachedTree(query) {
    const { id, nodeid } = query;
    const cachedObj = await this.getCachedData({ type: 'tree', id, nodeid }, treeGet.getTree);
    if (!cachedObj) throw { error: 'SOFTERR', message: 'No cached tree ' + id };
    return cachedObj.data[0];
    // return cachedObj.data[0]; // Это массив, берем первый узел (элемент массива)
  }

  async getCachedSubTree(query) {
    const { id, nodeid } = query;
    const cachedObj = await this.getCachedData({ type: 'subtree', id, nodeid }, treeGet.getSubTree);
    if (!cachedObj) throw { error: 'SOFTERR', message: 'No cached subtree ' + id };
    return cachedObj.data; // Это массив
  }

  async getImagegrid(nodeid) {
    const treeData = await treeGet.getDataFromTree('images', nodeid, this);
    const data = treeData.map(item => item.id);
    return { data };
  }

  async getIdsFromTree(treeName, nodeid) {
    const treeData = await treeGet.getDataFromTree(treeName, nodeid, this);
    const data = treeData.map(item => item.id);
    return data;
  }

  async tabledataFit(table, columns, dataArr, nodeid) {
    // Обработка полей типа droplist, подготовить списки
    const dropLists = await this.formDroplists(columns, nodeid);

    let order;
    // Уточнить состав полей, сформировать объекты для droplist
    const tdata = dataArr.map((item, idx) => {
      item.prop = item.prop || item.id;
      const row = { id: item._id || item.id || String(idx + 1) };

      columns.forEach(col => {
        if (col.prop == 'order') order = 1;
        if (col.prop.startsWith('__')) {
          // Служебные поля для чтения
          row[col.prop] = datautil.derivativeValue(col, item);
        } else if (col.prop == 'tags') {
          row[col.prop] = item[col.prop] ? item[col.prop].join(', ') : '';
        } else if (col.type == 'link') {
          row[col.prop] = this.datagetter.formLinkObj(table, col, item, item[col.prop]);
        } else if (col.type == 'droplist') {
          row[col.prop] = formDroplistValue(col.prop, item[col.prop]);
        } else if (col.type == 'smartbutton2') {
          row[col.prop] = this.datagetter.formSmartbutton2Obj(col, item, item[col.prop]);
    ...
    return order ? tdata.sort(hut.byorder('order')) : tdata;
  ...
  async formDroplists(columns, nodeid) {
    ...
    for (let item of larr) {
      if (item.data && Array.isArray(item.data)) {
        ...
        const listdata = await this.datagetter.getDroplist({ id: item.data, nodeid }, this); // => {data:arr}
  ...
  async getTree(treeId, filter = {}, opt = {}) {
    ...
    const data = await this.getCachedData({ method, type: 'tree', id, ...filter }, treeGet.getTree);
  ...
  async updateDocs(table, docs, beforeUpdate) {
    ...
    if (table.startsWith('i_')) {
      // i_applehomekit_devices
      const xx = table.split('_');
      if (xx.length == 3) {
        this.emit('updated:integration:' + xx[2], xx[1], docs);
      }
    } else this.emit('updated:' + table, docs);
  }
  ...
  getCacheIds(query) {
    if (query.method == 'getmeta') {
      const res = this.getMetaCacheIds(query);
      return res;
    }
