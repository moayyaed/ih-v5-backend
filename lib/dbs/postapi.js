...
console.log('treeInsert treeAfterInsert res='+util.inspect(res, null, 4))
  const tree = await treeAfterInsert(res, body.id);
  if (reorder) result.reorder = reorder;
  if (tree) result.data = tree;
  console.log(' treeInsert result='+util.inspect(result))
  return result;
}
