...
module.exports = {
  start(ddo) {
    ...
    // Создать список form для получения данных и редактирования
    this.forms = {};
    if (this.descObj.table) {
      Object.keys(this.descObj.table).forEach(name => {
        const descItem = this.descObj.table[name];
        if (descItem.form) {
          this.forms[descItem.form] = { table: name, collection: descItem.collection };
        }
      });
    }
    console.log('FORMS')
  ...
  findNonexistentListsRootNodes(found) {
    const result = [];
