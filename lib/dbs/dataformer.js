...
async function get(type, id, nodeid) {
  ...
  if (cache.has(key)) {
    ...
    return type == 'tree' && id == 'devices' ? cloneDevicesWithExpanded(result) : result;
  }
...
async function getXFormData(id, nodeid) {
  ...
  const meta = await getMeta('xform', id);
...
async function getMeta(type, idext, nodeid) {
  console.log('getMeta '+type+' '+idext+' '+nodeid);
  const key = descriptor.getCacheKey(type, idext, 'meta', nodeid);
  if (cache.has(key)) return cache.get(key);

  let id = idext;
  let instance;
  if (idext && idext.indexOf('.') > 0) {
    [id, instance] = idext.split('.');
  }
  console.log('getMeta id='+id+' instance='+instance);
  let data;
  if (nodeid && type == 'form') {
    // Доработать специфичные формы. И сохранить в кэше с учетом доработки для nodeid!!

    // if (id == 'formPluginCommon') {
    if (loadsys.isPluginMetaData(id)) {
      console.log('isPluginMetaData=');
      // Загрузить из папки плагина
      const plugindata = await loadsys.loadPluginMetaData(type, id, nodeid, instance);
      if (plugindata) data = plugindata; // Иначе стандарт
    }
  }
  ...
  if (arr[0] == 'channelview') {
    if (fid && fid.startsWith('modbus')) {
      data.p3 = [
        ...
        { prop: 'useact', title: 'Use action', type: 'cb' },
        {
          prop: 'vartype',
          title: 'Type',
          type: 'droplist',
        ...
        },
        {
          ...
          data: [
            ...
            {
              id: '1',
              title: 'FC1 - read, FC5 - write: FOR BOOL',
              hide: 'data.p3.vartype.id != "bool"'
            },

            {
              id: '2',
              title: 'FC2 - read: FOR BOOL',
              hide: 'data.p3.vartype.id != "bool"'
            },
            {
              id: '3',
              title: 'FC3 - read, FC6 - write',
              hide: 'data.p3.vartype.id == "bool"'
            },
            {
              id: '4',
...
async function getRecordByForm(id, nodeid) {
  ...
  async function getData(table, item) {
    ...
    switch (item.type) {
      ...
      case 'smartbutton':
        return datautil.getSmartbuttonItem(dataFromTable[table][0]);
      default:
        return '';
    }
  }
