...
async function getRecordByForm(id, nodeid, def) {
  ...
  try {
    ...
    if (formMetaData.breadcrumbs) data.breadcrumbs = getBreadcrumbs(formMetaData.breadcrumbs, nodeid);
...
async function loadTree(id) {
  ...
  console.log('makeTreeWithLeaves(b_array '+util.inspect(b_array));
  console.log('makeTreeWithLeaves(l_array '+util.inspect(l_array));

  // Создать tree_guide, хранить результат в кэше
  const tree_guide = {};
  b_array.forEach(item => {
    tree_guide[item.id] = {id:item.id, title:item.title, parent:item.parent, table:desc.branch.table};
  });
  l_array.forEach(item => {
    tree_guide[item.id] = {id:item.id, title:item.title, parent:item.parent, table:desc.leaf.table};
  });
...
function getBreadcrumbs(treeId, nodeId) {
  console.log('getBreadcrumbs start treeId '+treeId);
  const key = descriptor.getCacheKey('tree_guide', treeId);
  const cacheObj = cache.get(key);
  if (!cacheObj) return [];
  console.log('getBreadcrumbs tree_guide '+util.inspect(cacheObj));
  ...
  console.log('breadArr = '+util.inspect(breadArr));
  return breadArr;
}
