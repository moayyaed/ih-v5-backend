...
async function getSubTree(id, nodeid) {
  ...
  const b_array = docs
    ...
    .map(item => ({ id: item._id, title: item.chan, parent: 0, component: 'channelfolder.' + nodeid }));
  const l_array = docs
    .filter(item => !item.folder)
    .map(item => ({ id: item._id, title: item.chan, parent: 0, component: 'channelview.' + nodeid }));
