...
async function getRecordByForm(id, nodeid) {
  ...
  try {
    ...
    for (const cell of formMetaData.grid) {
      ...
      for (const item of formMetaData[cell.id]) {
        ...
        } else data[cell.id][item.prop] = getEmptyValue(item.type); 
      }
    }
  } catch (e) {
  ...
  async function getData(table, item) {
    ...
    return getDroplistItem(item.data, val);
  }
  ...
  async function formTableData(item) {
    ...
    } else if (item.prop == 'chartTable') {
      tabdata = await getChartTable();
    }
  ...
  async function getTypeTable() {
    ...
    arr.forEach(item => {
      ...
      if (item.dig == undefined) item.dig=null; 
      if (item.mu == undefined) item.mu=""; 
    });
    return arr;
  }
  async function getChartTable() {
    // Подготовить табличные данные
    // ЗАГЛУШКА
    const trec = await dbstore.get('charts', { _id: nodeid });
    // props развести в массив
    const pObj = trec[0] && trec[0].props ? trec[0].props : '';
    if (!pObj) return [];
  ...
  async function getDeviceTable() {
    ...
    const dObj = trec[0] && trec[0].aux ? trec[0].aux : '';
    if (!dObj) return [];
   
    const arr = hut.objectToArray(dObj, 'prop');
    arr.forEach(item => {
      item.id = item.prop;
      if (item.min == undefined) item.min=null; 
      if (item.max == undefined) item.max=null; 
