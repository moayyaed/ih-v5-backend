...
async function getMeta(type, id, nodeid) {
  ...
  if (nodeid && type == 'form') {
    ...
    if (loadsys.isPluginMetaData(id)) {
      ...
      const plugindata = await loadsys.loadPluginMetaData(type, id, nodeid);
...
// if (!data) throw { err: 'SOFTERR', message: 'Not found v5 metadata for ' + nodeid };
/*
        data.p2 = [
          {
            prop: 'transport',
          ...
          },
          {
            ...
            title: 'Modbus IP',
            type: 'input',
            hide: '!data.p2.transport.id || (data.p2.transport.id == "rtu")'
          },
          {
            prop: 'port',
            title: 'Port',
