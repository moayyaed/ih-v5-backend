...
async function getRecordByForm(id, nodeid) {
  ...
  async function getData(table, item) {
    ...
    if (!spec.includes(item.type)) return val;
   console.log(item.type +' '+util.inspect(dataFromTable[table][0]));
    switch (item.type) {
      case 'droplist': return datautil.getDroplistItem(item.data, val);
      case 'smartbutton': return datautil.getSmartbuttonItem(dataFromTable[table][0]);
      default: return '';
    }
