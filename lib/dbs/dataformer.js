...
async function getRecordByForm(id, nodeid) {
  ...
  const codeStr = `script({
    start() {
        ...
        let error = sensor.error;
        let errstr = sensor.getParam("errstr");
        this.log('Статус '+sensor.state+ ' errstr='+errstr+' isError='+sensor.isError())
    }
    ...
    for (const cell of formMetaData.grid) {
      ...
      for (const item of formMetaData[cell.id]) {
        if (item.type == 'table') {
          ...
          // Вернуть код сценария
          data[cell.id][item.prop] = codeStr;
