...
/**
 *  Получение данных для первоначального заполнения по типу
 *    type='menu', id='pmmenu' | type='tree', id='devices'}
 *
 * @param {String} type - тип объекта
 * @param {String} id - идентификатор по типу объекта
 * @param {String} nodeid - идентификатор нового узла, с которого пришел запрос
 * @return {Object}: {data}
 */
async function getDefault(type, id, nodeid) {
  // Получить данные формы:  type=form'&id=formDeviceCommon&nodeid=xxxx
  return (type == 'form') ? getRecordByForm(id, nodeid, 'def') : {data:{}};
}
