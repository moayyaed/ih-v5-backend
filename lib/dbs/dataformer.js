...
async function getMeta(type, id) {
  ...
  if (id && id.indexOf('.')>0) {
    ...
    data = await loadsys.loadMeta(type, id);
  }
  cache.set(key, data);
...
async function getSubTree(id, nodeid) {
  ...
  if (!docs) return [];
  const b_array = docs.filter(item => item.folder).map(item => ({ id: item._id, title: item.chan, parent: 0, component:'channelfolder.'+nodeid}));
  const l_array = docs.filter(item => !item.folder).map(item => ({ id: item._id, title: item.chan, parent: 0, component:'channelview.'+nodeid }));
 ...
 */
function formTreeAndGuideFromResDocs(res, treeId) {
  ...
  for (const table in res) {
    ...
    if (table == desc.leaf.table) {
      const docs = res[table].docs.sort(hut.byorder('order'));
      l_array = formTreeItemsArray(docs, desc);
      if (l_array) {
