...
function formTreeItemsArray(dataArr, descItem) {
  const result = hut.mapProps(dataArr, descItem.propmap);

  if (descItem.table == 'device') {
    result.forEach(item => {
      item.title = item.dn+' '+item.title;
    })
  }
  return result;
}
...
async function getMetaUpForm(id) {
  ...
  // Сохранить в кэш
  cache.set(key, { records, tables, alloc, fieldtype });
