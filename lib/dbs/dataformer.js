...
async function loadMetaForId(type, id) {
  ...
  let fid = arr[1];
  console.log('loadMetaForId ' + type + ' id' + id + ' fid' + fid);
...
function formTreeItemsArray(dataArr, desc) {
  console.log('formTreeItemsArray desc=' + util.inspect(desc));
  dataArr.forEach(item => {
    item.title = getTreeItemTitle(desc.table, item);
  });
  // return hut.mapProps(dataArr,{ "_id": "id", "name": "title", "parent": "parent", "order": "order" });
...
async function getSubTree(id, nodeid) {
  ...
  const b_desc = descriptor.getDescItem('table', desc.branch.table);
  const docs = await dbstore.get(b_desc.collection, { unit: nodeid });
  console.log('getSubTree ' + b_desc.collection + ' ' + id + ' ' + nodeid + ' docs=' + util.inspect(docs));
  if (!docs) return [];
  const b_array = docs
    .filter(item => item.folder)
    .map(item => ({ id: item._id, title: item.chan, parent:item.parent || 0, order:item.order, component: 'channelfolder.' + nodeid }));
  const l_array = docs
    .filter(item => !item.folder)
    .map(item => ({ id: item._id, title: item.chan, parent:item.parent || 0, order:item.order, component: 'channelview.' + nodeid }));
...
function formTreeAndGuideFromResDocs(res, treeId, subtree) {
  ...
  console.log('formTreeAndGuideFromResDocs treeId='+treeId+' desc='+util.inspect(desc))
  const treeItems = [];
  ...
  console.log('formTreeAndGuideFromResDocs treeItems='+util.inspect(treeItems))
  console.log('formTreeAndGuideFromResDocs treeItems='+util.inspect(treeItems))
  return { tree: treeutil.makeTreeWithLeaves(b_array, l_array), treeItems };
}

function formSubTreeAndGuideFromResDocs(res, treeId) {
  console.log('formSubTreeAndGuideFromResDocs treeId=' + treeId + ' res=' + util.inspect(res));
  const desc = descriptor.getDescItem('tree', treeId);
  const treeItems = [];
...
function getTreeItemTitle(table, item, olditem) {
  console.log('getTreeItemTitle ' + table + ' item=' + util.inspect(item));
  let title = item.name || (olditem && olditem.name ? olditem.name : '');
  if (title && table == 'device') {
  ...
  console.log('getTreeItemTitle result ' + title);
  return title;
}
