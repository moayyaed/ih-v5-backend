...
async function getSubTree(id, nodeid) {
  ...
  if (!docs) return [];
  const b_array = docs.filter(item => item.folder).map(item => ({ id: item._id, title: item.chan, parent: 0 }));
  const l_array = docs.filter(item => !item.folder).map(item => ({ id: item._id, title: item.chan, parent: 0 }));
