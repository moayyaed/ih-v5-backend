...
async function getXFormData(id, nodeid) {
  ...
  for (const cell of formMetaData.grid) {
    ...
    for (const item of formMetaData[cell.id]) {
      ...
      } else data[cell.id][item.prop] = xform.getData(item);
    }
  }
  return data;
}
  ...
  if (nodeid && type == 'form') {
    ...
    if (id == 'formPluginCommon') {
      ...
      if (nodeid.startsWith('modbus')) {
        data.p2 = [
          { prop: 'transport', title: 'Transport', type: 'droplist',
          data: [
            {
              id: "tcp",
              title: "Modbus TCP"
          ...
          { prop: 'ip', title: 'Modbus IP', type: 'input', hide: "!data.p2.transport || (data.transport == \"rtu\")" },
          { prop: 'port', title: 'Port', type: 'input', hide: "!data.p2.transport || (data.transport == \"rtu\")" },
          { prop: 'serialport', title: 'Serial Port', type: 'input', hide: "!data.p2.transport || (data.transport != \"rtu\")" }
        ];
