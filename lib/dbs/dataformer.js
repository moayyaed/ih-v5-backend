...
function createTreeGuide(b_array, l_array, desc) {
  ...
  b_array.forEach(item => {
    tree_guide[item.id] = { id: item.id, title: item.title, parent: item.parent, table: desc.branch.table, component: defComp.branch };
  });
  l_array.forEach(item => {
    tree_guide[item.id] = { id: item.id, title: item.title, parent: item.parent, table: desc.leaf.table, component: defComp.leaf };
  });
  return tree_guide;
}
...
async function getBreadcrumbs(breadcrumbsObj, nodeId) {
  ...
  while (parent) {
    if (tree_guide[parent]) {
      breadArr.unshift({ id: parent, title: tree_guide[parent].title, component: defComp.parent });
...
function getTreeDefaultComponents(treeId) {
  return {};
}
