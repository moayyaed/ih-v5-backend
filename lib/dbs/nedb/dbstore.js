...
module.exports = {
  ...
  // createICollection(app, 'types') {
  createICollection(app, name) {
    const cName = 'i_'+app+'_'+name;
    const filename = appconfig.getIntegrationPath(app) + '/' + name + '.db';
    db[cName] = new Datastore({ filename, autoload: true, corruptAlertThreshold: 1 });
    return cName;
  },
  ...
  async removeAll(collection) {
    ...
    if (count) await this.remove(collection, {}, {multi:true});
  },
