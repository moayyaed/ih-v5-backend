...
function getPluginFormName(id) {
  const plspec = ['formPluginCommon', 'channelview', 'channellink', 'channelfolder', 'formPluginChannelsTable'];
...
async function getForm(query, dm) {
  ...
  if (id == 'channellink') return getChannellink(query, dm); // Вызывают channellink без указания плагина
...
async function getChannellink(query, dm) {
  const { nodeid } = query;
  ...
  // Если нет привязки - показываем channellink
