...
module.exports = {
  ...
  addList(listname, table, dataArr) {
    ...
    this.dm.on('updated:' + table, docs => {
      ...
      const name = sTables[table];
      this.dm.emit('updated:list:' + name, this.getListAsArray(name));
    });
  ...
  getItemFromList(listname, key) {
    return listMap.has(listname) && listMap.get(listname).has(key)
      ? listMap.get(listname).get(key)
      : { id: '-', title: '' };
  },
