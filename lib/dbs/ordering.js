 ...
 *    и возвращает новые order-s сдвинутых узлов для передачи интерфейсу
 * @param {Object}: {treeObj, treeDesc, parentid, order, qItems}
 *
 ...
 *         order {Number} - order узла, за которым будет вставка (0 - в конец)
 ...
 */
async function exec({ treeObj, treeDesc, parentid, order, qItems = 1 }) {
  console.log('ordering.exec START parentid=' + parentid + ' treeObj=' + treeObj);
  // Поддерево, куда вставляем - это parent. Если 0 - вставка на верхний уровень
  let targetObj;
  if (parentid) {
    targetObj = treeutil.findNodeById(treeObj, parentid);
    console.log('ordering.exec 1 targetObj ' + targetObj);
    if (!targetObj) throw { err: 'SOFTERR', message: 'Not found parent node ' + parentid };
  } else targetObj = treeObj;

  console.log('ordering.exec BEFORE calculateOrder');
  const { startOrder, delta, targetIdx, needReorder } = calculateOrder(targetObj.children, order, qItems);

  console.log('after calculateOrder startOrder=' + startOrder + ' delta=' + delta);
  console.log('targetIdx=' + targetIdx + ' needReorder=' + needReorder);
  let shifted;
  if (needReorder) {
    shifted = await reordering(targetObj.children, targetIdx, treeDesc, startOrder + qItems * delta);
    console.log('after reordering shifted=' + util.inspect(shifted));
  }
  return { startOrder, delta, shifted };
}
 ...
 * @param {Number} targetorder - значение order, ПОСЛЕ которого нужно вставить узел (узлы)
 ...
 *
 */
function calculateOrder(children, targetorder, qItems = 1) {
  let startOrder = 0;
  let delta = 1000;
  let targetIdx = -1;
  ...
  if (lastIdx >= 0) {
    // Массив не пустой - вставить в конец по дефолту, delta = 1000
    targetIdx = lastIdx;
    startOrder = children[targetIdx].order;

    // Если targetorder определен - ищем его в массиве
    if (targetorder) {
