...
async function exec({ payloadItem, treeObj, treeDesc, parentid, previd, seq }) {
  ...
  console.log('ordering.exec START seq=' +util.inspect(seq));
  // Поддерево, куда вставляем - это parent. Если 0 - вставка на верхний уровень
  let targetObj;
  if (parentid) {
    console.log('ordering.exec parentid = ' + parentid+ 'targetObj='+util.inspect(treeObj, null, 4));
...
async function updateNodeOrders(nodes, table) {
  ...
  try {
    for (const item of nodes) {
      if (item.nodeid) {
        await dbstore.update(tdesc.collection, { _id: item.nodeid }, { $set: { order: item.order } });
      }
    }
  } catch (e) {
