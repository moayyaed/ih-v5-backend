...
async function updateForm(body) {
  ...
  const formMetaDataForUpdate = await dataformer.getMetaUpForm(id);
  const fieldtype = formMetaDataForUpdate.fieldtype || {};
  ...
  formMetaDataForUpdate.records.forEach(item => {
    if (payload[item.cell]) {
      ...
      Object.keys(payload[item.cell]).forEach(field => {
        ...
        } else if (fieldtype[field] == 'code') {
          // Если код - сразу сохранить в файл, возможно, предварительно проверить!!
          // console.log('SAVE TO FILE'+payload[item.cell][field]);
          const filename = appconfig.getScriptFilename(nodeid);
          fs.writeFileSync(filename, payload[item.cell][field], 'utf8');
        }
  ...
  try {
    for (const table in docsToWrite) {
      ...
      if (doc.tags) await updateTags(doc, desc.collection);
      ...
      if (doc.name) {
        data.push({ id: body.nodeid, title: doc.name, table });
      }
  ...
  // НЕТ, нужно вернуть запись для дерева, но с указанием таблицы: {id, title, table}
...
async function updateTags(doc, collection) {
  ...
  tagstore.update(olddoc.tags, doc.tags, doc._id, collection);
}
