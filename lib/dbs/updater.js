...
async function insertTree(body) {
  ...
  console.log('start insertTree ' + util.inspect(body));
  ...
  for (const rootid in payload) {
    ...
    if (payload[rootid].folders) {
      ...
      payload[rootid].folders.forEach(item => {
        // Добавить новые папки
        const res = formOne(tdesc, item, 'folder');
      ...
      // На выход нужно title, order, id, children
      docs.forEach(item => {
        data.push({id:item._id, title:item.name, order:item.order, children:[]});
      })
    }

    if (payload[rootid].nodes) {
      const docs = [];
      const table = desc.leaf.table;
      const tdesc = descriptor.getDescItem('table', table);
      payload[rootid].nodes.forEach(item => {
        // Добавить новые записи item = {parentid, order}
        const res = formOne(tdesc, item);
        if (res) docs.push(res);
      });
      ...
      await dbstore.insert(tdesc.collection, docs);
      docs.forEach(item => {
        data.push({id:item._id, title:item.name, order:item.order});
      })
    }
  }
  return { data };

 function formOne(tdesc, item, folder) {
    // Добавить новые узлы item = {parentid, order}
    // Нужен новый id
    const id = shortid.generate();
    const title = folder ? 'New folder' : 'New node' ;
    const doc = { _id: id, parent: item.parentid, order: item.order, name:title};

    console.log('insertTree item ' + util.inspect(item));
 
    if (tdesc.filter) {
      hut.extend(doc, tdesc.filter);
    }
