...
/**
 * Сформировать объект для редактирования формы и сохранить в кэш
  ...
  */
  ...
  for (const table in docsToWrite) {
    ...
    if (doc.name && doc.name.includes('test')) {
      ...
      /*
      const cellid = formMetaDataForUpdate.alloc[table]['name'];
      if (!errdata[cellid]) errdata[cellid] = {};
      errdata[cellid]['name'] = 'Что за чушь!';
      */
      ...
      if (typeof data[mainprop] != 'object') {
        ...
        for (const prop in data[mainprop]) {
          ...
          if (val < 0) {
            ...
            throw {
              error: 'Validation',
              data: { p3: { [table]: { [mainprop]: { [prop]: 'Отрицательное значение :' + val } } } }
            };
            */
          }
        }
