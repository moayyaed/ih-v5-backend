...
async function formTableData(dataFromTable, table, nodeid, pitem) {
  ...
  const tdata = arr.map((item, idx) => {
    ...
    columns.forEach(col => {
      if (col.type == 'link') {
        ...
        row[col.prop] = formLinkObj(col, item[col.prop])
  ...
  function formLinkObj(colItem, val) {
    console.log('formLinkObj')
    if (val == undefined) return {title:'', path:null};
...
async function unitchannels(dataFromTable, table, nodeid, pitem) {
  ...
  console.log('unitchannels arr ='+util.inspect(arr))
