...
module.exports = {
    ...
    try {
      ...
      Object.keys(cdesc).forEach(name => {
        ...
        if (cdesc[name].ensureIndex) {
          ...
          db[name].ensureIndex([[fieldName, 'ascending' ]], err => {
            if (err) {
              const errStr = 'bigstore collection ' + name + ', ensureIndex ' + fieldName + ': ';
              console.log('ERROR: ' + errStr + hut.getShortErrStr(err));
            }
          });
  ...
  getLimited(collection, filter = {}, opt = {}) {
    if (!db[collection]) return Promise.reject({ error: 'SOFTERR', message: 'No collection ' + collection });
      ...
      let { sort, limit } = opt;
      // coll.find({ num: { $gt: 25 } }).sort({ num2: -1 }).limit(3000).toArray(
      if (!sort) sort = {ts: 1};
      if (!limit) limit = 100;
