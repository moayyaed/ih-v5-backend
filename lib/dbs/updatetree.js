...
async function copyTo(body) {
  ...
  for (const rootid in payload) {
    ...
    const qNodes =
      (payload[rootid].nodes ? payload[rootid].nodes.length : 0) +
      (payload[rootid].folders ? payload[rootid].folders.length : 0);
  /*
    if (qNodes > 1) {
      // Решить с order
      const targetObj = treeutil.findNodeById(treeObj, targetid); // Получить поддерево узла, В который копируем
      if (!targetOb) throw {err:'ERR', message:'Not found target node:'+targetid};
      if (!targetOb.children) throw {err:'ERR', message:'Not allowed leaf as target node: '+targetid};

      // const targetIdx = targetOb.children.length ? targetOb.children.findIndex(item => item.order > order) : 0;
    }  
  */

    // простые листья
    if (payload[rootid].nodes) {
      const docs = await copyNodes(payload[rootid].nodes, desc.leaf.table, targetid, targetorder);
  ...
  async function copyNodes(nodes, table, target, torder) {
    const tdesc = descriptor.getDescItem('table', table);
