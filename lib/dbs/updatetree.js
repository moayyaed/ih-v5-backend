...
async function copypaste(body) {
  ...
  for (const rootid in payload) {
    ...
    const { startOrder, delta, targetIdx, needReorder } = calculateOrder(targetObj.children, targetorder, seq);

    // Если не смогли поместиться - нужно перенумеровать элементы следующие за targetIdx
    if (needReorder) {
      reorder = await reordering(targetObj.children, targetIdx, desc, startOrder + seq.length * delta);
    }
