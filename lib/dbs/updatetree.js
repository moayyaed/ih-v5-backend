...
const devicemanger = require('../devices/devicemanager');
...
async function copypaste(body) {
  ...
  async function copyNodes(nodes, table, target) {
    ...
    console.log('copyNodes get col ' + tdesc.collection + util.inspect(filter));
    const docs = await dbstore.get(tdesc.collection, filter);
    console.log('copyNodes docs src ' + util.inspect(docs));
    ...
    console.log('copyNodes docs res ' + util.inspect(docs));
    return docs;
  }
  ...
  async function processNodes(nodes, table, parentCollection, propremap, folder) {
    if (nodes) {
      ...
      for (const item of nodes) {
        let res;
        if (defRec) {
          // res = createOneTreeRecord(table, tdesc, item, defRec);
          const id = numerator.getNewId(table);
          res = Object.assign({ _id: id, parent: item.parentid, order: item.order }, tdesc.filter, defRec);
        } else {
          res = await createOneRecord(table, tdesc, item);
        }
...
function createOneRecord(table, tdesc, item) {
  console.log('createOneRecord '+table)
  const id = numerator.getNewId(table);
  const doc = Object.assign({ _id: id, parent: item.parentid, order: item.order }, tdesc.filter);
  switch (table) {
    case 'device': 
        // Сформировать запись для нового устройства на основе типа
        // 1. props нужо сформировать по типу
        doc.props = devicemanger.createPropsFromType(item.popupid);

        // 2. dn и name по правилу из типа 
        doc.dn = devicemanger.createDnFromType(item.popupid);
        doc.name = devicemanger.getNameFromType(item.popupid);
        break;
