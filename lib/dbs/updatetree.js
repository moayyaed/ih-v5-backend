 ...
 *   - reorder: объект с измененным order в редактируемой ветке - для response 
 */
async function copyTo(body) {
  ...
  const res = []; // Массив изменений передать
  const seqObj = {};
  const reorder = {};
  ...
  for (const rootid in payload) {
    ...
    const subtree = treeutil.findNodeById(treeObj, targetid);
    formOrders(subtree, payload[rootid].seq, desc);
  ...
  if (hut.isObjIdle(reorder)) res.reorder = reorder;
  return result;
  ...
  async function formOrders(targetObj, seq, desc) {
    ...
    if (needReorderIdx) {
      const l_arr = [];
      const b_arr = [];
      for (let i = targetIdx + 1, j = 1; i < targetObj.children.length; i++, j++) {
        const item = targetObj.children[targetIdx + 1];
        const id = item.id;
        nextOrder += 1000 * j;
