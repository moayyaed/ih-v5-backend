...
async function update(body) {
  ...
  for (const rootid in payload) {
    ...
    if (!theNode) continue;
    console.log('theNode '+util.inspect(theNode))
    ...
    console.log('after update folders ');
    console.log('before update nodes '+util.inspect(payload[rootid].nodes));
    ...
    for (const item of nodes) {
      ...
      if (item.nodeid && doc) {
        ...
        if (updatecheck.isNodeLostFolder(item.nodeid))
          ...
          console.log('before  '+dbstore.update);
        await dbstore.update(tdesc.collection, { _id: item.nodeid }, { $set: doc });
        console.log('after  '+dbstore.update);
