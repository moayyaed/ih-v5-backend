...
async function update(body) {
  ...
  for (const rootid in payload) {
    ...
    if (payload[rootid].folders) await updateNodes(payload[rootid].folders, desc.branch.table,parentCollection);
    if (payload[rootid].nodes) await updateNodes(payload[rootid].nodess, desc.leaf.table, parentCollection);
  }
  return res;

  async function updateNodes(nodes, table, parentCollection) {
