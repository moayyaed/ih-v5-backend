...
async function copypaste(body) {
  ...
  for (const rootid in payload) {
    ...
    const { startOrder, delta, targetIdx, needReorder } = calculateOrder(targetObj, targetorder, seq);

    // Если не смогли поместиться - нужно перенумеровать элементы следующие за targetIdx
    if (needReorder) {
      reorder = await reordering(targetObj, targetIdx, desc, startOrder + seq.length * delta);
    }
    ...
    console.log('BEFORE copy leaves');
  ...
  async function copyNodes(nodes, table, target) {
    const tdesc = descriptor.getDescItem('table', table);
    console.log('copyNodes ' + table + ' tdesc=' + util.inspect(tdesc));
...
function calculateOrder(targetObj, target, seq) {
  // console.log('calculateOrder START targetorder='+targetorder);
  ...
  const lastIdx = targetObj.children.length - 1;

  if (typeof target == 'object') {
    // Передают готовый объект, order уже посчитан (при вставке или сдвиге)
    // Ищем пару, между которой нужно вставить
    // Если не умещаемся - меняем, иначе оставить тот же
    targetorder = target.order ? target.order : 0;
    targetIdx = targetObj.children.findIndex(item => item.order >= targetorder);

    if (targetIdx > 0) {
      targetIdx--;
      startOrder = targetObj.children[targetIdx].order; // start - предыдущий
      delta = getDelta(targetObj.children[targetIdx].order, targetObj.children[targetIdx+1].order, 1);

    } else {
      targetIdx = lastIdx;
      startOrder = targetIdx >= 0 ? targetObj.children[targetIdx].order : 1000;
    }
  } else {
    // при копировании нужно считать
    ...
    if (lastIdx >= 0) {
      ...
      if (targetorder) {
        targetIdx = targetObj.children.findIndex(item => item.order >= targetorder);
        if (targetIdx >= 0 && targetIdx < targetObj.children.length - 1) {
          startOrder = targetObj.children[targetIdx].order;
          delta = getDelta(startOrder, targetObj.children[targetIdx + 1].order, qItems);
        } else {
  ...
  console.log('calculateOrder END');
 ...
 * @param {*} nextOrder
 */
async function reordering(targetObj, targetIdx, desc, nextOrder) {
  console.log('reordering START');
  if (!targetObj) throw { err: 'SOFTERR', message: 'Expected targetObj for reordering!' };

  if (targetIdx < 0 || !targetObj.children.length || targetIdx > targetObj.children.length - 1) return;
  ...
  console.log('reordering REORDER=' + util.inspect(reorder));
  return reorder;
}
...
async function insert(body) {
  ...
  for (const rootid in payload) {
    ...
    const theNode = getTheNode(rootid);
    if (!theNode) continue;
    ...
    // В массиве children найти элемент с order >= targetorder
    const { startOrder, delta, targetIdx, needReorder } = calculateOrder(targetObj, theNode);

    // Если не смогли поместиться - нужно перенумеровать элементы следующие за targetIdx
    if (needReorder) {
      reorder = await reordering(targetObj, targetIdx, desc, startOrder + delta);
    }

    // Проверить order для узла, если не совпадает - изменить перед записью
  ...
  const result = { data };
  console.log('insert REORDER=' + util.inspect(reorder));
  if (reorder && !hut.isObjIdle(reorder)) result.reorder = reorder;
  return result;

  function getTheNode(rootid) {
