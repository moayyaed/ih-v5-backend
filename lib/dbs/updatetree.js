...
async function copyTo(body) {
  ...
  for (const rootid in payload) {
    ...
    // Вычислить число узлов, которые будут вставлены и рассчитать новые order для узлов
    seq = payload[rootid].seq;
    if (!seq || !Array.isArray(seq)) throw {err:'ERR', message:'Expected seq for copypaste method!'};
  
   
    // Решить с order
    const targetObj = treeutil.findNodeById(treeObj, targetid); // Получить поддерево узла, В который копируем
    ...
    let targetIdx;
    if (targetorder) {
      targetIdx = targetObj.children.length ? targetObj.children.findIndex(item => item.order >= targetorder) : 0;
    } else {
      // Просто в конец
      targetIdx = targetObj.children.length-1;
    }

    // Если -1 => добавить в начало?
    // Берем найденный order и следующий и считаем интервал, в который можем вставить
    /** 
    let interval;
    if (targetIdx< targetObj.children.length-1) { 
      interval = targetObj.children[targetIdx+1].order - targetObj.children[targetIdx].order;
    } else {// (targetIdx = targetObj.children.length-1) - последний элемент - нумеруем просто подряд
      
    }
    */
   // Просто в конец

   startOrder = targetObj.children[targetObj.children.length-1].order || 0;
   delta = 1000;
  ...
  async function copyNodes(nodes, table, target) {
    ...
    const tdesc = descriptor.getDescItem('table', table);
    console.log('tdesc='+util.inspect(tdesc));
  ...
  function getOrder(docId) {
    const idx = seq.indexOf(docId);
    const res =  startOrder+delta*(idx+1);
    console.log('getOrder for '+docId+' = '+res)
    return res;
  }
