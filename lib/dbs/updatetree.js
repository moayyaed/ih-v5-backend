...
async function copyTo(body) {
  ...
  for (const rootid in payload) {
    ...
    // Решить с order
    // Получить поддерево узла, В который копируем
    const targetObj = treeutil.findNodeById(treeObj, targetid);
    ...
    delta = interval / (seq.length + 1);
    if (delta < 2) delta = 1000; // Просто добавляем каждому элементу по 1000
  ...
  function getOrder(docId) {
    ...
    console.log('getOrder find in ' + util.inspect(seq));
    console.log('getOrder for ' + docId + ' = ' + res + ' startOrder=' + startOrder);
    return res;
  }
  */
