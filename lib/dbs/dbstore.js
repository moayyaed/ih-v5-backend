...
function findOne(collection, filter, opt={}) {
  if (!db[collection]) return Promise.reject({ error: 'SOFTERR', message: 'No collection ' + collection });
  if (!filter || hut.isObjIdle(filter)) return Promise.reject({ error: 'SOFTERR', message: 'Expected filter for findOne, collection ' + collection });
...
function remove(collection, filter, options) {
  console.log('remove ' + collection+' filter='+util.inspect(filter));
  if (!db[collection]) return Promise.reject({ error: 'SOFTERR', message: 'No collection ' + collection });
...
function getData({ collection, filter, order, fields }) {
  const opt = { fields, order };
  return get(collection, filter, opt);
}
