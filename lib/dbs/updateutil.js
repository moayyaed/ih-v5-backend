...
async function getShifted(treeObj, theNode, desc) {
  ...
  // const treeObj = await dataformer.getCachedTree(treeId);
  if (!theNode) throw { err: 'SOFTERR', message: 'Undefined node in getShifted '};
  ...
  return processTheNodeOrder(targetObj.children, theNode, desc);
}
