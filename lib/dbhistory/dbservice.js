...
module.exports = async function(holder) {
  ...
  try {
    ...
    if (!dbname) throw { message: 'Missing "dbname". Database will not be used!' };
     ...
     // Получить опции для агента и передать 
    dbagentDoc = await dm.findRecordById('dbagent', dbname);
    if (!dbagentDoc) {
      // Создать новую запись с данными по умолчанию
      dbagentDoc = {_id:dbname, title:dbname, parent:'dbagentgroup', database:''};
      log("Create default options for "+dbname, 2);
      await dm.insertDocs('dbagent', [dbagentDoc])
    }
