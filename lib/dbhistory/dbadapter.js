...
const AkumuliAgent = require('./akumuli/dbagent');

module.exports = async function(holder) {
  const responseobj = {};
  
  const dbagent = new AkumuliAgent();
  dbagent.start();
  ...
  */
  ...
  function getDbagentModulePath() {
    return appconfig.getDbagentModulePath() || `${appconfig.get('appdir')}/lib/dbhistory/nedb/dbagent.js`;
  }

  function getDbagentOptions() {
    const res = appconfig.getDbagentOptions();
    if (res) return res;
    
    // Для nedb
    return {databases:[{name:'tsdb', folder:appconfig.get('hdbpath')}]}
  }
