  ...
  switch (cmd) {
    ...
    case 'templates':
      if (query.containerid) {
        // dataObj = await dataformer.getCachedProjectObj(cmd, query.id);
        dataObj = await joinTemplatesFromContainer(query.containerid);
      }
...
async function joinTemplatesFromContainer(containerId) {
  // console.log('joinTemplatesFromContainer '+containerId);

  const resObj = {};
  const dataObj = await dataformer.getCachedProjectObj('container', containerId);
  // console.log('joinTemplatesFromContainer dataObj.elements='+util.inspect(dataObj.elements));
  const ids = [];
...
function execDeviceAction(query, holder) {
  ...
  if (query.command) {
    if (!dobj.hasCommand(query.command)) throw { err: 'ERR', message: 'No command "'+ query.command+'" for device: '+dobj.dn+'  ('+dobj.name+ ')'};
    dobj[query.command]();
    return;
  }

  if (!dobj.hasProp(query.prop)) throw { err: 'ERR', message: 'No prop "'+ query.prop+'" for device: '+dobj.dn+'(id='+dobj._id+ ')'};
