...
async function exec(query, holder) {
  ...
  try {
    switch (query.command) {
      ...
      case 'checkupdate':
        return updateutil.checkUpdateSystem();
      ...
      case 'update':
        res = await updateutil.updateSystem();
        if (res.ok) {
          // Обновить системные плагины. В любом случае перезагрузить систему
          await updateutil.updateSystemPlugins();
          deferredExit(holder);
        }
        return res;
