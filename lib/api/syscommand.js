...
async function exec(query, holder) {
  ...
  try {
    switch (query.command) {
      ...
      case 'activate':
        if (!query || !query.payload || !query.payload.p3) {
          throw { message: 'Expected payload with p3 part!' };
        }
        res = await activateLicense(query.payload.p3.newlicense);
        return res;
  ...
  async function activateLicense(newlicense) {
    if (!newlicense) {
      throw { message: 'Ключ не введен!' };
    }

    const result = await portal.activation_license({key:newlicense}, holder);

    if (result.res) {   //  { res: 1, ...resObj }
      console.log('INFO: '+util.inspect(result))
      return { alert: 'info', message: 'Лицензия успешно активирована', ok: 1, refresh: true };
      // return { alert: 'info', message: 'Лицензия успешно активирована', ok: 1, refresh: true };
    } 
    return { alert: 'info', message: result.message || '???', ok: 0 };

    // 
   /*
    console.log('activateLicense START');
    await holder.dm.insertDocs('licenses', [{ _id: ts, id: ts, ts, title: 'Demo IntraSCADA v5: 100 тэгов на 3 дня' }]);

    const docs = await holder.dm.dbstore.get('licenses', {});
    console.log('after insertDocs docs=' + util.inspect(docs));
