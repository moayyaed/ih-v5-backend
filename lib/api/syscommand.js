...
async function exec(query, holder) {
  ...
  async function folderFromPayload(_id, payload) {

    if (!payload || !payload.p1._id || !payload.p1.projectfolder) {
      return getProjectFolderById(_id);
    }

    let folder = payload.p1.projectfolder;
    if (payload.p1._id != payload.p1.projectfolder) {
      // нужно переименовать папку проекта. Если нет папки - будет новый проект
      const from = appconfig.getTheProjectPath(payload.p1._id);
      const to = appconfig.getTheProjectPath(folder);
      if (fs.existsSync(from)) {
        console.log('INFO: RENAME Project '+from+' to '+to)
        projectutil.renameProjectFolder(from, to);
      }
    }
    return folder;
  } 
