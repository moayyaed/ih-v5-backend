...
async function exec(query, holder) {
  try {
    switch (query.command) {
      ...
      case 'restartwithdbagent':
        // Перезагрузка с заменой dbagent-a
        if (!query.nodeid) throw { message: 'Не определен агент БД!' };
        await dbagentutil.replaceActiveDbagent(query.nodeid, holder);
        deferredExit(holder); 
        return;

      case 'checkupdate':
        throw { message: 'Не найден сервер обновлений!' };
