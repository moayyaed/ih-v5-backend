...
async function exec(query, holder) {
  ...
  try {
    switch (query.command) {
      ...
      case 'rollback':
        res = await updateSystem('rollback');
        if (res.ok) {
          deferredExit(holder);
        }
        return res;
      ...
      case 'writetochannel':
        return writeTestValueToChannel();
  ...
  async function updatePlugin() {
    ...
    try {
      ...
      // Установить
      const newDoc = await plugininstall.installPlugin(packFolder, watch, holder);
  ...
  /**
   * Записать тестовое значение напрямую в канал
   *   query.nodeid = <unit>
   *   query.subnideid = <id в devhard>
   *   query.payload - объект, содержащий данные с формы. 
   *      Ищем только testValue - Значение, которое надо записать
   */
  async function writeTestValueToChannel() {
    ...
    if (!query.payload) throw { message: 'Expected payload for command:"writetochannel" ' };
    if (typeof query.payload != 'object') throw {message: 'Expected payload as object'}
 
    const unitId = query.nodeid;
    const pluginStr = appconfig.getMessage('Plugin') + ' ' + unitId + ' ';
    if (!holder.unitSet[unitId]) throw { message: pluginStr + appconfig.getMessage('notFound') };
    if (!holder.unitSet[unitId].ps) throw { message: pluginStr + appconfig.getMessage('notRunning') };

    let testValue = getTestValue(query.payload);
    if (testValue == undefined) throw { message: appconfig.getMessage('Enter_value_to_write') };
    // Передать число, если это число
    if (!isNaN(testValue)) testValue = Number(testValue);

    holder.emit('send:device:command', {unit: query.nodeid, _id: query.subnodeid, testValue});
    await sleep(1000);
    return { alert: 'info', message: 'Команда отправлена плагину', refresh: true };

    function getTestValue(pObj) {
      let xValue;
      Object.keys(pObj).forEach(pKey => {
        if (pObj[pKey]) {
          Object.keys(pObj[pKey]).forEach(prop => {
            if (prop == 'testValue') {
              xValue = pObj[pKey][prop];
            }
          });
        }
      });
  ...
  async function fromWriteObj() {
    // Значение, которое надо записать пришло на форме - payload
    let value;
    if (query.payload) {
      value = getTestValue(query.payload);
    }
    if (value == undefined) throw { message: appconfig.getMessage('Enter_value_to_write') };
    // Передать число, если это число
    ...
    return { unit: query.nodeid, chan: doc.chan, value, testValue: value };
