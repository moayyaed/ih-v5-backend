...
async function exec(query, holder) {
  ...
  try {
    switch (query.command) {
      ...
      case 'deactivate':
          if (!query || !query.payload || !query.payload.key) {
            throw { message: 'Expected payload with key!' };
          }
          res = await deactivateLicense(query.payload.key);
          return res;
  ...
  async function deactivateLicense(key) {
    ...
    const result = await portal.deactivate_license({key}, holder);
