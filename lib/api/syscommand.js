...
async function exec(query, holder) {
  ...
  function sendInfo() {
    try {
      ...
      // Проверить, что плагин информирования запущен
      if (!holder.unitSet[infotype]) throw { message: infotype+'. Плагин не найден!' };
      if (!holder.unitSet[infotype].ps) throw { message: infotype+'. Плагин не запущен!' };
     
      const addr = query.payload.addr;
      const sign = query.payload.sign;
      ...
      execSendInfo(infotype, { txt }, { addr, sign });

      if (infotype == 'pushnotification') {
        holder.dm.insertDocs('pushnotifications', [{time: Date.now(), txt, dest: userId}]);
      }

      return { alert: 'info', message: 'Отправка ' + infotype + ' для ' + addr };
  ...
  function execSendInfo(type, sobj, dest) {
    ...
    let txt = sobj.txt;
    const recs = [dest]; // {addr, sign}
    const sendTo = recs.map(item => ({ addr: item.addr, sign: item.sign || '' }));
    let resobj = { txt, sendTo };
    if (sobj.img) resobj.img = sobj.img;
    console.log('execSendInfo type:'+type)
    holder.emit('sendinfo', type, resobj);
   

    /*
    let logtxt = type;
    const needAdr = id || dest ? 1 : 0;
    let recs;
    ...
    if (needAdr) {
      recs = [];
      if (id) {
        let rec = jdb.find('infoaddr', id);
        if (rec) recs.push(rec);
      } else {
        // Формируем фильтр из dest
        let filter = getFilter();
        if (filter) {
          recs = jdb.get({ name: 'infoaddr', filter });
        }
      }
  
      if (recs && recs.length > 0) {
        let sendTo = recs.map(item => ({ addr: item.addr, sign: item.sign || '' }));
        let resobj = { type, txt, sendTo };
        if (sobj.img) resobj.img = sobj.img;
        houser.emit('sendinfo', resobj);
  
        if (type == 'pushnotification') {
          logtxt += ' send to ' + dest + ': ' + recs.length + ' receiver(s)';
        } else {
          logtxt += ' send: ' + sendTo.map(item => item.addr).join(',');
        }
      } else {
        logtxt += ' error: Not found records for ' + dest;
      }
    } else {
      */

    // if (type == 'pushnotification') {
    //  const time = hut.getDateTimeFor(new Date(), 'shortdt');
    //  houser.emit('writelog', 'pushnotifications', { dest, txt, time });

    // houser.emit('writelog', 'userlog', { txt: logtxt });
  }
  ...
  function startStopScene(start) {
    try {
      ...
      if (!holder.sceneSet[id]) throw { message: 'Сценарий не найден!' };
      if (holder.sceneSet[id].blk) throw { message: 'Сценарий заблокирован!' };
