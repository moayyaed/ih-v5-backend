...
const portalcheck = require('../appspec/portalsessioncheck');
...
async function getMeta(query, holder) {
  ...
  return specreq.needFinishing(query) ? specreq.finishing(query, data) : data;
}
...
async function getData(query, holder) {
  ...
  switch (type) {
    ...
    case 'xform':
      if (query.id == 'formDashboardUpgrade') return getformDashboardUpgrade(query, holder);
      return getXFormData(id);

    // type=droplist'&id=typeList
    case 'droplist':
      // return datautil.getDroplist(id);
      return id.startsWith('__') ? formDroplist(id, nodeid) : datautil.getDroplist(id);
...
async function formDroplist(id, nodeid) {
...
async function getXFormData(id, indata) {
  ...
  for (const cell of formMetaData.grid) {
    ...
    for (const item of formMetaData[cell.id]) {
      const getRes = indata && item.prop && indata[item.prop] ? indata[item.prop] : await xform.getData(item);
...
async function getformDashboardUpgrade(query, holder) {
  ...
  if (typeof registry == 'object' && typeof registry.payload == 'object') {
    ...
    const resObj = await portalcheck({hwid: registry.hwid, ...registry.payload}, 'sessioncheck');
  ...
  return getXFormData(query.id, {registryState, portalauth, login});
}
