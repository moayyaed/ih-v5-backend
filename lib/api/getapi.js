...
async function getData(query, holder) {
  ...
  switch (type) {
    ...
    // type=tree'&id=dev
    case 'tree':
      if (id == 'scanx') return getScanResult(holder);   // ДЛЯ ОТЛАДКИ
...
function getScanResult(holder) {
  const unitId = 'mqttclient1';
  return {data:[holder.unitSet[unitId].scanResult] || []};
}
