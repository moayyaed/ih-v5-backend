...
async function getPopupPlugin(id, nodeid) {
  switch (id) {
  case 'plugincommand': return getPopupPluginCommands(nodeid);
  // case 'plugininstance': return {data: [{id:nodeid, title: 'New '+nodeid+' instance'}]};
  case 'plugininstance': return {data: []}; // Поверить, что это узел (папка плагина), тогда можно добавить
  default: return {data: []};
  }
}
