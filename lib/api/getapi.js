...
async function getMeta(query, holder) {
  ...
  return dm.datagetter.needFinishing(query) ? dm.datagetter.finishing(query, data) : data;
}
...
async function getData(query, holder) {
  ...
  switch (type) {
    ...
    case 'popup':
      ...
      return dm.datagetter.isSpec('popup', id) ? dm.datagetter.getSpecData(query, holder) : getPopup(id);
