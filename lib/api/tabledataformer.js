  ...
  const tdata = dataArr.map((item, idx) => {
    ...
    columns.forEach(col => {
      if (col.prop.startsWith('__')) { 
        ...
        console.log('WARN: formColumn smartbutton2')
        row[col.prop] = datagetter.formSmartbutton2Obj(col, item, item[col.prop]);
...
function derivativeValue(col, item) {
  switch (col.prop) {
    case '__idprefix': return item._id && item._id.indexOf('@') ? item._id.split('@') : '';
    default: return '';
  }
}
...
async function formRows(table, arr, columns) {
  const dropLists = await formDroplists(columns);
