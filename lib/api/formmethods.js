...
async function update(body) {
  ...
  for (const field of Object.keys(exfieldtype)) {
    ...
    // console.log('forEach field=' + field + ' item=' + util.inspect(item) + ' payload=' + payload[item.cell][field]);
    ...
    if (payload[item.cell]) {
      ...
      Object.keys(payload[item.cell]).forEach(field => {
        if (spec.includes(field)) {
          const sf = specFields[field];
          const val = payload[item.cell][field];
          if (typeof val == 'object') {
            // Пришли пары ключ-значение, записать как отдельные поля! Но нужно выбрать, не все подряд!!
            for (const prop in val) {
              if (!sf || sf[prop]) recsToWrite[item.table].doc[prop] = val[prop];
            }
          }
    ...
    for (const table in recsToWrite) {
      ...
      if (desc.rows) {
        ...
        if (recsToWrite[table].table) {
          res[table] = { docs: [] };
          for (const rowid in recsToWrite[table].table) {
            if (recsToWrite[table].table[rowid] == null) {
              // Удаление строки
              if (!remove[table]) remove[table] = { docs: [] };
              remove[table].docs.push({ _id: rowid });
            } else if (isNewRow(rowid)) {
              // Добавление строки
              ...
              insert[table].docs.push(
                await datamaker.createOneRecord(table, {}, recsToWrite[table].table[rowid], recsToWrite[table]._id)
              );
            } else {
              const olddoc = await dm.dbstore.findOne(collection, { _id: rowid });
              if (!olddoc) throw new Error('Not found doc: _id = ' + rowid + ', collection ' + collection);
              const resDoc = olddoc;
              resDoc.$set = recsToWrite[table].table[rowid];
        ...
        if (!olddoc) {
          if (collection == 'devicedb') {
            // Записи пока нет - добавить
          } else {
            throw new Error('Not found doc: _id = ' + _id + ', collection ' + collection);
          }
        }
        */
        const olddoc = await datamaker.findOrAddDoc(collection, _id);
        const resDoc = olddoc;
        ...
        if (recsToWrite[table].table) {
          ...
          resDoc.$renamed = processNewRows(table, tabdata, resDoc);
...
function processNewRows(table, data, olddoc) {
  const tempRec = [];
  const renamed = {};
