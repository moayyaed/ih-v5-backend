...
async function update(body, holder) {
  ...
  const spec = ['devlink', 'chanlink', 'dn_prop']; // Значение разбирается и пишется в несколько полей
  const specFields = {
    devlink: ['did', 'prop'],
    chanlink: ['did', 'prop']
  };
  ...
  for (const tableId in metaUpForm.tables) {
    ...
    if (payload[item.cell] && payload[item.cell][item.prop] && typeof payload[item.cell][item.prop] == 'object') {
      ...
      for (const rowId in payloadItem) {
        const rowObj = payloadItem[rowId];
        for (const columnField in rowObj) {
          if (spec.includes(columnField) && typeof rowObj[columnField] == 'object') {
            // chanlink:{id, prop, title, ...}
            const specVal = await datamaker.processTableSpecField(nodeid, rowId, columnField, rowObj[columnField]);
            ...
            processInputval(item.table, rowId, rowObj[columnField]);
            delete rowObj[columnField];
          }
        }
      }
  ...
  async function processExfieldtype(exfieldtype) {
    ...
    for (const field of Object.keys(exfieldtype)) {
      ...
      if (payload[item.cell] && payload[item.cell][field]) {
        // await datamaker.saveExField(item, field, nodeid, payload[item.cell][field]);
        const chItem = await datamaker.saveExField(item, field, preNodeid, payload[item.cell][field]);
  ...
  async function saveOther(table) {
    console.log('SAVE OTHER ' + table + ' nodeid=' + nodeid);
    if (table == 'systemsettingsTable') {
      await datamaker.saveConfig(recsToWrite[table].doc);
      ...
      for (const rid in recsToWrite[table].table) {
        if (recsToWrite[table].table[rid] == null) {
          ...
          insert[restable].docs.push(
            await datamaker.createOneRecord(restable, {}, recsToWrite[table].table[rid], recsToWrite[table]._id)
          );
        } else {
          ...
          if (!olddoc) {
            ...
            // Генерировать новый id в подчиненной таблице, сформировать строку (включить туда id главной таблицы)
            insert[restable].docs.push(
              await datamaker.createOneRecord(
                restable,
                { id: rid, nodeid },
                recsToWrite[table].table[rid],
                recsToWrite[table]._id
              )
            );
          } else {
            const resDoc = olddoc;
  ...
  async function formNewRecord(table, desc) {
    if (recsToWrite[table].doc) {
      ...
      insert[table].docs.push(
        await datamaker.createOneRecord(table, {}, recsToWrite[table].doc, recsToWrite[table]._id, body)
      );
    }
  }
}
