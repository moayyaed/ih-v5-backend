...
const treemethods = require('../apptools/treemethods');
const formmethods = require('../apptools/formmethods');
const updatetg = require('../apptools/updatetreeguide');
...
async function treeAfterInsert({ res, treeId, subtree, navnodeid }, holder) {
  ...
  let tree;
  for (const table in res) {
    const result = treeformer.formTreeAndGuideFromResDocs(res, treeId, subtree, navnodeid, dm);
...
async function formUpdate(body, holder) {
  ...
  if (res) {
    const form = await dm.getCachedData({ type: 'form', id: body.id, nodeid: body.nodeid, method: 'getmeta' });
    ...
    for (const table in res) {
      ...
      // Если на форме несколько таблиц - нужно определить основную
      if (formData && (formData.updateTreeFromTable == undefined || formData.updateTreeFromTable == table)) {
        if (res[table].docs && res[table].docs.length) {
          treeItem = treeformer.getUpdatedTreeItem(table, res[table].docs[0], dm);
  ...
  if (res) {
    for (const table in res) {
      if (res[table].docs && res[table].docs.length) {
        ...
        const treeItems = res[table].docs.map(doc => treeformer.getUpdatedTreeItem(table, doc, dm));
...
async function browseInsert(body, holder) {
  if (!body.payload) throw { message: 'Missing body.payload!' };
  if (!body.unit) throw { message: 'Missing body.unit!' };
  return devhardutil.addDevhardDocs(body.payload, body.unit, body.nodeid, holder.dm);
}
