...
async function treeInsert(body, holder) {
  ...
  const result = {};
  try {
    const { res, reorder } = body.method == 'copypaste' ? await treemethods.copy(body,dm) : await treemethods.insert(body,dm);
...
async function subtreeDialog(body, holder) {
  ...
  throw {  message: 'BODY: '+util.inspect(body) };
}
