...
const treemethods = require('../apptools/treeUpdate');
const formmethods = require('../apptools/formdataUpdate');
...
async function treeAfterInsert({ res, treeId, subtree, navnodeid }, holder) {
  ...
  let tree;
  for (const table in res) {
    const result = treeGet.formTreeAndGuideFromResDocs(res, treeId, subtree, navnodeid, dm);
    ...
    for (const table in res) {
      ...
      if (formData && (formData.updateTreeFromTable == undefined || formData.updateTreeFromTable == table)) {
        if (res[table].docs && res[table].docs.length) {
          treeItem = treeGet.getUpdatedTreeItem(table, res[table].docs[0], dm);
  ...
  if (res) {
    for (const table in res) {
      if (res[table].docs && res[table].docs.length) {
        ...
        const treeItems = res[table].docs.map(doc => treeGet.getUpdatedTreeItem(table, doc, dm));
