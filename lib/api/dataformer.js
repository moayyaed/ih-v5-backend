...
async function getRecordByForm(query, holder) {
      ...
      if (nodeid && cell.table && !dataFromTable[cell.table]) {
        ...
        if (desc.store == 'db') {
          ...
          if (virttables[cell.table]) {
            dataFromTable[cell.table] = await virttables[cell.table]([], cell.table, nodeid, '', holder, id);
          }
        }
  ...
  async function getHandler(preId) {
    if ((preId && preId.startsWith('api')) || preId.startsWith('restapi')) return loadsys.loadRestapihandler(preId);
...
async function getTree({ id }) {
  ...
  if (desc.branch.component) {
    b_array.forEach(item => {
      item.component = datagetter.chooseTreeItemComponent(item, desc.branch);
    });
  }
