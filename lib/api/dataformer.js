...
function getRealtimeValues(table, id, nodeid, holder) {
  ...
  switch (table) {
    case 'devicecommonTable':
      if (!holder.devSet[nodeid]) throw {err:'SOFTERR', message:'Not found device '+nodeid+' in holder.devSet '}
...
async function getSubTree({ id, nodeid }) {
  if (id == 'deviceprops') return getDevpropsSubTree(nodeid);
...
async function getDevpropsSubTree(nodeid) {
 // Поддерево свойств - Плоское не редактируемое без папок  
  ...
  if (docs && docs[0]) {
    ...
    Object.keys(props).forEach(prop => {
      order++;
      data.push({id:prop, title:prop, order, component:'devproptochannellink.'+nodeid+'_'+prop})
    });
  }
