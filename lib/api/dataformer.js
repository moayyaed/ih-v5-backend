...
async function getRecordByForm(query, holder) {
  ...
  } else if (formId == 'formIntegration' && navnodeid) {
    // id=formIntegration&navnodeid=d0335&nodeid=mobile
    // Создать запись, если ее нет
    
    const doc = await dm.datamaker.findOrAddDoc({ collection: 'integrations', id:nodeid+'_'+navnodeid,  nodeid, navnodeid }, dm);
    nodeid = doc._id;
  }
 // console.log(formId+' getRecordByForm preNodeid ' + preNodeid + ' nodeid=' + nodeid);
    ...
    for (const cell of formMetaData.grid) {
      ...
      if (cell.table && !dataFromTable[cell.table]) {
        ...
        if ((nodeid && nodeid != 'undefined') || cell.nodeid) {
          ...
          dataFromTable[cell.table] = await getDataFromTree(desc.tree, '', dm);
        }
      }
    }
  ...
  async function findOne(collection) {
    ...
    /*
    if (!query.navnodeid) return dm.dbstore.findOne(collection, { _id: nodeid });
   
    const rec = await dm.dbstore.findOne(collection, { _id: query.navnodeid });
    if (rec) {
      return rec.props && rec.props[query.nodeid] ? rec.props[query.nodeid] : '';
    }
    */
  }
  ...
  async function getExtlog(param, lid) {
    try {
      ...
      if (lid && lid.startsWith('__UNIT_')) {
        const name = lid.substr(7);
        logfile =logpath + '/ih_' + name + '.log' ;
      } else {
        logfile = param == 'dblog' ? logpath + '/ih_' + lid + '.log' : logpath + '/ih.log';
      }
      */
...
async function getImagegrid(nodeid, dm) {
  const treeData = await getDataFromTree('images', nodeid, dm);
