...
async function getRecordByForm(query, holder) {
    ...
    for (const cell of formMetaData.grid) {
      ...
      if (nodeid && cell.table && !dataFromTable[cell.table]) {
        ...
        if (desc.store == 'db') {
          ...
          if (virttables[cell.table]) {
            dataFromTable[cell.table] = await virttables[cell.table]([], cell.table, nodeid, '', holder);
          }
        }
      ...
      for (const item of formMetaData[cell.id]) {
        if (item.type == 'table') {
          ...
          if (item.prop == 'handler') {
            data[cell.id][item.prop] = await getHandler(preNodeid);
          } else if (item.prop == 'extlog') {
            data[cell.id][item.prop] = await getExtlog('/tmp/akumuli.log', 4000);
            // data[cell.id][item.prop] = await fut.readLogTail('/tmp/akumuli.log', 4000);
          } else {
  ...
  async function getHandler(preId) {
    const [typeId, prop] = preId.split('.');
    return typestore.getHandlerStr(typeId, prop);
  }

  async function getExtlog() {
    try {
      const log = await fut.readLogTail('/tmp/akumuli.log', 40000);
      return log;
    } catch (e) {
  ...
  function foundData(table, prop, cNodeid) {
    ...
    if (record && datautil.isLink(cNodeid) && virttables[table]) {
      return true;
    }
  ...
  async function getData(table, item, cNodeid) {
    ...
    if (datautil.isLink(cNodeid) && virttables[table]) {
      record = await virttables[table](dataFromTable, table, cNodeid, item, holder);
    }
...
async function getDataFromTree(treeId, nodeid) {
  // данные берем из дерева в форме плоской таблицы с полем  path
  // получить все листья из вложенных папок, затем в том же порядке их показать
  const treeRootItem = await getCachedTree(treeId);
...
async function getTree({ id }) {
  ...
  const dataArr = await getDataArr(desc, b_desc, l_desc);
...
async function getBranchTree(id, desc, b_desc) {
  const dataArr = [
...
async function getDataArr(desc, b_desc, l_desc) {
  const promises =
    l_desc.store == 'none'
      ? [virttables[desc.branch.table](), virttables[desc.leaf.table]()]
      : [
          dm.dbstore.getData(
            Object.assign({}, b_desc, { order: 'order', fields: hut.getFieldProjection(desc.branch.propmap) })
          ),
