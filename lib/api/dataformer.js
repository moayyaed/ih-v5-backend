...
async function getRecordByForm(query, holder) {
    ...
    for (const cell of formMetaData.grid) {
      ...
      for (const item of formMetaData[cell.id]) {
        if (item.type == 'table') {
          ...
          data[cell.id][item.prop] = await getExtlog(item.param);
  ...
  async function getExtlog(param) {
    try {
      const logfile =  param == 'dblog' ? '/opt/intrahouse-d/log/ih_influx.log' : '/opt/intrahouse-d/log/ih.log';
