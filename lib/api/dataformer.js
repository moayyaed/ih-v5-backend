    ...
    for (const cell of formMetaData.grid) {
      ...
      for (const item of formMetaData[cell.id]) {
        if (item.type == 'table') {
          ...
          data[cell.id][item.prop] = await getProjectObj(item.type, nodeid);
...
function getProjectObj(id, nodeid) {
  // Загрузить объект из соотв папки проекта. TODO из кэша взять!??
  const folders = ['layout', 'container'];
  if (!folders.includes(id)) throw {err:'SOFTERR', message:'Unknkwn exttype: '+id};
