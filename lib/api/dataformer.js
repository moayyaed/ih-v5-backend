...
async function getRecordByForm(query, holder) {
  ...
  async function getData(table, item, cNodeid) {
    ...
    const value = isDerivative(item.prop) ? derivativeValue(item.prop, item) : record[item.prop];
  ...
  function derivativeValue(prop, item) {
    switch (prop) {
      case '__idprefix': return item._id && item._id.indexOf('@') ? item._id.split('@').unshift() : '';
      default: return '';
    }
  }
