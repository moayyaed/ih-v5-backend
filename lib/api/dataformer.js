...
async function getRecordByForm(query, holder) {
  ...
  // console.log('getRecordByForm preNodeid ' + preNodeid + ' nodeid=' + nodeid);
  ...
  async function getMarkdown(table, preId) {
    if (query.navnodeid) {
      const lang = query.nodeid;
      const filename = appconfig.getDocPageFilename(query.navnodeid, lang);
...
function formTreeItemsArray(dataArr, desc, dm) {
  ...
  dataArr.forEach(item => {
    ...
    if (desc.copylink) {
      item.copylink = '![' + item._id + '](/' + item._id + ')';
    }
  });
