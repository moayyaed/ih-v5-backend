...
async function insert(body) {
  ...
  if (body.type == 'subtree') {
    ...
    console.log('WARN: tm.insert '+util.inspect(body))
    for (const rootid in payload) {
      ...
      if (payload[rootid].folders && payload[rootid].folders.length) {
        res[desc.branch.table] = { docs: await datamaker.createNewDocsFromTree(payload[rootid].folders, desc.branch.table, parentid) };
      }
...
async function copy(body) {
  ...
  async function docsToCopy(treeObj, payloadItem, desc) {
    ...
    if (payloadItem.nodes) {
      res[desc.leaf.table] = { docs: await datamaker.copyDocsFromTree(payloadItem.nodes, desc.leaf.table, targetid, seqObj) };
    }
