...
async function insert(body) {
  ...
  if (body.type == 'subtree') {
    ...
    res[desc.branch.table] = {
      docs: await datamaker.createNewDocsFromTree(newDocs, desc.branch.table, parentid)
    };
  } else {
    for (const rootid in payload) {
      ...
      if (payload[rootid].folders && payload[rootid].folders.length) {
        res[desc.branch.table] = {
          docs: await datamaker.createNewDocsFromTree(payload[rootid].folders, desc.branch.table, parentid)
        };
      }
      if (payload[rootid].nodes && payload[rootid].nodes.length) {
        // Передали как nodes, но возможно добавляю как folder (для мультиплагинов)
        const docs = await datamaker.createNewDocsFromTree(payload[rootid].nodes, desc.leaf.table, parentid);
        docs.forEach(doc => {
          if (doc.folder) {
        ...
        // res[desc.leaf.table] = { docs: await datamaker.createNewDocsFromTree(payload[rootid].nodes, desc.leaf.table, parentid) };
      }
    }
  }
  return { res };
...
async function copy(body) {
  ...
  async function docsToCopy(treeObj, payloadItem, desc) {
    ...
    if (payloadItem.nodes) {
      res[desc.leaf.table] = {
        docs: await datamaker.copyDocsFromTree(payloadItem.nodes, desc.leaf.table, targetid, seqObj)
      };
    }
  ...
  async function copyWithChildren(node, branchTable, leafTable, target) {
    ...
    const newDoc = await datamaker.copyOneFromTree(node, table, target, seqObj);
  ...
  async function docsToClone(treeObj, payloadItem, desc) {
    console.log('payloadItem ='+util.inspect(payloadItem)+' payloadItem.folders.length='+payloadItem.folders.length)
  ...
  async function cloneWithChildren(node, branchTable, leafTable, target) {
    ...
    const newDoc = await datamaker.cloneOneFromTree(node, table, target, body, seqObj);
    ...
    if (!res[table]) res[table] = { docs: [] };
    for (const child of node.children) {
      res[table].docs.push(await datamaker.cloneOneFromTree(child, table, parent,  body));
    }
  }
}
