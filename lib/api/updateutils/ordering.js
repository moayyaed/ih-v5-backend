 ...
 */
async function exec({ payloadItem, treeObj, treeDesc, parentid, previd, seq }, dm) {
  ...
  let shifted;
  if (needReorder) {
    shifted = await reordering(targetObj.children, targetIdx, treeDesc, startOrder + qItems * delta, dm);
  }
...
function calculateOrder(children, previd, qItems = 1) {
  ...
  let needReorder;
  if (delta < 2) {
    // Может быть и отрицательное значение
    delta = 1000;
    needReorder = true;
  }
 ...
 */
async function reordering(children, targetIdx, desc, nextOrder, dm) {
  ...
  if (oneTable) {
    await updateNodeOrders(l_arr, desc.leaf.table, dm);
  } else {
    if (l_arr.length) await updateNodeOrders(l_arr, desc.leaf.table, dm); // таблицы разные!!
    if (b_arr.length) await updateNodeOrders(b_arr, desc.branch.table, dm);
  }
  return reorder;
}

async function updateNodeOrders(nodes, table, dm) {
  const tdesc = descriptor.getDescItem('table', table);

  try {
    for (const item of nodes) {
      if (item.nodeid && !item.nodeid.startsWith('lost_')) {
