...
module.exports = async function getMetaUpForm({ id, nodeid }, dm) {
  // Получить саму форму
  ...
  for (const cell of formMetaData.grid) {
    if (formMetaData[cell.id]) {
      ...
      formMetaData[cell.id].forEach(item => {
        if (item.type == 'table') {
          tables.push({ cell: cell.id, table: cell.table, prop: item.prop, genfield:item.genfield, genfilter:item.genfilter });
          ...
          exfieldtype[item.prop] = { type: item.type, cell: cell.id };
        } else if (cell.table && item.type != 'header') {
