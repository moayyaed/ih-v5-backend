...
function derivativeValue(prop, record) {
  ...
  switch (prop) {
    ...
    // case '__expert':  Теперь этот атрибут на уровне пользователя
    //     return appconfig.get('expert');
    default:
      return '';
  }
}
...
function getLogFilename(param, lid) {
  ...
  if (lid && lid.startsWith('__UNIT_')) {
    ...
    if (name == 'mainprocess') {
      logfile = 'ih';
    } else if (name == 'dbagent') { // TODO - Может быть другой агент! 
      logfile = 'ih_sqlite';
...
/*
async function calcNewInstanseId(collection, prefix) {
  const docs = await dm.dbstore.get(collection);
  return hut.calcNewId(docs, '_id', prefix);
}


async function getRecord(table, nodeid) {
  return dm.findRecordById(table, nodeid);
}
*/
