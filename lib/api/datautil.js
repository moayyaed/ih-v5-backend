...
function derivativeValue(prop, record) {
  ...
  switch (prop) {
    case '__exid': return record._id;
    case '__expref': return appconfig.get('project_prefix');
    case '__impref': return hut.prefAt(record._id);
    case '__currentproject': return appconfig.get('project');
    default: return '';
  }
}
...
function getDroplist(listname) {
  ...
  } else if (listname == 'devicedbList') {
      liststore.getListAsArray('deviceList').forEach(item => {
        res.push({ id: item.dn+'.value', title: item.dn+'.value' });
        res.push({ id: item.dn+'.state', title: item.dn+'.state' });
      });
    }
  return { data: res };
}
