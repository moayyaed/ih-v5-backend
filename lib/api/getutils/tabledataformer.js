...
async function unitchannels(dataFromTable, table, nodeid, pitem) {
    ...
    .map(item => {
      ...
      columns.forEach(col => {
        if (col.type == 'link') {
          ...
          row[col.prop] = dataFromTable.realtime ? getRealtimeVal(col.prop, item) : '';
        } else if (item[col.prop] != undefined) {
  ...
  function getRealtimeVal(prop, item) {
    if (!dataFromTable.realtime) return '';
    const id = item.chan;
    if (id && dataFromTable.realtime[id]) {
      // realtime_value, realtime_ts
      if (prop.endsWith('ts') && dataFromTable.realtime[id].ts) {
        return hut.getDateTimeFor(new Date(dataFromTable.realtime[id].ts));
      }
      if (prop.endsWith('value')) {
        return dataFromTable.realtime[id].value;
      }
    }
    return '';
  }
