...
function getGenfieldObjFromDataRecord(dataFromTable, table, pitem) {
  ...
  console.log('genfilter='+util.inspect(genfilter));
...
function getFilteredObject(obj, filter) {
  console.log('getFilteredObject obj='+util.inspect(obj));

  const result = {};
  Object.keys(obj).forEach(prop => {
    console.log('getFilteredObject prop='+prop);
...
async function devicecommon(dataFromTable, table, nodeid, pitem, holder) {
  ...
  arr.forEach(item => {
    ...
    if (typeItem.props[item.prop]) {
      ...
      item.type_op = { title: typeItem.props[item.prop].command ? 'Command' : typeItem.props[item.prop].op.toUpperCase(), path: typePath };
    }
    // Данные канала
    if (hObj[item.prop]) {
...
/*
async function devicehw(dataFromTable, table, nodeid, pitem, holder) {
  let pObj = getGenfieldObjFromDataRecord(dataFromTable, table, pitem);
  if (!pObj) pObj = {};
  const arr = transfromGenfieldObjToArray(pObj);

  // Получить данные из devhard
  // Добавить данные каналов
  const hrec = await dm.dbstore.get('devhard', { did: nodeid });
  const hObj = hut.arrayToObject(hrec, 'prop');

  console.log('deviceHW 0 arr=' + util.inspect(arr));
  console.log('deviceHW hObj=' + util.inspect(hObj));
  arr.forEach(item => {
    const prop = item.id;
    // Данные канала
    if (hObj[prop]) {
      const unit = hObj[prop].unit;
      const chan = hObj[prop].chan;
      item = Object.assign(item, { unit, chan }); // И все остальные свойства кроме _id??
    }
  });
  console.log('deviceHW 1 arr=' + util.inspect(arr));
  // Сформировать данные по столбцам
  return formColumnVals(pitem.columns, arr);
}
*/
...
function formLinkObj(table, colItem, item, val) {
  ...
  switch (colItem.prop) {
    ...
    case 'did':
      return { title: datagetter.getDeviceTitle(val), path: 'dev/devices/deviceview/' + val + '/tabDeviceCommon' };
    case 'chan':
...
function getDeviceLinkObj(did) {
  return { title: datagetter.getDeviceTitle(did), path: 'dev/devices/deviceview/' + did + '/tabDeviceCommon' };
}
