...
const typestore = require('../devices/typestore');
const scenefs = require('../scene/scenefs');
...
async function createOneRecord(table, tdesc, item, parentid) {
  ...
  switch (table) {
    ...
    case 'scene':
        ...
        await scenefs.createNewScriptFromTemplate(doc._id);
        return doc;
...
async function copypasteDoc(table, doc, target, seqObj) {
  ...
  switch (table) {
    ...
    case 'scene':  
      ...
      await scenefs.copyScript(oldId, doc._id);
      break;
    default:
      doc.name += ' (copy)'; // name+copy
  }
