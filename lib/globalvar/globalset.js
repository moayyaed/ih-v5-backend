...
class Globalset {
  ...
  addHandler(gl_did) {
    ...
    if (filename) {
      this.handlersOnChange[gl_did] = require(filename);
    }
  }
  ...
  runHandlersOnChange(changed) {
    ...
    for (const gl_did of toRun) {
      // Запуск функции 
      if (this.handlersOnChange[gl_did]) {
        const res = this.handlersOnChange[gl_did](this.getValue(gl_did), this.holder.dnSet);
