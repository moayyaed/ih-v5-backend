...
class Globalset {
  ...
  addHandler(gl_did) {
    const filename =  appconfig.getHandlerFilenameIfExists(gl_did);
  ...
  runHandlersOnChange(changed) {
    ...
    for (const gl_did of toRun) {
      // Запуск функции 
      if (this.handlersOnChange[gl_did].handler) {
        const handler = this.handlersOnChange[gl_did].handler;
        console.log('RUN HANDLER devs='+util.inspect(this.handlersOnChange[gl_did].devs))
