...
async function prepareMobileData(dm) {
  ...
  rebuidDeviceMobileLists(dm);
  rebuidSceneMobileLists(dm);
}
...
async function rebuidDeviceMobileLists(dm) {
  ...
  for (const img of imageSet) {
    ...
    if (imgItem && img) {
      ...
      // mobile_devicesimagelist.push({ img, mtime:ts });
    }
  }
...
function getV4Type(doc) {
  switch (doc.cl) {
    case 'ActorD':
    case 'ActorA': return 500;
    case 'RGB': return 530;
    case 'SensorA': return 200;
    default: return 100;
  }
}
...
async function rebuidSceneMobileLists(dm) {
  ...
  for (const doc of docs) {
    ...
    const item = {id:doc._id, name:doc.mobname || doc.name, group}
    mobile_scenes.push(item);
  }

  const mobile_scenegroups = Array.from(scenegroups).map(el => ({ id: el, name: el }));
