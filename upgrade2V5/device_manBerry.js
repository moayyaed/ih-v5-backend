  ...
  load(project_c) {
    ...
    tut.getSourceData('places', 'jbase', project_c).forEach(item => {
      if (item.num && item.num != 0) {
        item.id = item.num;
        placeObj[tut.getPlaceId(item)] = item.name;
      }
    });
    tut.getSourceData('rooms', 'jbase', project_c).forEach(item => {
      if (item.num && item.num != 0) {
        item.id = item.num;
        placeObj[tut.getRoomId(item)] = item.name;
      }
    });

    const devrefData = tut.getSourceData('devref', 'jbase', project_c);
    console.log('devrefData '+util.inspect(devrefData))
    ...
    devrefData.forEach(item => {
      ...
      const _id = tut.getNewId('d', 4, idNum++);
      const type = 't' + item.kind; // tSensorA - новый тип - для создания устройства по типу
      const oldtype = item.type;
      const tags = item.subs && subsObj[item.subs] ? [subsObj[item.subs]] : [];
      let name = item.dn;
      if (item.note) {
        name = (typeof item.note == 'object') ? item.note.ru : item.note;
      } 
      const dobj = { _id, parent, type, dn: item.dn, name, tags, oldtype };
      console.log(item.dn+' '+util.inspect(dobj))
      if (clObj[item.kind]) {
        dobj.props = formProps(item, Object.keys(clObj[item.kind].props));
        oldDevMap[item.dn] = dobj;
      } else {
        console.log('Not found '+item.kind+' dn='+item.dn )
      }
...
function isAnalog(item) {
  return item.kind == 'SensorA' || item.kind == 'ActorA';
}
